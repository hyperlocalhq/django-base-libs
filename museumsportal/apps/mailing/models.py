...
class EmailTemplatePlaceholder(SysnameMixin(verbose_name=_("Placeholder Internal Name"), prepopulate_from=("name",))):
    ...
    name = MultilingualCharField(_('Placeholder Name'), max_length=64, unique=False)
    ...
    def __unicode__(self):
        return getattr(self, "name_%s" % get_current_language())
...
class EmailTemplate(SingleSiteMixin, SlugMixin(prepopulate_from=("name",))):
    ...
    subject_fr = models.CharField(_("Subject (French)"), max_length=255)
    subject_pl = models.CharField(_("Subject (Polish)"), max_length=255)
    subject_tr = models.CharField(_("Subject (Turkish)"), max_length=255)
    subject_es = models.CharField(_("Subject (Spanish)"), max_length=255)
    subject_it = models.CharField(_("Subject (Italian)"), max_length=255)
    body = PlainTextModelField(_("Template Text (English)"), blank=True)
    body_de = PlainTextModelField(_("Template Text (German)"), blank=True)
    body_fr = PlainTextModelField(_("Template Text (French)"), blank=True)
    body_pl = PlainTextModelField(_("Template Text (Polish)"), blank=True)
    body_tr = PlainTextModelField(_("Template Text (Turkish)"), blank=True)
    body_es = PlainTextModelField(_("Template Text (Spanish)"), blank=True)
    body_it = PlainTextModelField(_("Template Text (Italian)"), blank=True)
    ...
    body_html_fr = models.TextField(_("Template HTML (French)"), null=True, blank=True)
    body_html_pl = models.TextField(_("Template HTML (Polish)"), null=True, blank=True)
    body_html_tr = models.TextField(_("Template HTML (Turkish)"), null=True, blank=True)
    body_html_es = models.TextField(_("Template HTML (Spanish)"), null=True, blank=True)
    body_html_it = models.TextField(_("Template HTML (Italian)"), null=True, blank=True)
    timestamp = models.DateTimeField(_("Written"), auto_now_add=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.body_fr:
            self.body_fr = html_to_plain_text(self.body_html_fr)
        if not self.body_pl:
            self.body_pl = html_to_plain_text(self.body_html_pl)
        if not self.body_tr:
            self.body_tr = html_to_plain_text(self.body_html_tr)
        if not self.body_es:
            self.body_es = html_to_plain_text(self.body_html_es)
        if not self.body_it:
            self.body_it = html_to_plain_text(self.body_html_it)
