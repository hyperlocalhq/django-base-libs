    ...
    def __init__(self, media_file_obj=None, *args, **kwargs):
        ...
        layout_blocks.append(layout.Fieldset(
            ...
            <div class="row row-md">
                ...
                <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                    {% if media_file.path %}
                        {% parse "{{ event.get_url_path }}change/" as goto_next %}
        ...
        layout_blocks.append(bootstrap.FormActions(
            ...
            layout.HTML(u"""{% load i18n base_tags image_modifications %}
                {% if media_file %}
                    {% parse "{{ event.get_url_path }}change/" as goto_next %}
                    <input type="button" id="button-id-delete-photo" class="delete_photo btn btn btn-lg btn-info" data-href="{{ event.get_url_path }}gallery/file_{{ media_file.get_token }}/delete/" value="{% trans "Delete" %}" />&zwnj;
                    <!-- Modal -->
                            ...
                            <div class="cell">
                                <div class="inner">
                                    ...
                                    <div class="modal-footer">
                                        <button class="btn btn-lg btn-primary" id="button-id-confirm-deletion">{% trans "Yes, Please" %}</button>
                                        <button class="btn btn-lg btn-info" name="cancel" data-dismiss="modal" aria-hidden="true">{% trans "No, Thanks" %}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
