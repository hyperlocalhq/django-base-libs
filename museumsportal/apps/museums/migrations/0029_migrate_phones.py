...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for m in orm.Museum.objects.all():
            # parse all phone numbers
            if m.phone:
                m.phone_country, m.phone_area, m.phone_number = self.parse_phone_number(m.phone)
            if m.fax:
                m.fax_country, m.fax_area, m.fax_number = self.parse_phone_number(m.fax)
            if m.group_bookings_phone:
                m.group_bookings_phone_country, m.group_bookings_phone_area, m.group_bookings_phone_number = self.parse_group_bookings_phone_number(m.group_bookings_phone)
            if m.service_phone:
                m.service_phone_country, m.service_phone_area, m.service_phone_number = self.parse_service_phone_number(m.service_phone)
            # quick hacks for non-working empty translatable fields
            if not m.service_books_info_en:
                m.service_books_info_en = u""
            if not m.accessibility_en:
                m.accessibility_en = u""
            if not m.service_cafe_info_en:
                m.service_cafe_info_en = u""
            if not m.yearly_ticket_en:
                m.yearly_ticket_en = u""
            if not m.service_rent_info_en:
                m.service_rent_info_en = u""
            # save
            m.save()
