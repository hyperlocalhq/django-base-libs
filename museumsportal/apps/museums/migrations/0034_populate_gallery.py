...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for museum in orm.Museum.objects.all():
            if museum.image:
                rel_dir = "museums/%s/gallery/" % museum.slug
                abs_path = os.path.join(settings.MEDIA_ROOT, museum.image.path)
                ...
                new_file = FileObject(os.path.join(rel_dir, museum.image.path.rsplit("/")[-1]))
                ...
                if museum.image_caption:
                    FileDescription = orm['filebrowser.FileDescription']
                    fd, created = FileDescription.objects.get_or_create(
                        file_path=new_file,
                        defaults=dict(
                            description_de=strip_html(museum.image_caption_de),
                            description_en=strip_html(museum.image_caption_en),
                            )
                        )
    ...
    complete_apps = ['filebrowser', 'museums']
