...
class EncryptEmail(template.Node):
    ...
    def render(self, context):
        ...
        if len(email_address) > letter_count:
            script += u'document.getElementById("'+id+'").innerHTML="<a href=\\"mailto:"+d+"\\">"+d.slice(0,' + unicode(letter_count - 1) +u')+"…</a>";'
        else:
            script += u'document.getElementById("'+id+'").innerHTML="<a href=\\"mailto:"+d+"\\">"+d+"</a>";'
        
        script = u"eval(\""+ script.replace("\\","\\\\").replace('"','\\"') + u"\")"
...
def  encrypt_email(parser, token):
    ...
    if len(tokens) > 3:
        raise template.TemplateSyntaxError("%r tag accepts one or two arguments" % tokens[0])
