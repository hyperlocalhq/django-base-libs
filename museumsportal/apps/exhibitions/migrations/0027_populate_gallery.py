...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for exhibition in orm.Exhibition.objects.all():
            if exhibition.image:
                rel_dir = "exhibitions/%s/gallery/" % exhibition.slug
                abs_path = os.path.join(settings.MEDIA_ROOT, exhibition.image.path)
                new_abs_dir_path = os.path.join(settings.MEDIA_ROOT, rel_dir)
                # copy file to new location
                FileManager.path_exists(new_abs_dir_path)
                # if not the same file
                try:
                    shutil.copy2(abs_path, new_abs_dir_path)
                except:
                    pass
                
                new_file = FileObject(os.path.join(rel_dir, exhibition.image.path.rsplit("/")[-1]))
                
                # create media file
                mf = orm.MediaFile.objects.create(
                ...
                if exhibition.image_caption:
                    FileDescription = orm['filebrowser.FileDescription']
                    fd, created = FileDescription.objects.get_or_create(
                        file_path=new_file,
                        defaults=dict(
                            description_de=strip_html(exhibition.image_caption_de),
                            description_en=strip_html(exhibition.image_caption_en),
                            )
                        )
    ...
    complete_apps = ['filebrowser', 'exhibitions']
