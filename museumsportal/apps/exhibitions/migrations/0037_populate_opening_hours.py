...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for exhibition in orm.Exhibition.objects.all():
            if not exhibition.museum:
                continue
            if not exhibition.season_set.count():
                # prepopulate seasons from museum
                for museum_season in exhibition.museum.season_set.all():
                    exhibition_season = orm.Season(exhibition=exhibition)
                    for field_name in museum_season.__dict__:
                        if not field_name.startswith("_") and not field_name.endswith("id"):
                            setattr(exhibition_season, field_name, getattr(museum_season, field_name))
                    exhibition_season.save()
            if not exhibition.specialopeningtime_set.count():
                # prepopulate special opening times from museum
                for museum_sot in exhibition.museum.specialopeningtime_set.all():
                    exhibition_sot = orm.SpecialOpeningTime(exhibition=exhibition)
                    for field_name in museum_sot.__dict__:
                        if not field_name.startswith("_") and not field_name.endswith("id"):
                            setattr(exhibition_sot, field_name, getattr(museum_sot, field_name))
                    exhibition_sot.save()
    ...
    complete_apps = ['museums', 'exhibitions']
