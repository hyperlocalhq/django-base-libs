...
(function($, undefined) {
    ...
    $(window).load(function() {
        ...
        $('.search', $filters).on('submit', function(e, dont_load_data_yet){
            e.preventDefault();
            ...
            var q = $('.search input', $filters).val();
        ...
        $('#sorting a', $filters).on(activation_event, function(e, dont_load_data_yet){
            e.preventDefault();
            if (filtering_busy && !dont_load_data_yet) {
                return false;
            }
            ...
            sort_by = $list_item.data('value');
            $('#sorting .active').removeClass('active');
            $list_item.addClass('active');

            url_filters['page'] = ''; // reset pagination
            url_filters['sort_by'] = sort_by;
        ...
        // filter buttons
        $('.filter a', $filters).on(activation_event, function(e, dont_load_data_yet){
            e.preventDefault();
            if (filtering_busy && !dont_load_data_yet) {
                return;
            }
    ...
    //            http_state_filters.push(filters[prop].join(''));
        ...
        $('#filter-reset').on(activation_event, function(e) {
            ...
            $('.search input', $filters).val('');
            url_filters['q'] = ''; // Reset q
            var url = '?' + window.append_to_get(url_filters, true);
            window.history.pushState({}, document.title, url);
            $('#container').load(url + ' #container>*', function() {
                reinit_infinite_scroll();
