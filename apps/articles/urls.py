...
if ARTICLES_HAVE_TYPES:
    urlpatterns = patterns(
        path_in_installed_app('articles.views'),
        ...
        url(
            r'^(?P<type_sysname>[^/]+)/$', 'article_archive_index',
            dict(num_latest=5)
        ),

        # articles aggregated date-based by year
        url(
            r'^(?P<type_sysname>[^/]+)/(?P<year>\d{4})/$',
            'article_archive_year', dict(paginate_by=5)
        ),

        # articles aggregated date-based by month
        url(
            r'^(?P<type_sysname>[^/]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/$',
            'article_archive_month', dict(paginate_by=5)
        ),

        # articles aggregated date-based by day
        url(
            r'^(?P<type_sysname>[^/]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/$',
            'article_archive_day', dict(paginate_by=5)
        ),

        # article details
        url(
            r'^(?P<type_sysname>[^/]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/(?P<article_slug>[^/]+)/$',
            'article_object_detail',
            name='article_object_detail'
        ),
        url(
            r'^(?P<type_sysname>[^/]+)/feeds/(?P<feed_type>rss|atom)/$',
    ...
    urlpatterns = patterns(
        ...
        # articles aggregated date-based by year
        url(r'^(?P<year>\d{4})/$', 'article_archive_year', dict(paginate_by=5)),

        # articles aggregated date-based by month
        url(
            r'^(?P<year>\d{4})/(?P<month>\d{1,2})/$', 'article_archive_month',
            dict(paginate_by=5)
        ),
