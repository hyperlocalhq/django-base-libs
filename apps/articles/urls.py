...
urlpatterns = patterns('ccb.apps.articles.views',
    ...
    url(r'^creative-sector/(?P<creative_sector_slug>[^/]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/$', 
        'article_archive_day', 
        dict(paginate_by=10)
    ),
    
    # articles for creative sector details
    url(r'^creative-sector/(?P<creative_sector_slug>[^/]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/(?P<article_slug>[^/]+)/$', 
        'article_object_detail', 
    ),
