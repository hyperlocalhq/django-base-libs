...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Term = orm['structure.term']
        for article in orm.Article.objects.all():
            article_type_id = article.article_type_id
            try:
                orm.ArticleType.objects.get(pk=article_type_id)
            except orm.ArticleType.DoesNotExist:
                try:
                    term = Term.objects.get(pk=article_type_id)
                except Term.DoesNotExist:
                    article.article_type = None
                else:
                    article_type = orm.ArticleType()
                    article_type.id = term.id
                    article_type.creation_date = datetime.datetime.now()
                    article_type.sort_order = term.sort_order
                    article_type.slug = term.slug
                    article_type.title = term.title
                    for language in settings.LANGUAGES:
                        setattr(
                            article_type,
                            "title_%s" % language[0],
                            getattr(term, "title_%s" % language[0], ""),
                            )
                    article_type.save()
    ...
    def backwards(self, orm):
        if not db.dry_run:
            for article_type in orm.ArticleType.objects.all():
                article_type.delete()
    ...
    complete_apps = ['structure', 'articles']
