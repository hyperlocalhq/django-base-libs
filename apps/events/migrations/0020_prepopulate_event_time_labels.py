...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for term in orm['structure.term'].objects.filter(
            vocabulary__sysname='event_time_labels',
            ):
            etl = orm.EventTimeLabel()
            etl.id = term.id
            etl.creation_date = datetime.datetime.now()
            etl.title_en = term.title_en
            etl.title_de = term.title_de
            etl.title = term.title
            etl.slug = term.slug
            etl.sort_order = sort_order
            etl.save()
            sort_order += 1
