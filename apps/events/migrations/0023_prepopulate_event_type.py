...
def _get_path_search(node):
    pk_list = []
    pk_list.append(str(node.pk))
    while node.parent:
        node = node.parent
        pk_list.append(str(node.pk))
    pk_list.reverse()
    return "/%s/" % "/".join(pk_list)
    
class Migration(DataMigration):
    
    def forwards(self, orm):
        parent = orm['structure.term'].objects.get(
            vocabulary__sysname="basics_object_types",
            sysname="event",
            )
        parent_path = "/%d/" % parent.id
        for term in orm['structure.term'].objects.filter(
            vocabulary__sysname='basics_object_types',
            path_search__contains=parent_path,
            ).exclude(sysname="event"):
            et = orm.EventType()
            et.id = term.id
            et.title = term.title
            et.title_en = title(term.slug) # WTF?
            et.title_de = term.title_de
            et.slug = term.slug
            et.sort_order = term.sort_order
            if term.parent != parent:
                et.parent_id = term.parent_id
            et.path = get_path(et)
            et.path_search = _get_path_search(et)
            et.save()    
    ...
    models = {
        ...
        'events.eventtype': {
            'Meta': {'ordering': "['path', 'sort_order']", 'object_name': 'EventType'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'parent': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'child_set'", 'null': 'True', 'to': "orm['events.EventType']"}),
        ...
        'institutions.institutiontype': {
            'Meta': {'ordering': "['path', 'sort_order']", 'object_name': 'InstitutionType'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'parent': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'child_set'", 'null': 'True', 'to': "orm['institutions.InstitutionType']"}),
            'path': ('django.db.models.fields.CharField', [], {'max_length': '8192', 'null': 'True'}),
            'path_search': ('django.db.models.fields.TextField', [], {'blank': 'True'}),
            'slug': ('django.db.models.fields.SlugField', [], {'unique': 'True', 'max_length': '255', 'db_index': 'True'}),
            'sort_order': ('django.db.models.fields.IntegerField', [], {'blank': 'True'}),
        ...
        'people.individualtype': {
            'Meta': {'ordering': "['path', 'sort_order']", 'object_name': 'IndividualType'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'parent': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'child_set'", 'null': 'True', 'to': "orm['people.IndividualType']"}),
            'path': ('django.db.models.fields.CharField', [], {'max_length': '8192', 'null': 'True'}),
            'path_search': ('django.db.models.fields.TextField', [], {'blank': 'True'}),
            'slug': ('django.db.models.fields.SlugField', [], {'unique': 'True', 'max_length': '255', 'db_index': 'True'}),
            'sort_order': ('django.db.models.fields.IntegerField', [], {'blank': 'True'}),
        ...
        'people.person': {
            ...
            'individual_type': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['people.IndividualType']", 'null': 'True', 'blank': 'True'}),
