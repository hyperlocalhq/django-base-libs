...
class DatePeriodNode(template.Node):
    def __init__(
        self,
        var_name,
        start_yyyy_lookup_var,
        start_mm_lookup_var,
        start_dd_lookup_var,
        end_yyyy_lookup_var=None,
        end_mm_lookup_var=None,
    ...
    def _get_start_date_formatted(self):
        ...
        start_date = datetime(
            int(self.start_yyyy or tz_now().year),
    ...
    def _get_end_date_formatted(self):
        ...
        try:
            end_date = datetime(
                int(self.end_yyyy or tz_now().year),
    ...
    def render(self, context):
        ...
        if self.start_yyyy_lookup_var:
            try:
                self.start_yyyy = template.resolve_variable(
                    self.start_yyyy_lookup_var, context
                )
        ...
        if self.start_mm_lookup_var:
            try:
                self.start_mm = template.resolve_variable(
                    self.start_mm_lookup_var, context
                )
            except template.VariableDoesNotExist:
                return ''
        ...
        if self.start_dd_lookup_var:
            try:
                self.start_dd = template.resolve_variable(
                    self.start_dd_lookup_var, context
                )
            except template.VariableDoesNotExist:
                return ''
        ...
        if self.end_yyyy_lookup_var:
            try:
                self.end_yyyy = template.resolve_variable(
                    self.end_yyyy_lookup_var, context
                )
            except template.VariableDoesNotExist:
                return ''
        ...
        if self.end_mm_lookup_var:
            try:
                self.end_mm = template.resolve_variable(
                    self.end_mm_lookup_var, context
                )
            except template.VariableDoesNotExist:
                return ''
        ...
        if self.end_dd_lookup_var:
            try:
                self.end_dd = template.resolve_variable(
                    self.end_dd_lookup_var, context
                )
            except template.VariableDoesNotExist:
                return ''
...
class InstitutionNode(template.Node):
    ...
    def render(self, context):
        ...
        if self.institution_id_lookup_var:
            try:
                self.institution_id = template.resolve_variable(
                    self.institution_id_lookup_var, context
                )
            except template.VariableDoesNotExist:
                return ''
...
class DoGetDatePeriodFormatted:
    ...
    def __call__(self, parser, token):
        ...
        if not len(tokens) in (11, 7):
            raise template.TemplateSyntaxError, "%r tag requires 6 or 10 arguments" % tokens[
                0]
        if tokens[1] != 'from':
            raise template.TemplateSyntaxError, "first argument in %r tag must be 'from'" % tokens[
                0]

        if len(tokens) == 7:
            if tokens[6] != 'as':
                raise template.TemplateSyntaxError, "sixth argument in %r tag must be 'as'" % tokens[
                    0]
            else:
        ...
        if len(tokens) == 11:
            if tokens[9] != 'as':
                raise template.TemplateSyntaxError, "10th argument in %r tag must be 'as'" % tokens[
                    0]
            elif tokens[5] != 'to':
                raise template.TemplateSyntaxError, "fifth argument in %r tag must be 'to'" % tokens[
                    0]
            else:
                return DatePeriodNode(
                    tokens[10], tokens[2], tokens[3], tokens[4], tokens[6],
                    tokens[7], tokens[8]
                )
...
class DoGetInstitution:
    ...
    def __call__(self, parser, token):
        ...
        """
        if len(tokens) != 4:
            raise template.TemplateSyntaxError, "%r tag requires 3 arguments" % tokens[
                0]
        if tokens[2] != 'as':
            raise template.TemplateSyntaxError, "second argument in %r tag must be 'as'" % tokens[
                0]
