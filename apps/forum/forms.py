...
class ForumOptionsForm(dynamicforms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.container:
            kwargs.pop('container') #we must not pass extra kwargs to the super constructor!!!
        #TODO!!!!!
...
class ForumForm(dynamicforms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        forums_with_threads = [thread.forum.id for thread in ForumThread.objects.all()]
        forums = forums.exclude(id__in=forums_with_threads)
        parent_choices= BLANK_CHOICE_DASH + [(forum.id, smart_truncate(str(forum), 32)) for forum in forums]
...
class ThreadForm(dynamicforms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        forums = Forum.objects.filter(container=self.container)
        forum_choices= [(forum.id, smart_truncate(str(forum), 32)) for forum in forums]
