...
def get_forum_params(
    ...
    **kwargs
    ...
    if container.max_level == 0:
        ...
        if nof_forums > 1:
            raise RuntimeError, "Database integrity corrupted. For container '%s' with max_level=0, there must not be more than one forum. %d forums detected. Please check that!" % (
                str(container), nof_forums
            )
        elif nof_forums == 1:
            current_forum = forums[0]
        else:
            current_forum = Forum(
                container=container, title=force_unicode("Discussion Board")
            )
            current_forum.save()
...
def handle_request(
    ...
    **kwargs
    ...
    extra_context = get_forum_params(
        object_url_part, url_identifier, forum_slug, thread_slug, **kwargs
    )
...
class ForumOptionsFormHandler(FormHandler):
    ...
    def get_form_template(self, use_ajax):
        return get_template_name_list_for_object(
            "forum_options", self.container, "forum/forms", use_ajax
        )
...
class ForumFormPreviewHandler(FormPreviewHandler):
    ...
    def get_form_template(self, use_ajax):
        return get_template_name_list_for_object(
            "forum", self.container, "forum/forms", use_ajax
        )

    def get_confirm_delete_template(self, use_ajax):
        return get_template_name_list_for_object(
            "confirm_delete", self.container, "forum/forms", use_ajax
        )
    ...
    def check_warnings(self, request, action):
        ...
        if action == ID_ACTION_NEW:
            if self.current_forum and self.current_forum.has_threads():
                return _(
                    "WARNING! You want to create a new forum under the '%(current_forum)s' forum, which already have threads. All threads of '%(current_forum)s' will be moved to your new forum!"
                ) % {
                    'current_forum': unicode(self.current_forum)
                }
        return None
    ...
    def save_new(self, cleaned):
        ...
        if self.current_forum:
            for thread in self.current_forum.get_threads(
                exclude_is_sticky=False
            ):
                thread.forum = forum
                thread.save()
...
class ThreadFormPreviewHandler(FormPreviewHandler):
    ...
    def get_form_template(self, use_ajax):
        return get_template_name_list_for_object(
            "thread", self.container, "forum/forms", use_ajax
        )
...
class ReplyFormPreviewHandler(FormPreviewHandler):
    ...
    def get_form_template(self, use_ajax):
        return get_template_name_list_for_object(
            "reply", self.container, "forum/forms", use_ajax
        )
    ...
    def save_new(self, cleaned):
        ...
        reply = ForumReply(
            thread=thread, parent=parent, message=cleaned['message']
        )
        reply.save()
        return self.redirect()
