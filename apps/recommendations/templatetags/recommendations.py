...
class ObjectAdding2Recommendations(template.Node):
    ...
    def render(self, context):
        ...
        is_not_recommendation_for_user = not Recommendation.objects.filter(user=context['request'].user, content_type=ct, object_id=obj_to_add.id).count()
        ...
        output = loader.render_to_string(template_path or "generic/recommendation.html", c)
        c.pop()
        return output
