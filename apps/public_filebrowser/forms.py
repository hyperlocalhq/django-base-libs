...
class MakeDirForm(forms.Form):
    ...
    dir_name = forms.CharField(
        ...
        label=_(u'Name'),
        help_text=_(
            u'Only letters, numbers, underscores, spaces and hyphens are allowed.'
        ),
        required=True
    )
    ...
    def clean_dir_name(self):
        if self.cleaned_data['dir_name']:
            ...
            if os.path.isdir(
                os.path.join(
                    self.path, convert_filename(self.cleaned_data['dir_name'])
                )
            ):
...
class RenameForm(forms.Form):
    ...
    def clean_name(self):
        if self.cleaned_data['name']:
            ...
            if os.path.isdir(
                os.path.join(
                    self.path, convert_filename(self.cleaned_data['name'])
                )
            ):
                raise forms.ValidationError(_(u'The Folder already exists.'))
            elif os.path.isfile(
                os.path.join(
                    self.path,
                    convert_filename(self.cleaned_data['name']) +
                    self.file_extension
                )
            ):
