...
from jetson.apps.public_filebrowser.decorators import flash_login_required
from jetson.apps.public_filebrowser.functions import path_to_url, sort_by_attr, get_path, get_file, get_version_path, get_filterdate, get_settings_var, handle_file_upload, convert_filename
...
def get_user_breadcrumbs(user, path):
    """
    Get user's breadcrumbs.
    """
    accessible_directories = DirectoryAccess.objects.filter(
        group__user=user,
    ).distinct()
    if not accessible_directories:
        raise Http404, _("You have no access to the file system.")
    path = path[len(accessible_directories[0].accessible_root.path + "/"):]
...
def get_user_path(user, relative_directory=""):
    """
    Get path from FILEBROWSER_DIRECTORY to relative_directory through user's accessible_root
    """
    accessible_directories = DirectoryAccess.objects.filter(
        group__user=user,
    ).distinct()
    ...
    return get_path(
        accessible_directories[0].accessible_root.path + "/" +
        relative_directory
    )
...
def browse(request):
    ...
    path = get_user_path(request.user, query.get('dir', ''))
    user_path_prefix = get_user_path(request.user)
    directory = get_path('')
    ...
    if path is None:
        ...
        redirect_url = reverse("pfb_browse") + query_helper(query, "", "dir")
    ...
    for file in dir_list:
        ...
        if append:
            try:
                ...
                if fileobject.path.startswith(user_path_prefix):
                    fileobject.public_path_relative_directory = fileobject.path[
                        len(user_path_prefix):]
    ...
    if not request.GET.get(
        'ot'
    ) and DEFAULT_SORTING_ORDER == "desc" or request.GET.get('ot') == "desc":
        files.reverse()
    ...
    return render_to_response(
        'public_filebrowser/index.html', {
            ...
            'breadcrumbs': get_user_breadcrumbs(request.user, path),
            'breadcrumbs_title': ""
        },
...
def mkdir(request):
    ...
    from jetson.apps.public_filebrowser.forms import MakeDirForm
    ...
    if path is None:
        msg = _('The requested Folder does not exist.')
        messages.error(request, msg)
        return HttpResponseRedirect(reverse("pfb_browse"))
    abs_path = os.path.join(MEDIA_ROOT, DIRECTORY, path)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # remove pagination
                redirect_url = reverse("pfb_browse") + query_helper(
    ...
    return render_to_response(
        'public_filebrowser/makedir.html', {
            'form': form,
            'query': query,
...
def upload(request):
    ...
    return render_to_response(
        'public_filebrowser/upload.html', {
            'query': query,
...
def _check_file(request):
    ...
    folder = get_user_path(request.user, folder)
...
def _upload_file(request):
    ...
    if request.method == 'POST':
        ...
        if request.FILES:
            ...
            if os.path.isfile(
                os.path.join(MEDIA_ROOT, DIRECTORY, folder, filedata.name)
            ):
            ...
            filebrowser_post_upload.send(
                ...
                file=FileObject(os.path.join(DIRECTORY, folder, filedata.name))
            )
...
def delete(request):
    ...
    dir_name = query.get(
        'dir', ''
    )  # TODO: find out what exactly dir_name has to be
    filename = get_file(path or "", query.get('filename', ''))
    ...
    if request.GET:
        if request.GET.get('filetype') != "Folder":
            ...
            try:
                # PRE DELETE SIGNAL
                filebrowser_pre_delete.send(
                    sender=request, path=path, filename=filename
                )
    ...
    return render_to_response(
        'public_filebrowser/index.html', {
            ...
            'query': query,
            'settings_var': get_settings_var(),
            'breadcrumbs': get_user_breadcrumbs(request.user, dir_name),
            'breadcrumbs_title': ""
        },
        context_instance=Context(request)
    )
...
def rename(request):
    ...
    from jetson.apps.public_filebrowser.forms import RenameForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_relative_server_path = os.path.join(
                DIRECTORY, path, new_filename
            )
            try:
                # PRE RENAME SIGNAL
                ...
                # RENAME ORIGINAL
                os.rename(
                    os.path.join(MEDIA_ROOT, relative_server_path),
                    os.path.join(MEDIA_ROOT, new_relative_server_path)
                )
                # POST RENAME SIGNAL
                ...
                messages.success(request, msg)
                redirect_url = reverse("pfb_browse"
        ...
        form = RenameForm(
            abs_path,
            file_extension,
            initial={'name': os.path.splitext(filename)[0].lower()}
        )

    return render_to_response(
        'public_filebrowser/rename.html', {
            'form': form,
            'query': query,
...
def versions(request):
    ...
    return render_to_response(
        'public_filebrowser/versions.html', {
...
def download(request):
    """
    Download a file. Requires a user to be logged in.
    """
    query = request.GET
    path = get_user_path(request.user, query.get('dir', ''))
    filename = get_file(path or "", query.get('filename', ''))
    if path is None or filename is None:
        ...
        return HttpResponseRedirect(reverse("pfb_browse"))
    abs_path = os.path.join(MEDIA_ROOT, DIRECTORY, path)
    f = file(os.path.join(abs_path, filename), "rb")
...
download = login_required(download)
