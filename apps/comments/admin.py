...
class CommentOptions(ObjectRelationMixinAdminOptions()):
    ...
    fieldsets = (
        ...
        ('Author', {
            'fields': ('user', 'name', 'email', 'url_link')
        }),
        ('Content', {
            ...
            'Ratings', {
                'fields':
                    (
                        ...
                        'rating6', 'rating7', 'rating8', 'valid_rating'
                    )
            }
        ),
        (
            'Meta', {
                'fields': ('is_public', 'is_removed', 'is_spam', 'ip_address')
            }
        ),
    )
    list_display = (
        'name', 'submit_date', 'content_type', 'get_content_object_display'
    )
