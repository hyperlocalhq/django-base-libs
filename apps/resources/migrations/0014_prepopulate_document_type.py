...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for term in orm['structure.term'].objects.filter(
            ...
            ).exclude(sysname="document"):
            dt = orm.DocumentType()
            dt.id = term.id
            dt.title = term.title
            dt.title_en = title(term.slug) # WTF?
            dt.title_de = term.title_de
            dt.slug = term.slug
            dt.sort_order = term.sort_order
            if term.parent != parent:
                dt.parent_id = term.parent_id
            dt.path = get_path(dt)
            dt.path_search = _get_path_search(dt)
            dt.save()    
    ...
    models = {
        ...
        'resources.documenttype': {
            'Meta': {'ordering': "['path', 'sort_order']", 'object_name': 'DocumentType'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'parent': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'child_set'", 'null': 'True', 'to': "orm['resources.DocumentType']"}),
            'path': ('django.db.models.fields.CharField', [], {'max_length': '8192', 'null': 'True'}),
            'path_search': ('django.db.models.fields.TextField', [], {'blank': 'True'}),
            'slug': ('django.db.models.fields.SlugField', [], {'unique': 'True', 'max_length': '255', 'db_index': 'True'}),
            'sort_order': ('django.db.models.fields.IntegerField', [], {'blank': 'True'}),
