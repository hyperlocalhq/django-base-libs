...
class InternalMessageOptions(ExtendedModelAdmin):
    ...
    save_on_top = True
    fieldsets = (
        (_("Sender"), {'classes': ('collapse open',), 'fields': ('sender', )}),
        (_("Recipient"), {'classes': ('collapse open',), 'fields': ('recipient',)}),
        (_("Message"), {'classes': ('collapse open',), 'fields': ('subject', 'body',)}),
        (_("Flags"), {'classes': ('collapse closed','float-checkbox'), 'fields': ('is_read', 'is_deleted', 'is_spam', 'is_draft', 'is_replied',)}),
        )
