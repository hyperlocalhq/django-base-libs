    ...
    c = RequestContext(
        request, {
            ...
            'message':
                message,
            'last_url':
                request.httpstate.get("last_url", reverse("messages_list")),
        }
    )
    # mark mesage as read
...
def forward_message(request, pk, **kwargs):
    ...
    if not (
        message.recipient == request.user or message.sender == request.user
    ):
        raise Http404
...
def contact(request, **kwargs):
    ...
    obj = get_object_or_404(
        kwargs['queryset'], **{kwargs['slug_field']: kwargs['slug']}
    )
    if hasattr(
        obj, "content_object"
    ):  # if the obj is ContextItem or alike, then use its related object
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(
                '%smessage/alldone/' % obj.get_url_path()
            )
    else:
