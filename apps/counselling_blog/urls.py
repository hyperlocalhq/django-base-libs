...
from kb.apps.blog.forms import BlogPostForm
from kb.apps.blog.views import BlogPostFormPreviewHandler
from kb.apps.blog.feeds import RssFeed, AtomFeed
...
urlpatterns = patterns(
    ...
    # post detail
    (r'^(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/(?P<post_slug>[0-9A-Za-z-_]+)/$', 'handle_request',
     all_dict),
    ...
    (r'^helper/comment/(?P<comment_id>\d+)/(?P<action>refuse|accept|mark_as_spam)/$', 'blog_modify_comment',
     {'use_popup': True}),
    (
