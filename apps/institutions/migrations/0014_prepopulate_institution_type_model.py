...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for term in orm['structure.term'].objects.filter(
            ...
            ).exclude(sysname="institution"):
            it = orm.InstitutionType()
            it.id = term.id
            it.title = term.title
            it.title_en = title(term.slug) # WTF?
            it.title_de = term.title_de
            it.slug = term.slug
            it.sort_order = term.sort_order
            if term.parent != parent:
                it.parent_id = term.parent_id
            it.path = get_path(it)
            it.path_search = _get_path_search(it)
            it.save()
