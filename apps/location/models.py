...
class CountryChoices(object):
    def __iter__(self):
        ...
        choices = [
            (item.iso2_code, item.name)
            for item in Country.objects.filter(display=True)
        ]
        return iter(choices)
...
class AddressManager(models.Manager):
    def cleanup(self):
        ...
        for el in self.get_queryset():
            if el.address_events.count() + el.individual_address.count(
            ) + el.institutional_address.count() == 0:
                el.delete()
        for el in InstitutionalContact.objects.all():
            if el.postal_address:
                self.set_for(
                    el, "postal_address", **el.postal_address.get_dict()
                )
    ...
    def set_for(
        ...
        # else create one
            ...
            new_address.geoposition_set.create(
                latitude=(
                    not _is_numeric(latitude) and [None] or [float(latitude)]
                )[0],
                longitude=(
                    not _is_numeric(longitude) and [None] or [float(longitude)]
                )[0],
                altitude=(
                    not _is_numeric(altitude) and [None] or [int(altitude)]
                )[0],
            )
...
class Address(models.Model):
    ...
    street_address2 = models.CharField(
        _("Additional Address"), max_length=255, blank=True
    )
    street_address3 = models.CharField(
        _("Additional Address"), max_length=255, blank=True
    )
    postal_code = models.CharField(
        _("Postal/ZIP Code"), max_length=10, blank=True
    )
    ...
    def __unicode__(self):
        return u", ".join(
            [
                force_unicode(item) for item in (
                    self.street_address, self.street_address2, self.city,
                    self.country
                ) if item
            ]
        ).strip()
...
class Locality(models.Model):
    ...
    neighborhood = models.CharField(
        _("Neighborhood"), max_length=255, blank=True
    )
...
class Geoposition(models.Model):
    ...
    altitude = models.IntegerField(
        _("Altitude"),
        help_text=_("The elevation above the sea level measured in meters"),
        blank=True,
        null=True
    )
