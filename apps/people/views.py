    ...
    if len(person_filters) == 0 and criterion:
        ...
        for k, v in person_filters.items():
            ...
            return HttpResponseRedirect(
                '/%s/by-%s/%s/%s' % (URL_ID_PEOPLE, k, v, page)
            )
    elif not len(request.GET) and len(person_filters) > 1:
        query_vars = "?" + "&".join(
            ["%s=%s" % (k, v) for k, v in person_filters.items()]
        )
        page = 'page' in request.GET and "?page=%s" % request.GET.get(
            "page", ""
        ) or ""
        return HttpResponseRedirect(
            '/%s/%s%s' % (URL_ID_PEOPLE, page, query_vars)
        )
    else:
            ...
            if k == "type":
                ...
                if v == "activity":
                    ...
                    queryset = queryset.filter(
                        models.Q(object_id__in=person_ids
                                ) & models.Q(content_type=person_ctype) |
        ...
        if view_type == "map":
            queryset = queryset.filter(
                individualcontact__postal_address__geoposition__latitude__gte=
                -90,
            ).distinct()
