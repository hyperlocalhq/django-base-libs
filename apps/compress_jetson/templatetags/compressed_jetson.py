...
from jetson.apps.compress_jetson.conf import settings
from jetson.apps.compress_jetson.utils import media_root, media_url, needs_update, filter_css, filter_js, get_output_filename, get_version, get_version_from_file
...
def render_css(css, filename, version=None):
    return render_common(css.get('template_name', 'pipeline/css.html'), css, filename, version)

def render_js(js, filename, version=None):
    return render_common(js.get('template_name', 'pipeline/js.html'), js, filename, version)
...
class CompressedCSSNode(template.Node):
    ...
    def render(self, context):
        ...
        try:
            css = settings.COMPRESS_JETSON_CSS[css_name]
        except KeyError:
...
class CompressedJSNode(template.Node):
    ...
    def render(self, context):
        ...
        try:
            js = settings.COMPRESS_JETSON_JS[js_name]
        except KeyError:
            return '' # fail silently, do not return anything if an invalid group is specified
...
#@register.tag
def compressed_jetson_css(parser, token):
    try:
        tag_name, name = token.split_contents()
    except ValueError:
        raise template.TemplateSyntaxError, '%r requires exactly one argument: the name of a group in the COMPRESS_JETSON_CSS setting' % token.split_contents()[0]

    return CompressedCSSNode(name)
compressed_jetson_css = register.tag(compressed_jetson_css)

#@register.tag
def compressed_jetson_js(parser, token):
    try:
        tag_name, name = token.split_contents()
    except ValueError:
        raise template.TemplateSyntaxError, '%r requires exactly one argument: the name of a group in the COMPRESS_JETSON_JS setting' % token.split_contents()[0]

    return CompressedJSNode(name)
compressed_jetson_js = register.tag(compressed_jetson_js)
