...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        parent = orm['structure.term'].objects.get(
            ...
            sysname="person_group",
            )
        parent_path = "/%d/" % parent.id
        for term in orm['structure.term'].objects.filter(
            vocabulary__sysname='basics_object_types',
            path_search__contains=parent_path,
            ).exclude(sysname="person_group"):
            gt = orm.GroupType()
            gt.id = term.id
            gt.title = term.title
            gt.title_en = title(term.slug) # WTF?
            gt.title_de = term.title_de
            gt.slug = term.slug
            gt.sort_order = term.sort_order
            if term.parent != parent:
                gt.parent_id = term.parent_id
            gt.path = get_path(gt)
            gt.path_search = _get_path_search(gt)
            gt.save()    
    ...
    models = {
        ...
        'groups_networks.grouptype': {
            'Meta': {'ordering': "['path', 'sort_order']", 'object_name': 'GroupType'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'parent': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'child_set'", 'null': 'True', 'to': "orm['groups_networks.GroupType']"}),
