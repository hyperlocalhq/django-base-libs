...
class GroupMembership_Inline(ExtendedStackedInline):
    ...
    fieldsets = [
        (_("Main"), {'fields': ("user", "role"), 'classes': ["grp-collapse grp-open"]}),
        ]
    fieldsets += get_admin_lang_section(None, ['title'])
    fieldsets += [
        (_("Details"), {'fields': ("inviter", "is_accepted", "is_blocked", "is_contact_person", "confirmer"), 'classes': ["grp-collapse grp-open"]}),
        ]
...
class PersonGroupOptions(ExtendedModelAdmin):
    ...
    save_on_top = True
    fieldsets = [
        (_("Main"), {'fields': ("content_type", "object_id", "title", "title2", "slug"), 'classes': ["grp-collapse grp-open"]}),
        (_("Details"), {'fields': ("organizing_institution", "preferred_language", "image"), 'classes': ["grp-collapse grp-closed"]}),
        (_("Configuration"), {'fields': ("group_type", "access_type", "is_by_invitation", "is_by_confirmation",), 'classes': ["grp-collapse grp-closed"]}),
        ]
    fieldsets += get_admin_lang_section(_("Description"), ['description',], False)
    fieldsets += [
        (_("Categories"), {'fields': ("context_categories",), 'classes': ["grp-collapse grp-closed"]}),
        (_("Status"), {'fields': ("status",), 'classes': ["grp-collapse grp-closed"]}),
        ]
