...
def do_bookmarks(parser, token):
    ...
    try:
        ...
        raise template.TemplateSyntaxError, "%r tag requires a following syntax: {%% %r [using <template_path>] %%}" % token.contents[
            0]
...
class Bookmarks(template.Node):
    ...
    def render(self, context):
        ...
        output = loader.render_to_string(
            template_path or "generic/bookmarks.html", context_vars
        )
        context_vars.pop()
        return output
