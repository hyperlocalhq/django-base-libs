...
class HttpStateStore(HttpStateBase):
    ...
    def load(self):
        httpstate_data = self._cache.get(self.httpstate_key)
    ...
    def save(self, must_create=False):
        ...
        result = func(self.httpstate_key, self._get_httpstate(no_load=must_create),
                self.get_expiry_age())
        if must_create and not result:
            raise CreateError

    def exists(self, httpstate_key):
        if self._cache.get(httpstate_key):
            return True
        return False
    ...
    def delete(self, httpstate_key=None):
        ...
        self._cache.delete(httpstate_key)
