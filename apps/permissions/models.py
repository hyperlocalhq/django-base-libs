...
class RowLevelPermissionManager(models.Manager):
    def create_row_level_permission(
        self, model_instance, owner, permission, negative=False
    ):
        ...
        if content_type != permission.content_type:
            raise TypeError, "Permission content type (%s) and object content type (%s) do not match" % (
    ...
    def create_default_row_permissions(
        self,
        model_instance,
        owner,
        change=True,
    ...
    def get_model_list(self, user, model, perm):
        ...
        user_group_list = [
            g['id'] for g in user.groups.select_related().values('id')
        ]
        if user_group_list:
...
class RowLevelPermission(
    ObjectRelationMixin(is_required=True), RowLevelPermissionOwner
):
    """
...
class PerObjectGroup(
    ObjectRelationMixin(is_required=True), SysnameMixin(max_length=80)
):
    """
    ...
    users = models.ManyToManyField(
        ...
        blank=True,
        help_text=_(
            "In addition to the permissions manually assigned, these users will also get all row level permissions granted to this group."
        )
    )
    ...
    def __unicode__(self):
        try:
            return u"%s | %s \"%s\"" % (
                force_unicode(self.get_title()),
                force_unicode(type(self.content_object)._meta.verbose_name),
                self.content_object
            )
        except:
            return self.title
