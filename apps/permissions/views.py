...
def manage_row_level_permissions(request, app_label, model_name, object_id):
    ...
    can_change = u.has_perm(
        "%s.%s" % (
            app_label,
            get_permission_codename("change", RowLevelPermission._meta)
        )
    )
    can_add = u.has_perm(
        "%s.%s" %
        (app_label, get_permission_codename("add", RowLevelPermission._meta))
    )
    if not (can_change or can_add):
        return access_denied(request)

    RLPFormSet = formset_factory(
        form=RLPForm, formset=BaseRLPFormSet, can_delete=can_change, extra=0
    )
    ...
    return render_to_response(
        [
            ...
            "admin/row_level_permissions.html"
        ],
        context_instance=c
    )
