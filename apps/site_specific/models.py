...
class ContextItemManager(models.Manager):
    ...
    def update_for(self, obj):
        ...
        item.context_categories.add(*list(obj.get_context_categories()))
        ...
        if obj.is_public and obj.postal_address:
            ...
            if address_dict['latitude'] and address_dict['longitude']:
                ...
                for lang_code, lang_name in settings.LANGUAGES:
                    ...
                    template_name = 'search/indexes/%s/%s_%s.txt' % (obj._meta.app_label, obj._meta.module_name, field_name)
...
class MappedItem(ObjectRelationMixin(is_required=True)):
    ...
    lat = models.FloatField(_("Latitude"), help_text=_("Latitude (Lat.) is the angle between any point and the equator (north pole is at 90; south pole is at -90)."))
    lng = models.FloatField(_("Longitude"), help_text=_("Longitude (Long.) is the angle east or west of an arbitrary point on Earth from Greenwich (UK), which is the international zero-longitude point (longitude=0 degrees). The anti-meridian of Greenwich is both 180 (direction to east) and -180 (direction to west)."))
...
class ClaimRequest(ObjectRelationForClaimRequest):
    ...
    best_time_to_call = models.CharField(_("Best Time to Call"), max_length=25, choices=TIME_OF_DAY_CHOICES, blank=True, null=True)
