...
from ccb.apps.marketplace.models import JobOffer, URL_ID_JOB_OFFER, URL_ID_JOB_OFFERS, SECURITY_SUMMAND as MARKETPLACE_SECURITY_SUMMAND
...
def object_list_for_map(request):
    ...
    object_list = MappedItem.objects.raw(
        'SELECT *, ( 6371 * acos( cos( radians(' + str(center_lat) + ') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('+str(center_lng)+') ) + sin( radians('+ str(center_lat) +') ) * sin( radians( lat ) ) ) ) AS distance FROM site_specific_mappeditem HAVING distance < ' + str(distance) + ' ORDER BY distance;'
        )
