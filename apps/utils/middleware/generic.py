...
class RequireLoginMiddleware(object):
    ...
    def process_request(self, request):
        path = request.path
        if path != self.require_login_path and not request.user.is_authenticated() and not path.startswith("/admin/") and not path.startswith("/jssettings/") and not path.startswith("/jsi18n/") and not (path.startswith("/project/") and path.endswith("/confirm/")) and not settings.DEBUG:
            if request.POST:
                return login(request)
            else:
                return HttpResponseRedirect('%s?goto_next=%s' % (self.require_login_path, request.path))
...
class AdminScriptUpdateMiddleware(object):
	...
	return response # temporary problem solving
        if response.content.startswith("<script"):
            ...
            <script type="text/javascript">
                ...
                //"{{ %s }}";
                //]]>
            </script>%s""" % (site.domain, response.content)
        return response
