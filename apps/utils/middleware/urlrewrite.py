...
class URLRewriteMiddleware(object):
    ...
    def process_response(self, request, response):
        ...
        if SUBDOMAINS_SUPPORTED:
            site = Site.objects.get_current()
            content = re.sub(
                r'"(https?://)(%s)(/)(%s)([^/]+)/([^"]*)"' % (re.escape(site.domain), re.escape(SUBDOMAIN_MNG_DIR)),
                r'"\1\5.\2\3\6"',
                response.content
            )
            current_protocol = int(request.META['SERVER_PORT'])==443 and "https" or "http"
            response.content = re.sub(
                r'"(/)(%s)([^/]+)/([^"]*)"' % re.escape(SUBDOMAIN_MNG_DIR),
                r'"%s://\3.%s\1\4"' % (current_protocol, site.domain),
                content
            )
        return response
