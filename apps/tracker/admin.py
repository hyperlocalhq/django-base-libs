...
class TicketOptions(ObjectRelationMixinAdminOptions()):
    ...
    list_display = ('concern', 'submitted_date', 'priority', 'get_submitter', 'get_content_object_display', 'status', 'modifier')
    list_filter = ('status', 'concern', 'submitted_date')
    search_fields = ('description', 'concern__title', 'status',)
    fieldsets = (
        (None, 
        {'fields': ('concern', 'description', 'client_info', 'status', 'priority', 'url', 'submitter_name', 'submitter_email', 'modifier')}
        ),
    )
