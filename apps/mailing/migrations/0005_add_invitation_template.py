...
class Migration:
    ...
    def forwards(self, orm):
        t1 = (list(orm.EmailTemplate.objects.filter(
            slug="message_received",
            )) + [None])[0] # get email template or None
        
        t2 = (list(orm.EmailTemplate.objects.filter(
            slug="invitation",
            )) + [None])[0] # get email template or None
        ...
        if t1 and not t2:
            ...
            t2.slug = "invitation"
            t2.save()
            for etp in orm.EmailTemplatePlaceholder.objects.filter(sysname__in=(
                ...
                t2.allowed_placeholders.add(etp)
    ...
    models = {
        ...
        'mailing.emailtemplate': {
            'Meta': {'ordering': "XFieldList(['-timestamp','name'])"},
        ...
        'mailing.emailtemplateplaceholder': {
            'Meta': {'ordering': "XFieldList(['relates_to','name_'])"},
