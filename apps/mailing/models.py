...
class EmailMessage(CreationModificationMixin):
    sender = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="sent_by_message_set"
    )
    recipient = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="received_by_message_set"
    )
    sender_name = models.CharField(
        _("Sender name"), null=True, blank=True, max_length=255
    )
    ...
    delete_after_sending = models.BooleanField(
        _("Delete after sending"), default=False
    )
...
class EmailTemplatePlaceholder(
    SysnameMixin(
        verbose_name=_("Placeholder Internal Name"),
        prepopulate_from=("name", )
    )
):

    name = MultilingualCharField(
        _('Placeholder Name'), max_length=64, unique=True
    )
    relates_to = models.IntegerField(
        _("relates to"), default=1, choices=PLACEHOLDER_RELATES_TO
    )
    help_text = models.CharField(
        _("Placeholder Help text"), max_length=255, null=True, blank=True
    )
    ...
    def __unicode__(self):
        return " | ".join(
            [
                getattr(self, "name_%s" % lang_code)
...
class EmailTemplate(SingleSiteMixin, SlugMixin(prepopulate_from=("name", ))):
    ...
    body_html = models.TextField(
        _("Template HTML (English)"), null=True, blank=True
    )
    body_html_de = models.TextField(
        _("Template HTML (German)"), null=True, blank=True
    )
    timestamp = models.DateTimeField(_("Written"), auto_now_add=True)
    allowed_placeholders = models.ManyToManyField(
        EmailTemplatePlaceholder,
        verbose_name=_("Allowed Placeholders"),
        blank=True
    )
    ...
    def get_allowed_placeholders(self):
        return EmailTemplatePlaceholder.objects.distinct().extra(
            ...
            where=[
                'mailing_emailtemplateplaceholder.id=mailing_emailtemplate_allowed_placeholders.emailtemplateplaceholder_id',
                'mailing_emailtemplate_allowed_placeholders.emailtemplate_id=%d'
                % (self.id or 0)
            ],
        )
