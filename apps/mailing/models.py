...
class EmailMessage(CreationModificationMixin):
    ...
    def send(self):
        try:
            send_mail(
                smart_str(self.subject),
                smart_str(self.body_html),
                "%s <%s>" % (smart_str(self.sender_name), smart_str(self.sender_email)),
                smart_str(self.recipient_emails).split(","),
                plain_message = smart_str(self.body),
                )
        except:
            return False
        else:
...
class EmailTemplatePlaceholder(SysnameMixin(verbose_name=_("Placeholder Internal Name"), prepopulate_from=("name",))):
    ...
    class Meta:
        ...
        ordering = XFieldList(['relates_to', 'name_'])
...
class EmailTemplate(SingleSiteMixin, SlugMixin(prepopulate_from=("name",))):
    ...
    class Meta:
        ...
        ordering = XFieldList(['-timestamp', 'name'])
