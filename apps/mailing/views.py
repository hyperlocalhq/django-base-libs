...
from jetson.apps.mailing.forms import GenericMailForm
...
def do_generic_mail(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if data.has_key("save_as_draft"):
                ...
                if len(recipients) > 0:
                    ...
                    user.message_set.create(message=_("Your message has been sent."))
        ...
        if email_template:
            ...
            if is_html:
                form.fields['body'].initial = BeautifulSoup(email_template.body_html).prettify()
                form.fields['body_de'].initial = BeautifulSoup(email_template.body_html_de).prettify()
            else:
        ...
        if reply_to:
            ...
            if is_html:
                form.fields['body'].initial = BeautifulSoup(reply_to.body_html).prettify()
                form.fields['body_de'].initial = BeautifulSoup(reply_to.body_html).prettify()
            else:
        ...
        if forward:
            ...
            if is_html:
                form.fields['body'].initial = BeautifulSoup(forward.body_html).prettify()
                form.fields['body_de'].initial = BeautifulSoup(forward.body_html).prettify()
            else:
        ...
        if draft:
            ...
            if is_html:
                form.fields['body'].initial = BeautifulSoup(draft.body_html).prettify()
                form.fields['body_de'].initial = BeautifulSoup(draft.body_html).prettify()
            else:
...
def send_email_using_template(
    ...
    if sender:
        sender.message_set.create(message=_("Your message has been sent."))
