...
RELATION_TYPE_CHOICES = XChoiceList(
    get_related_queryset(IndividualRelation,
                         'relation_types').filter(parent__isnull=True, ),
    ...
    def __init__(self, relation_action, user_1, user_2, **kwargs):
        ...
        try:
            ...
            self.fields['relation_types'].initial = [
                el.id for el in rel.relation_types.all()
            ]
        ...
        self.helper.layout = layout.Layout(
            ...
            bootstrap.FormActions(
                layout.Submit(
                    'submit',
                    _(self.relation_action.title()).upper()
                ),
    ...
    def save(self):
        ...
        if action == "edit":
            IndividualRelation.objects.edit(
                user_1=user_1, user_2=user_2, **cleaned
            )
        elif action == "invite":
...
class InvitationConfirmation(dynamicforms.Form):
    ...
    login_email_confirm = forms.EmailField(
        ...
        help_text=_(
            "IMPORTANT: The provided email address must be valid! You will receive your account data via this address."
        )
        ...
        if username != initial:
            ...
            if username.lower() != suggested.lower():
                raise forms.ValidationError(
                    _(
                        "This username is already used for another account. But you can use \"%s\" as your username."
                    ) % suggested
                )
        return username
    ...
    def clean_login_email(self):
        ...
        if login_email != initial:
            if User.objects.filter(email=login_email).count():
                raise forms.ValidationError(
                    _(
                        "This email address is already used for another account."
                    )
                )
        return login_email
    ...
    def clean_login_email_confirm(self):
        if not 'login_email' in self.cleaned_data:
            raise forms.ValidationError(
                _("Please, correct the error for the login email at first.")
            )
        login_email = self.cleaned_data['login_email']
        login_email_confirm = self.cleaned_data['login_email_confirm']
        if login_email != login_email_confirm:
            raise forms.ValidationError(
                _("The confirmed e-mail doesn't match the e-mail.")
            )
        return login_email
    ...
    def clean_password(self):
        ...
        if len(password) < 3:
            raise forms.ValidationError(
                _("Passwords must be at least 3 characters long")
            )
        return password
    ...
    def clean_password_confirm(self):
        if not 'password' in self.cleaned_data:
            raise forms.ValidationError(
                _("Please, correct the error for the password at first.")
            )
        password = self.cleaned_data['password']
        password_confirm = self.cleaned_data['password_confirm']
        if password != password_confirm:
            raise forms.ValidationError(
                _("The confirmed password doesn't match the password.")
            )
        return password
