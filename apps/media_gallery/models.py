...
class PortfolioSettings(ObjectRelationMixin(is_required=True, limit_content_type_choices_to={'model__in': ['person', 'institution', 'event']}), CreationModificationDateMixin):
    ...
    landing_page = models.CharField(_("Landing page"), choices=LANDING_PAGE_CHOICES, default="first_album", max_length=20)
    landing_page_image = FileBrowseField(_('Landing page image'), max_length=255, blank=True, help_text=_("A path to a custom landing page image."))
...
class Section(ObjectRelationMixin(is_required=True, limit_content_type_choices_to={'model__in': ['person', 'institution', 'event']}), CreationModificationDateMixin):
...
class MediaGallery(MediaGalleryBase, PublishingMixin):
    ...
    format = models.CharField(_("Presentation format"), choices=PRESENTATION_FORMAT_CHOICES, default="slideshow", max_length=20)
