...
VIDEO_SPLASH_URL = getattr(
    settings,
    "VIDEO_SPLASH_PATH",
    "%ssite/img/default/placeholder/video.png" % settings.STATIC_URL,
    )
VIDEO_SPLASH_TN_URL = getattr(
    settings,
    "VIDEO_SPLASH_TN_PATH",
    "%ssite/img/default/placeholder/video_tn.png" % settings.STATIC_URL,
    )
VIDEO_SPLASH_LIST_URL = getattr(
    settings,
    "VIDEO_SPLASH_LIST_PATH",
    "%ssite/img/default/placeholder/video_list.png" % settings.STATIC_URL,
    )
AUDIO_SPLASH_URL = getattr(
    settings,
    "AUDIO_SPLASH_PATH",
    "%ssite/img/default/placeholder/audio.png" % settings.STATIC_URL,
    )
AUDIO_SPLASH_TN_URL = getattr(
    settings,
    "AUDIO_SPLASH_TN_PATH",
    "%ssite/img/default/placeholder/audio_tn.png" % settings.STATIC_URL,
    )
AUDIO_SPLASH_LIST_URL = getattr(
    settings,
    "AUDIO_SPLASH_LIST_PATH",
    "%ssite/img/default/placeholder/audio_list.png" % settings.STATIC_URL,
    )
IMAGE_SPLASH_URL = getattr(
    settings,
    "IMAGE_SPLASH_PATH",
    "%ssite/img/default/placeholder/image.png" % settings.STATIC_URL,
    )
IMAGE_SPLASH_TN_URL = getattr(
    settings,
    "IMAGE_SPLASH_TN_PATH",
    "%ssite/img/default/placeholder/image_tn.png" % settings.STATIC_URL,
    )
IMAGE_SPLASH_LIST_URL = getattr(
    settings,
    "IMAGE_SPLASH_LIST_PATH",
    "%ssite/img/default/placeholder/image_list.png" % settings.STATIC_URL,
    )
...
MEDIA_FILE_TYPE_RECOGNITION = (
    (re.compile(r"^http://.*?youtube\.com/watch\?v=|http://youtu.be/"), "y"),
...
class MediaGalleryBase(ObjectRelationMixin(is_required=True), CreationModificationDateMixin, ViewsMixin, UrlMixin):
    ...
    cover_image = FileBrowseField(_('Cover image'), max_length=255, extensions=['.jpg', '.jpeg', '.gif','.png','.tif','.tiff'], blank=True, default="")
...
class MediaFileBase(CreationModificationMixin):
    ...
    splash_image_path = FileBrowseField(_('Splash-image path'), max_length=255, extensions=['.jpg', '.jpeg', '.gif','.png','.tif','.tiff'], blank=True, help_text=_("Used for still images in Flash players and for thumbnails."))
    ...
    def get_representation(self, suffix=""):
        ...
        if self.file_type == "i":
            if self.path:
                return render_to_string(
                    "media_gallery/includes/uploaded_image%s.html" % suffix,
                    context_dict,
                    )
            if self.external_url:
                return render_to_string(
                    "media_gallery/includes/external_image%s.html" % suffix,
                    context_dict,
                    )
        if self.file_type == "a":
            if self.path:
                return render_to_string(
                    "media_gallery/includes/uploaded_audio%s.html" % suffix,
                    context_dict,
                    )
            if self.external_url:
                return render_to_string(
                    "media_gallery/includes/external_audio%s.html" % suffix,
                    context_dict,
                    )
        if self.file_type == "v":
            if self.path:
                return render_to_string(
                    "media_gallery/includes/uploaded_video%s.html" % suffix,
                    context_dict,
                    )
            if self.external_url:
                return render_to_string(
                    "media_gallery/includes/external_video%s.html" % suffix,
                    context_dict,
                    )
        if self.file_type == "y":
            if self.external_url:
                match = re.compile(
                    r"^http://(.*?youtube\.com/watch\?v=|youtu\.be/)([^&]+)",
                    ).match(self.external_url)
                if match:
                    return render_to_string(
                        "media_gallery/includes/youtube_video%s.html" % suffix,
        ...
        if self.file_type == "m":
            if self.external_url:
                ...
                if match:
                    return render_to_string(
                        "media_gallery/includes/vimeo_video%s.html" % suffix,
