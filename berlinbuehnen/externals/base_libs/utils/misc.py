...
def get_unique_value(
        model, proposal, field_name="slug", instance_pk=None, separator="-", number_first=False,
    ...
    If an instance_pk is passed to the function and proposal matches the value of existing instance,
    ...
    * ignore_case defines if "Test" should be treated the same as "test"
    ...
    if instance_pk:
        try:
            existing_name = qs.filter(pk=instance_pk).values_list(field_name, flat=True)[0]
        except IndexError:
            pass
        else:
            match = re.match(r'^%s%s(\d+)%s$' % (proposal, separator_regex, postfix_regex), existing_name)
            if match:
...
def strip_html(text):
    ...
    # one linebreak after <br />
    text = re.sub(r'<br ?/?>', '<br />\n', text, flags=re.I)

    # one linebreak after </p>
    text = re.sub(r'</p>', '</p>\n', text, flags=re.I)
