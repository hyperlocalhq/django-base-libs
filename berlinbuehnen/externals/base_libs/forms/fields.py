...
class ObjectChoiceField(forms.Field):
    ...
    def __init__(self, choices=(), obj_list=(), default_text="", required=True, widget=None, label=None, initial=None, help_text=None, *args, **kwargs):
        super(ObjectChoiceField, self).__init__(required, widget, label, initial, help_text, *args, **kwargs)
    ...
    def _set_obj_list(self, value):
        ...
        for q in self._obj_list:
            object_choice = [
                (ObjectChoiceField.returnKey(o), force_unicode(o))
                for o in q
                ]
            choice_list.append(
    ...
    def clean(self, value):
        ...
        if value == u'':
            return value
        # flatten key choices
        valid_values = set(reduce(
            lambda a,b: a+b, [
                (dict(group).keys())
                for group in dict(self.widget.choices).values()
                ]
            ))
    ...
    def __init__(self,
        ...
        *args, **kwargs):
        self.default_error_messages.update({
            'too_small_dimensions': _("The media file is too small. The minimal dimensions are %(width)dx%(height)d.") % {
                'width': min_dimensions[0],
                'height': min_dimensions[1],
                },
            'too_large_ratio': _("The ratio of the dimensions is too large. The maximal allowed ratio is %d:1.") % max_ratio,
            'too_large_file': _("The media file is too large. The maximal allowed file size is %s.") % filesizeformat(max_file_size),
            })
...
class AudioField(forms.FileField):
    ...
    """
    default_error_messages = {
        'invalid_audio': _(u"Upload a valid audio file. The file you uploaded was either not an audio or a corrupted audio."),
        'invalid_extension': _("File type of the uploaded file is invalid."),
        }

    def __init__(self, 
        valid_file_extensions=("aac", "aif", "iff", "mid", "midi", "mp3", "mpa", "ra", "ram", "wav", "wma"),
        max_file_size=5*1024*1024, # 5 MB
        *args, **kwargs):
        self.default_error_messages.update({
            'too_large_file': _("The media file is too large. The maximal allowed file size is %s.") % filesizeformat(max_file_size),
                ...
                try:
                    for f in os.listdir(path):
                        ...
                        if os.path.isfile(full_file) and (self.match is None or self.match_re.search(f)):
                            choices.add((
                                full_file.replace(templates_root + "/", "", 1),
                                f,
                                ))
                except OSError:
                    pass
