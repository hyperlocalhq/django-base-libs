/**
 * Initiates all list filters on the page.
 *
...
(function() {
    ...
    Filter.prototype.onLinkClicked = function(event) {
        ...
        var onContentLoaded = function(response, status, xhr) {
            ...
            for (var i=0, length = $new_options.length; i<length; i++) {
                
                var $new_option = $($new_options.get(i));
                var $old_option = $(me.$options.get(i));
                
                $new_option.attr('class', $old_option.attr('class'));
            }
            
            for (var i=0, length = $new_suboptions.length; i<length; i++) {
                
                var $new_option = $($new_suboptions.get(i));
                var $old_option = $(me.$suboptions.get(i));
