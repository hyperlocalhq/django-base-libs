...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, *args, **options):
        ...
        for fav_index, favorite in enumerate(Favorite.objects.filter(content_type=ct), 1):
            if self.verbosity >= NORMAL:
                ...
                if prod:
                    ...
                    if event:
                        ...
                        favorite.save()
                        print "%d/%d %s changed" % (fav_index, fav_count, smart_str(prod.title_de))
                    else:
                        favorite.delete()
                        print "%d/%d %s deleted" % (fav_index, fav_count, smart_str(prod.title_de))
                else:
                    favorite.delete()
