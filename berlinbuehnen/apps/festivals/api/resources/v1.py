...
from berlinbuehnen.apps.festivals.models import SocialMediaChannel
from berlinbuehnen.apps.festivals.models import Image
from berlinbuehnen.apps.festivals.models import FestivalPDF

from berlinbuehnen.apps.productions.api.resources.v1 import ProductionResource
...
class FestivalImageResource(ModelResource):
    ...
    def get_object_list(self, request):
        object_list = super(FestivalImageResource, self).get_object_list(request)
...
class FestivalResource(ModelResource):
    ...
    social_media = fields.ToManyField(FestivalSocialMediaChannelResource, "socialmediachannel_set", full=True)
    ...
    class Meta(BaseMetaForModelResource):
        ...
        fields = [
            ...
            'email',
            'website',
            'tickets_street_address', 'tickets_street_address2', 'tickets_postal_code', 'tickets_city', 'tickets_country',
            'press_contact_name',
            'press_street_address', 'press_street_address2', 'press_postal_code', 'press_city', 'press_country',
            'press_email',
            'press_website',
            'start', 'end',
            'status',
    ...
    def dehydrate(self, bundle):
        ...
        for lang_code, lang_name in settings.FRONTEND_LANGUAGES:
            try:
                activate(lang_code)
                path = reverse('festival_detail', kwargs={'slug': bundle.obj.slug})
        ...
        if bundle.obj.tickets_phone_number:
            bundle.data['tickets_phone'] = [bundle.obj.tickets_phone_country, bundle.obj.tickets_phone_area, bundle.obj.tickets_phone_number]
        if bundle.obj.tickets_fax_number:
            bundle.data['tickets_fax'] = [bundle.obj.tickets_fax_country, bundle.obj.tickets_fax_area, bundle.obj.tickets_fax_number]

        if bundle.obj.press_phone_number:
            bundle.data['press_phone'] = [bundle.obj.press_phone_country, bundle.obj.press_phone_area, bundle.obj.press_phone_number]
        if bundle.obj.press_fax_number:
            bundle.data['press_fax'] = [bundle.obj.press_fax_country, bundle.obj.press_fax_area, bundle.obj.press_fax_number]

        bundle.data['tickets_calling_prices_de'] = strip_invalid_chars(strip_html(bundle.obj.get_rendered_tickets_calling_prices_de()))
        bundle.data['tickets_calling_prices_en'] = strip_invalid_chars(strip_html(bundle.obj.get_rendered_tickets_calling_prices_en()))

        return bundle

    def apply_filters(self, request, applicable_filters):
        from dateutil.parser import parse
        base_object_list = super(FestivalResource, self).apply_filters(request, applicable_filters)
