...
from berlinbuehnen.apps.education.models import ProjectImage
...
class BasicInfoForm(autocomplete_light.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            'age_from', 'age_till', 'needs_teachers', 'free_entrance', 'tickets_website',
    ...
    def __init__(self, *args, **kwargs):
        ...
        layout_blocks.append(layout.Fieldset(
            ...
            layout.HTML("""{% load i18n %}<div class="subtitle">{% trans "leave blank if you want to use the data from the department" %}</div>"""),
            layout.Row(
                layout.Div(
                    "location_title",
                    "street_address",
        ...
        layout_blocks.append(layout.Fieldset(
            ...
            layout.Row(
                layout.Div(
                    'contact_department', css_class="col-xs-6 col-sm-6 col-md-6 col-lg-6"
                ),
                layout.Div(
                    'contact_name', css_class="col-xs-6 col-sm-6 col-md-6 col-lg-6"
                ),
                css_class="row-md"
            ),
            layout.Row(
...
class ProjectTimeForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        layout_blocks.append(
            layout.Row(
                layout.Div(
                    "start_date", css_class="col-xs-6 col-sm-6 col-md-6 col-lg-6"
                ),
                layout.Div(
                    "start_time",
                    "id",
...
ProjectTimeFormset = inlineformset_factory(Project, ProjectTime, form=ProjectTimeForm, formset=InlineFormSet, extra=0)
...
ProjectMemberFormset = inlineformset_factory(Project, ProjectMember, form=ProjectMemberForm, formset=InlineFormSet, extra=0)
...
def submit_step(current_step, form_steps, form_step_data, instance=None):
    if current_step == "basic":
        ...
        if not instance.slug:
            instance.slug = get_unique_value(Project, better_slugify(instance.title_de), instance_pk=instance.pk)
    ...
    if current_step == "description":
        ...
        for lang_code, lang_name in FRONTEND_LANGUAGES:
            ...
            setattr(instance, 'special_conditions_%s_markup_type' % lang_code, 'pt')
