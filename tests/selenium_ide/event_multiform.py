...
class EventMultiform(unittest.TestCase):
    ...
    def test_event_multiform(self):
        ...
        driver.find_element_by_css_selector("input.primaryAction").click()
        self.assertRegexpMatches(driver.title, r"^exact:Creative City Berlin - Wie erreiche ich mein Publikum[\s\S] - Dr\. Helen Adkins$")
        self.assertRegexpMatches(driver.title, r"^exact:Creative City Berlin - Wie erreiche ich mein Publikum[\s\S] - Dr\. Helen Adkins$")
        ...
        for i in range(60):
            try:
                if re.search(r"^exact:Creative City Berlin - Wie erreiche ich mein Publikum[\s\S] - Dr\. Helen Adkins$", driver.title): break
            except: pass
            time.sleep(1)
