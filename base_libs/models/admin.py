...
def get_admin_lang_section(heading, field_list, default_expanded=True):
    ...
    for lang_code, lang_verbose in settings.LANGUAGES:
        ...
        fieldset.append((section, {"fields": fields, "classes": classes,}),)
    return fieldset
...
def ObjectRelationMixinAdminOptions(
    prefix="", prefix_verbose="", admin_order_field="", extending=ExtendedModelAdmin
):
    """
    ...
    setattr(
        klass, "get_%scontent_object_display" % prefix, get_content_object_display,
    )
...
def ObjectRelationMixinAdminForm(prefix=None):
    ...
    class klass(forms.ModelForm):
        class Media:
            js = ("%sjs/admin/RelatedObjectSelect.js" % ADMIN_MEDIA_URL,)
...
class SingleSiteContainerMixinAdminForm(ObjectRelationMixinAdminForm()):
    def clean(self):
        ...
        if content_type_value and object_id_value:
            qs = qs.filter(
                content_type=content_type_value, object_id=int(object_id_value)
            )
        else:
        ...
        if qs.count() > 0:
            raise forms.ValidationError(
                _("There already exists a container with these properties.")
            )
...
class MultiSiteContainerMixinAdminForm(ObjectRelationMixinAdminForm()):
    def clean(self):
        ...
        if content_type_value and object_id_value:
            qs = qs.filter(content_type=content_type_value, object_id=object_id_value,)
        else:
            qs = qs.filter(content_type__isnull=True, object_id__isnull=True)
...
class HierarchyMixinAdminOptions(ExtendedModelAdmin):
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        """ applying custom widgets here! """
        field = super(HierarchyMixinAdminOptions, self).formfield_for_dbfield(
            db_field, **kwargs
        )
    ...
    def clean_parent(self):
        ...
        while pid:
            if pid == cid:
                raise forms.ValidationError(
                    _(
                        "This parent is not allowed, as it creates a cyclical reference. Please select a different parent."
                    )
                )
...
class ContentBaseMixinAdminOptions(PublishingMixinAdminOptions):
    ...
    fieldsets = get_admin_lang_section(
        _("Content"), ["title", "subtitle", "short_title", "content"],
    )
    fieldsets += [
        (_("Sites"), {"classes": ("collapse open",), "fields": ("sites",),}),
    ]
...
class MetaTagsMixinAdminOptions(ExtendedModelAdmin):
    fieldsets = (
        (
            ...
            {
                "classes": ("collapse open",),
                "fields": get_admin_lang_section(_("Keywords"), ["meta_keywords"])
                + get_admin_lang_section(_("Description"), ["meta_description",])
                + [
                    (_("Author"), {"fields": ["meta_author"]}),
                    (_("Copyright"), {"fields": ["meta_copyright"]}),
                ],
            },
        ),
    )
...
class SEOMixinAdminOptions(ExtendedModelAdmin):
    fieldsets = (
        (
            ...
            {
                "classes": ("collapse closed",),
                "fields": get_admin_lang_section(_("Page title"), ["page_title"])
                + get_admin_lang_section(_("Keywords"), ["meta_keywords"])
                + get_admin_lang_section(_("Description"), ["meta_description",])
                + [
                    (_("Author"), {"fields": ["meta_author"]}),
                    (_("Copyright"), {"fields": ["meta_copyright"]}),
