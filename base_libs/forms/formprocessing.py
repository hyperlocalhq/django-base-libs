...
class FormPreviewHandler(FormHandler):
    ...
    def post(self, request, action):
        ...
        if form.is_valid():
            if self.security_hash(request, form) != request.POST.get(
                self._check_name("hash")
            ):
    ...
    def failed_hash(self, request, action):
        ...
        self.extra_context["hash_failed_message"] = _(
            "Security check failed. Maybe your form has been tampered. Please try again."
        )
