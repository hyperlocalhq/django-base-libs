...
try:
    ...
    class HierarchyRelatedFilterSpec(RelatedFieldListFilter):
        def __init__(self, f, request, params, model, model_admin, field_path=None):
            super(HierarchyRelatedFilterSpec, self).__init__(
            ...
            if isinstance(f, models.ManyToManyField):
                ...
                restrict_to = model._default_manager.values_list(f.attname, flat=True,)
            ...
            if qs.count():
                ...
                qs = qs | f.rel.to._default_manager.filter(
                    pk__in=list(self.filtered_out)
                )
            ...
            for item in self.lookup_choices:
                ...
                yield {
                    ...
                    "display": {
                        ...
                        "val": (
                            "%s %s" % ("-" * item.get_level(), unicode(item))
                        ).strip(),
                    },
                }

    FieldListFilter.register(
        lambda f: bool(f.rel) and getattr(f.rel.to, "is_hierarchical", False),
        HierarchyRelatedFilterSpec,
    )
