...
def get_container(container_model, site, related_obj=None, sysname=None, create=True):
    ...
    if qs.count() == 0:
        obj = container_model(
            content_type=content_type, object_id=object_id, sysname=sysname,
        )
        if create:
            if site:
...
def json_objects_to_select(
    request, app_name, model_name, obj_pk, field_name, content_type_id
):
    """
    ...
    if request.user.has_perm("%s.change_%s" % (app_name, model_name), edited_obj,):
        ...
        if request.user.has_perm(
            ...
            if len(field_name_bits) == 1:
                ...
                limit_choices_to = (
                    getattr(model(), field_name_bits[0])
                    .model._meta.get_field(field_name_bits[2])
                    .limit_choices_to
                )
...
def ajax_autocomplete(
    request, app, qs_function, display_attr, add_display_attr=None, limit=1000
):
    """
    ...
    if "q" in request.GET:
        ...
        func = get_installed(
            "%(app)s.ajax.%(func)s" % {"app": app, "func": qs_function,}
        )
        ...
        for obj in queryset[:limit]:
            ...
            if add_display_attr:
                ...
                if callable(add_display) and not getattr(
                    add_display, "alters_data", False
                ):
