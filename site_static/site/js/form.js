...
$(document).ready(function() {
    ...
    SelectBox.prototype.fillDropdown = function() {
        ...
        $options.each(function(index) {
            ...
            var group = ($parent.is("optgroup")) ? $parent.attr("label") : "";

            var multiple = (me.multiple) ? ' multiple' : '';
            var first_none_group = '';

            if (group != old_group) {
                if (group != "") {
                    var $group = $('<div class="select-option-group'+multiple+'">'+group+'</div>');
                    me.$dropdown.append($group);
                } else {
                    first_none_group = ' first-none-group';
                }
                old_group = group;
            }
