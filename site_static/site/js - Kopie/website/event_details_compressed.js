...
(function($,undefined){var gDaysOfWeek=["mon","tue","wed","thu","fri","sat","sun"];
self.OpeningHoursManager={init:function(){var oSelf=self.OpeningHoursManager;
$("#id_show_breaks").change(function(){if($(this).attr("checked")){$(".field_info").addClass("breaks_shown")
}else{$(".field_info").removeClass("breaks_shown")
}for(i=0;
i<7;
i++){oSelf.toggleOpeningHours(gDaysOfWeek[i])
}});
for(i=0;
i<7;
i++){$("#id_"+gDaysOfWeek[i]+"_is_closed").change(function(){var sDay=$(this).attr("id").match(/id_(.+)_is_closed$/)[1];
if($(this).attr("checked")){$(this).parent().parent().addClass("is_closed")
}else{$(this).parent().parent().removeClass("is_closed")
}oSelf.toggleOpeningHours(sDay)
})
}$("#id_apply_all_days").click(function(){var sMon=gDaysOfWeek[0];
...
i++){var sDay=gDaysOfWeek[i];
oSelf.toggleOpeningHours(sDay);
if($("#id_"+sDay+"_is_closed").attr("checked")){$("#id_"+sDay+"_is_closed").parent().parent().addClass("is_closed")
}else{$("#id_"+sDay+"_is_closed").parent().parent().removeClass("is_closed")
}}if($("#id_show_breaks").attr("checked")){$(".field_info").addClass("breaks_shown")
}else{$(".field_info").removeClass("breaks_shown")
}},destruct:function(){self.OpeningHoursManager=null
},toggleOpeningHours:function(sDay){var $oOpen0=$("#id_"+sDay+"_open0");
var $oClose0=$("#id_"+sDay+"_close0");
var $oOpen1=$("#id_"+sDay+"_open1");
var $oClose1=$("#id_"+sDay+"_close1");
if($("#id_"+sDay+"_is_closed").attr("checked")){$oOpen0.val("").parent().addClass("invisible");
$oClose0.val("").parent().addClass("invisible");
$oOpen1.val("").parents(".ctrlHolder").hide();
$oClose1.val("");
$("#id_"+sDay+"_errors").hide();
$("#id_"+sDay+"_break_errors").hide()
}else{$oOpen0.parent().removeClass("invisible");
$oClose0.parent().removeClass("invisible");
$("#id_"+sDay+"_errors").show();
if($("#id_show_breaks").attr("checked")){$oOpen1.parents(".ctrlHolder").show();
$("#id_"+sDay+"_break_errors").show()
}else{$oOpen1.val("").parents(".ctrlHolder").hide();
$oClose1.val("");
$("#id_"+sDay+"_break_errors").hide()
}}}};
$(document).ready(function(){self.OpeningHoursManager.init()
});
$(window).unload(function(){self.OpeningHoursManager.destruct()
})
}(jQuery));
(function($,undefined){self.EventTimesManager={fillInContactData:true,init:function(){var oSelf=self.EventTimesManager;
$("input[id^=id_event_times][id$=is_all_day]").change(function(){oSelf.showHideAllday(this)
}).each(function(){oSelf.showHideAllday(this)
});
$(".event_end").each(function(i){var hasValue=false;
$("select",this).each(function(){if($(this).val()){hasValue=true
}});
if(hasValue){$(this).parent().find("input.toggle_event_end").attr("checked",true)
}else{$(this).hide()
}});
$(".toggle_event_end").change(function(){var $oEndDateField=$(this).parents(".ctrlHolder").next(".event_end");
if($(this).attr("checked")){$oEndDateField.slideDown();
$(this).parents(".ctrlHolder").find("input[id^=id_event_times][id$=is_all_day]").attr("checked",true).change()
}else{$oEndDateField.slideUp()
}})
},showHideAllday:function(checkbox){if($(checkbox).attr("checked")){$(checkbox).parents(".event_time").find("select[id$=start_hh], select[id$=start_ii], select[id$=end_hh], select[id$=end_ii]").val("").parents(".ctrlHolder").slideUp()
}else{$(checkbox).parents(".event_time").find("select[id$=start_hh], select[id$=start_ii], select[id$=end_hh], select[id$=end_ii]").parents(".ctrlHolder").slideDown()
}},destruct:function(){self.EventTimesManager=null
}};
$(document).ready(function(){self.EventTimesManager.init()
});
$(window).unload(function(){self.EventTimesManager.destruct()
})
}(jQuery));
