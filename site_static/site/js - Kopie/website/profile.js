    ...
    self.ProfileManager = {
        ...
        init: function() {
            ...
            $("div.dyn_section").each(function() {
                ...
                $(
                    '<a href="#edit_' + sSection + '" id="but_edit_' + sSection + '"><span><span>' + gettext("edit") + '</span></span></a>'
                ).addClass("but_edit").click(
                    new Function('return self.ProfileManager.edit_section("' + sSection + '")')
                ).insertAfter($("#dyn_" + sSection));
            });
            ...
            } else if (document.location.hash.indexOf("#view_")==0){
                sCurrentSection = document.location.hash.substr(6);
        ...
        clear_current_section: function() {
            var oSelf = self.ProfileManager;
            if (oSelf.sSelected) {
                $("#dyn_" + oSelf.sSelected).html(
                    oSelf.oCachedDefaults[oSelf.sSelected]
                );
        ...
        delete_contact: function(iIndex) {
            ...
            if (!oSelf.bInProgress) {
                ...
                open_popup(
                    ...
                    584, "auto",
                    "/helper/popup-window/delete-contact/?type=" + oSelf.sContextItemType + "&slug=" + oSelf.sSlug + "&index=" + sIndex,
                    true
                );
            }
            return false;
        ...
        section_loaded: function(sHtml, bUpdate) {
            ...
            if (bUpdate) {
                ...
                if ("categories" == sSection) {
                ...
                } else if ("avatar" == sSection) {
                    $(
                        '<input type="button" class="button_delete button_bad" />'
                    ).val(gettext("Delete").toUpperCase()).insertBefore(".buttons").click(
                        oSelf.delete_avatar
                    );
                } else if ("event_times" == sSection) {
                ...
                } else if (sSection.match(/^contact_/)) {
                    if (self.ContactManager) {
                        self.ContactManager.init();
                    }
                    if (self.ContactDetailsManager) {
                    ...
                    if (sSection != "contact_new") {
                        $(
                            ...
                            oSelf.delete_contact
                        );
                    }
                } else if ("additional_info" == sSection) {
        ...
        delete_avatar: function() {
            ...
            if (!oSelf.bInProgress) {
                open_popup(
                    gettext("Delete Avatar"),
                    584, "auto",
                    "/helper/popup-window/delete-avatar/?type=" + oSelf.sContextItemType + "&slug=" + oSelf.sSlug,
                    true
                );
            }
            return false;
