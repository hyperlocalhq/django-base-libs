...
function getType(v) {
    ...
    // sevenload
    if (v.match(/\.sevenload\.com\/videos\/([^\-]+)\-/)) {
		f.width.value = (window.settings.tinymce_media_default_width ? window.settings.tinymce_media_default_width : '500');
		f.height.value = (window.settings.tinymce_media_default_height ? window.settings.tinymce_media_default_height : '408');
		f.src.value = 'http://de.sevenload.com/pl/' + v.match(/\/videos\/([^\-]+)\-/)[1] + '/' + f.width.value + 'x' + f.height.value + '/swf';
		return 'flash';
	}
