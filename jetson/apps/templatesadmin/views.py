...
from jetson.apps.utils.utils import any
from jetson.apps.templatesadmin.forms import TemplateForm
from jetson.apps.templatesadmin import TemplatesAdminException
...
TEMPLATESADMIN_EDITHOOKS = getattr(
    ...
    ('jetson.apps.templatesadmin.edithooks.dotbackupfiles.DotBackupFilesHook', )
)
...
TEMPLATESADMIN_HIDE_HIDDEN = getattr(
    settings,
    'TEMPLATESADMIN_HIDE_HIDDEN',
    True,
    )

if isinstance(TEMPLATESADMIN_EDITHOOKS, basestring):
...
TEMPLATESADMIN_TEMPLATE_DIRS = [_fixpath(d) for d in TEMPLATESADMIN_TEMPLATE_DIRS]
...
def overview(request, template_name='templatesadmin/overview.html'):
    ...
    for templatedir in TEMPLATESADMIN_TEMPLATE_DIRS:
        for root, dirs, files in os.walk(templatedir):
            cur_dir_path, cur_dir_name = os.path.split(root)
            if TEMPLATESADMIN_HIDE_HIDDEN and fnmatch(cur_dir_name, ".*"):
                continue
