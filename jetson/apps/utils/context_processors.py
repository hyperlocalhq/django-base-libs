...
def prev_next_processor(request):
    ...
    """
    obj = request.httpstate.get('current_object', None)
    ...
    source_list = request.httpstate.get('source_list', None)
    ...
    if queryset_model and queryset_pk_list and isinstance(obj, queryset_model) and obj.pk in queryset_pk_list:
        count = len(queryset_pk_list)     
        ct = ContentType.objects.get_for_model(obj)
        index = queryset_index_dict.get('%s_%s' % (ct.id, obj.id), 0)
