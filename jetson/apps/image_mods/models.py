...
class FileManager:
    ...
    def delete_file(path):
        ...
        if os.path.isdir(abs_path):
            ...
            for version in VERSIONS:
                ...
                try:
                    os.unlink(os.path.join(settings.UPLOADS_ROOT, get_version_path(relative_server_path, version)))
                except:
                    pass
    ...
    def mod_exists(rel_orig_path, sysname):
        ...
        or else False.
        """
        orig_path_server = os.path.join(settings.UPLOADS_ROOT, *rel_orig_path.split("/"))
...
class ImageModification(models.Model):
    ...
    mask = FileBrowseField(_("Mask"), max_length=255, extensions=['.png'], blank=True, null=True)
    
    frame = FileBrowseField(_("Frame"), max_length=255, extensions=['.png'], blank=True, null=True)
    ...
    def delete_cached_images(self):
        ...
        for path in FileManager.locate("*_%s.*" % self.sysname, settings.UPLOADS_ROOT):
    ...
    def process_image(self, absolute_original_path):
        ...
        try:
            cropping = self.imagecropping_set.get(
                original=FileObject(absolute_original_path[len(settings.UPLOADS_ROOT) + 1:])
                )
        except:
            cropping = None
        ...
        if cropping:
            ...
            canvas = Image.new("RGB", (canvas_w, canvas_h), cropping.bgcolor.upper())
            cropped = im.crop(box)
        ...
        if self.mask:
            try:
                mask = Image.open(
                    os.path.join(settings.UPLOADS_ROOT, *self.mask.path.split("/")),
                    )
            except:
                pass
            else:
        ...
        if self.frame:
            try:
                frame = Image.open(
                    os.path.join(settings.UPLOADS_ROOT, *self.frame.path.split("/")),
                    )
            except:
                pass
            else:
                resized = resized.convert("RGBA")
                # resize/crop the image to the size of the mask-image
                resized = ImageOps.fit(resized, frame.size, method=Image.ANTIALIAS)    
...
class ImageCropping(models.Model):
    ...
    bgcolor = models.CharField(_('Canvas color'), max_length=7, default="#ffffff")
    ...
    def repr(self):
        return "%s (%d,%d)-(%d,%d)" % (
            self.original, x1, y1, x2, y2
            )
        
    def update_versions(self):
        for mod in self.mods.all():
            mod.process_image(os.path.join(settings.UPLOADS_ROOT, *self.original.path.split("/")))
    ...
    def delete(self, *args, **kwargs):
        ...
        for mod in mods:
            abs_path = os.path.join(settings.UPLOADS_ROOT, *self.original.path.split("/"))
