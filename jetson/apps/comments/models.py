...
class Comment(ObjectRelationMixin(is_required=True), UrlMixin):
    ...
    is_spam = models.BooleanField(_('is_spam'), default=False, help_text=_('Check this box if the comment should be marked as spam. The comment will not be displayed in this case.'))
    ...
    def __repr__(self):
        if self.user:
            return "%s: %s..." % (smart_str(self.user.username), smart_str(self.comment[:100]))
        else:
            return "%s: %s..." % (smart_str(self.name), smart_str(self.comment[:100]))

    def __unicode__(self):
        if self.user:
            return "%s: %s..." % (force_unicode(self.user.username), force_unicode(self.comment[:100]))
        else:
            return "%s: %s..." % (force_unicode(self.name), force_unicode(self.comment[:100]))
...
class KarmaScore(models.Model):
    ...
    def __repr__(self):
        return smart_str(_("%(score)d rating by %(user)s") % {'score': self.score, 'user': self.user})
    
    def __unicode__(self):
        return force_unicode(_("%(score)d rating by %(user)s") % {'score': self.score, 'user': self.user})
...
class ModeratorDeletion(models.Model):
    ...
    deletion_reason = models.ForeignKey(ModeratorDeletionReason, verbose_name='deletion reason')
...
class UserRatingManager(models.Manager):
    ...
    def can_rate(self, comment, user, rate_index):
        ...
        try:
            ...
            f = self.get(user__pk=user.id, comment__pk=comment.id, rate_index=rate_index)
            return False 
        except:
            pass
        return True
...
class UserRating(models.Model):
    ...
    def __repr__(self):
        return smart_str(_("Rating with index %(rate_index)d by %(user)r") % {"rate_index": self.rate_index, "user": self.user})
        
    def __unicode__(self):
        return force_unicode(_("Rating with index %(rate_index)d by %(user)r") % {"rate_index": self.rate_index, "user": self.user})
...
def comment_added(sender, instance, **kwargs):
    ...
    if creator and notification.NoticeSetting.objects.filter(
        user=creator,
        notice_type__sysname="comment_added",
        medium="1",
    ).exclude(frequency="never"):
        if instance.email:
