...
class IndividualRelation(models.Model):
    ...
    def _set_activation(self):
        ...
        self.activation = is_active and (self.activation or datetime.now()) or None
...
def individual_relation_requested(sender, instance, **kwargs):
    ...
    if notification.NoticeSetting.objects.filter(
        ...
        notice_type__sysname="individual_relation_requested",
        send=True,
        medium="1",
        ):
        notification.send(
...
def individual_relation_confirmed(sender, instance, **kwargs):
    ...
    if notification.NoticeSetting.objects.filter(
        user=instance.to_user,
        notice_type__sysname="individual_relation_confirmed",
        send=True,
        medium="1",
        ):
        notification.send(
