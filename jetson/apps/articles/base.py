...
from jetson.apps.utils.images import save_png_image, crop_to_rect
...
class ArticleTypeBase(MPTTModel, CreationModificationDateMixin, UrlMixin, SlugMixin()):
...
class ArticleBase(
    ...
    article_type = TreeForeignKey("articles.ArticleType", verbose_name=_("Type"), blank=True, null=True)
    ...
    image_title = models.CharField(_('image title'), max_length=50, blank=True, default="")
    image_description = ExtendedTextField(_('image description'), blank=True, default="")
    image = FileBrowseField(_('overview image'), max_length=255, directory="articles/", extensions=['.jpg', '.jpeg', '.gif','.png','.tif','.tiff'], blank=True)
