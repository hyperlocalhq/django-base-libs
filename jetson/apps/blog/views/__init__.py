...
def _get_month_archive(year, month, container, queryset, extra_context, allow_future, allow_empty):
    ...
    if last_day >= datetime.datetime.now().date() and not allow_future:
...
def get_blog_params(object_url_part, url_identifier, post_slug=None, **kwargs):
    """
...
def handle_request(request, object_url_part, url_identifier,
     year=None, month=None, day=None, post_slug=None, tag=None,   
     status=STATUS_CODE_PUBLISHED, paginate_by=None, page=None, 
     allow_future=False, allow_empty=True, extra_context={},
    ...
    extra_context = (get_blog_params(object_url_part, url_identifier, post_slug, **kwargs))
...
class BlogPostFormPreviewHandler(FormPreviewHandler):    
    ...
    def save_new(self, cleaned):
        if not self.container.pk:
            self.container.create_for_current_site()
        post = Post(
             blog = self.container,
             title = cleaned['title'],
             body = cleaned['body'],
...
def blog_feed(request, object_url_part, url_identifier, 
    ...
    wrapper for feeds
    """
    context = get_blog_params(object_url_part, url_identifier, **kwargs)
    obj = context['object']
    if obj:
...
def blog_modify_comment(request, object_url_part, url_identifier, comment_id, 
    ...
    """
    # first of all, get blog parameters from the url parts...
    extra_context = (get_blog_params(object_url_part, url_identifier, **kwargs))
    container = extra_context['container']
    obj = extra_context['object']
