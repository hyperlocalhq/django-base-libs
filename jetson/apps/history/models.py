...
class ExtendedLogEntryManager(models.Manager):
    def log_action(self, user=None, content_object=None, action_flag=A_UNDEFINED, scope=AS_SYSTEM, change_message='', change_message_de=''):
        e = self.model(
            user=user,
            action_flag=action_flag,
            change_message=change_message,
            change_message_de=change_message_de,
    ...
    def import_from_log_entries(self):
        ...
        for item in LogEntry.objects.all():
            e = self.model(
                ...
                change_message_de = item.change_message,
                )
            e.save()
        ...
        if user:
            if instance.pk:
                if A_CHANGE in TRACKED_MODELS[str_model]:
                    ...
                    named = dict([
                        (
                            lang[:2]=="en" and "change_message" or "change_message_" + lang[:2],
                    ...
                    if hasattr(instance, "get_log_message"):
                        for lang in dict(settings.LANGUAGES):
                            ...
                            if message:
                                named[lang=="en" and "change_message" or "change_message_" + lang] = message
