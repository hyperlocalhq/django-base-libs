    ...
    if ml.mailchimp_id:
        ...
        for mc_member in ms.listMembers(
            ...
            sub, created = Subscription.objects.get_or_create(
                ...
                mailinglist=ml,
                defaults={
                    'first_name': mc_member_info['data'][0]['merges']['FNAME'],
                    'last_name': mc_member_info['data'][0]['merges']['LNAME'],
                    'ip': "127.0.0.1",
                    }
                )
            ...
            counter += 1
        if counter:
            messages.append(_("%(counter)d members at %(list_title)s added from MailChimp.") % {
                'counter': counter,
                'list_title': ml.title,
                })
        ...
        for mc_member in ms.listMembers(
            ...
            sub.first_name = mc_member_info['data'][0]['merges']['FNAME']
            sub.last_name = mc_member_info['data'][0]['merges']['LNAME']
            sub.status = "subscribed"
            sub.save()
            triggered_emails.append(mc_member['email'])
            counter += 1
        if counter:
            messages.append(_("%(counter)d members at %(list_title)s updated from MailChimp.") % {
                'counter': counter,
                'list_title': ml.title,
                })
        ...
        for mc_member in ms.listMembers(
            ...
            counter += 1
        if counter:
            messages.append(_("%(counter)d members at %(list_title)s unsubscribed from MailChimp.") % {
                'counter': counter,
                'list_title': ml.title,
                })
        ...
        if subscribed_counter:
            messages.append(_("%(counter)d members at %(list_title)s subscribed on MailChimp.") % {
                'counter': subscribed_counter,
                'list_title': ml.title,
                })
        if unsubscribed_counter:
            messages.append(_("%(counter)d members at %(list_title)s unsubscribed on MailChimp.") % {
                'counter': unsubscribed_counter,
                'list_title': ml.title,
                })
                
        ml.last_sync = datetime.now()
        ml.save()
    return messages
