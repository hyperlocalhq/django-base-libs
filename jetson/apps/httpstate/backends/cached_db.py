...
class HttpStateStore(DBStore):
    ...
    def load(self):
        data = cache.get(self.httpstate_key, None)
        if data is None:
            data = super(HttpStateStore, self).load()
            cache.set(self.httpstate_key, data, httpstate_settings.HTTPSTATE_COOKIE_AGE)
        return data
    ...
    def save(self, must_create=False):
        ...
        cache.set(self.httpstate_key, self._httpstate, httpstate_settings.HTTPSTATE_COOKIE_AGE)

    def delete(self, httpstate_key=None):
        super(HttpStateStore, self).delete(httpstate_key)
        cache.delete(httpstate_key or self.httpstate_key)
