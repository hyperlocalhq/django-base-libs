...
from ruhrbuehnen.apps.locations.models import Location, Stage, Image, SocialMediaChannel
...
class BasicInfoForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            'latitude', 'longitude', 'phone_country', 'phone_area',
            'phone_number', 'fax_country', 'fax_area', 'fax_number', 'email',
            'website', 'tickets_street_address', 'tickets_street_address2',
            'tickets_postal_code', 'tickets_city', 'tickets_email',
            'tickets_website', 'tickets_phone_country', 'tickets_phone_area',
            'tickets_phone_number', 'tickets_fax_country', 'tickets_fax_area',
            'tickets_fax_number', 'mon_open', 'mon_break_close',
            'mon_break_open', 'mon_close', 'tue_open', 'tue_break_close',
            'tue_break_open', 'tue_close', 'wed_open', 'wed_break_close',
            'wed_break_open', 'wed_close', 'thu_open', 'thu_break_close',
            'thu_break_open', 'thu_close', 'fri_open', 'fri_break_close',
            'fri_break_open', 'fri_close', 'sat_open', 'sat_break_close',
            'sat_break_open', 'sat_close', 'sun_open', 'sun_break_close',
            'sun_break_open', 'sun_close', 'services', 'accessibility_options',
            'categories'
        ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['accessibility_options'
        ...
        for access_opt in self.fields['accessibility_options'].queryset:
            choices.append(
                ...
                <img src="%s%s" alt="" /> %s
                """ % (
                            settings.MEDIA_URL, access_opt.image.path,
                            access_opt.title
                        )
                    )
                )
        ...
        fieldset_content = [
            ...
            layout.Row(
                ...
                *[
                    layout.Div(
                        layout.Field(
                            'description_%s' % lang_code, disabled="disabled"
                        ),
                        css_class="col-xs-6 col-sm-6 col-md-6 col-lg-6",
                    ) for lang_code, lang_name in FRONTEND_LANGUAGES
                ]
            ...
            layout.Row(
                ...
                *[
                    layout.Div(
                        layout.
                        Field('teaser_%s' % lang_code, disabled="disabled"),
                        css_class="col-xs-6 col-sm-6 col-md-6 col-lg-6",
                    ) for lang_code, lang_name in FRONTEND_LANGUAGES
                ]
            )
...
class StageForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        layout_blocks.append(layout.Field('id'), )
        layout_blocks.append(layout.Field('DELETE'), )

        self.helper.layout = layout.Layout(*layout_blocks)


StageFormset = inlineformset_factory(
    Location, Stage, form=StageForm, formset=InlineFormSet, extra=0
)
...
def load_data(instance=None, request=None):
    ...
    if instance:
        ...
        form_step_data['basic']['accessibility_options'
                               ] = instance.accessibility_options.all()
        ...
        for social_media_channel in instance.socialmediachannel_set.all():
            ...
            form_step_data['basic']['sets']['social'].append(
                social_media_channel_dict
            )
...
def submit_step(current_step, form_steps, form_step_data, instance=None):
    if current_step == "basic":
        ...
        for lang_code, lang_name in FRONTEND_LANGUAGES:
            ...
            setattr(
                instance, 'tickets_calling_prices_%s_markup_type' % lang_code,
                'pt'
            )
            setattr(
                instance, 'tickets_additional_info_%s_markup_type' % lang_code,
                'pt'
            )

        for lang_code, lang_name in FRONTEND_LANGUAGES:
            form_step_data[current_step]['description_%s' % lang_code
                                        ] = getattr(
                                            instance,
                                            'description_%s' % lang_code
                                        )
        ...
        if not instance.slug:
            instance.slug = get_unique_value(
                Location, slugify(instance.title_de), instance_pk=instance.pk
            )
    ...
    if current_step == "stages":
        ...
        for stage_dict in form_step_data['stages']['sets']['stages']:
            ...
            for lang_code, lang_name in FRONTEND_LANGUAGES:
                stage_dict['description_%s' % lang_code
                          ] = getattr(stage, 'description_%s' % lang_code)
