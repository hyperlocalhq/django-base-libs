...
def location_list(request, year=None, month=None, day=None):
    ...
    qs = Location.objects.filter(
        status="published"
    ).order_by("sort_order", Lower("title_%s" % request.LANGUAGE_CODE))
...
def change_location(request, slug):
    ...
    return show_form_step(
        request,
        LOCATION_FORM_STEPS,
        extra_context={'location': instance},
        instance=instance
    )
...
def image_overview(request, slug):
    ...
    return render(
        request, "locations/gallery/overview.html", {'location': instance}
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = LocationImage(location=instance)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "location_gallery_overview", slug=instance.slug
                    )
    else:
        if media_file_obj:
            # existing media file
