...
class FestivalAdmin(ExtendedModelAdmin):
    list_display = (
        'id', 'title', 'start', 'end', 'get_organizers', 'get_owners_list',
        'creation_date', 'modified_date', 'featured', 'slideshow', 'newsletter',
        'status'
    )
    ...
    list_filter = (
        'start', 'creation_date', 'modified_date', 'featured', 'slideshow',
        'newsletter', 'status'
    )
    ...
    fieldsets += [
        (
            _("Tickets"), {
                'fields':
                    (
                        'tickets_street_address', 'tickets_street_address2',
                        ...
                        'tickets_website', (
                            ...
                            _("Fax"), {
                                'fields':
                                    (
                                        'tickets_fax_country',
                                        'tickets_fax_area', 'tickets_fax_number'
                                    )
                            }
                        ),
                        get_admin_lang_section(
                            _("Calling prices"), [
                                'tickets_calling_prices',
                            ]
                        )
                    )
            }
            ...
            _("Tickets Opening Hours"), {
                'fields':
                    (
                        ...
                        (
                            'mon_open', 'mon_break_close', 'mon_break_open',
                            'mon_close'
                        ),
                        (
                            'tue_open', 'tue_break_close', 'tue_break_open',
                            'tue_close'
                        ),
                        (
                            'wed_open', 'wed_break_close', 'wed_break_open',
                            'wed_close'
                        ),
                        (
                            'thu_open', 'thu_break_close', 'thu_break_open',
                            'thu_close'
                        ),
                        (
                            'fri_open', 'fri_break_close', 'fri_break_open',
                            'fri_close'
                        ),
                        (
                            'sat_open', 'sat_break_close', 'sat_break_open',
                            'sat_close'
                        ),
                        (
                            'sun_open', 'sun_break_close', 'sun_break_open',
                            'sun_close'
                        ),
                        get_admin_lang_section(
                            _("Exceptions"), [
                        ...
                        'press_contact_name', 'press_email', 'press_website', (
                            _("Phone"), {
                                'fields':
                                    (
                                        'press_phone_country',
                                        'press_phone_area', 'press_phone_number'
                                    )
                            }
                        ), (
                            _("Mobile"), {
                            ...
                            _("Fax"), {
                                'fields':
                                    (
                                        'press_fax_country', 'press_fax_area',
                                        'press_fax_number'
                                    )
                            }
                        )
    ...
    def get_organizers(self, obj):
        ...
        if obj.organizers.count():
            html.append(
                ', '.join(
                    (
                        '<a href="' + loc.get_url_path() + '" tagret="_blank">'
                        + unicode(loc) + '</a>' for loc in obj.organizers.all()
                    )
                )
            )
        return '<br />'.join(html)
    ...
    def owners_view(self, request, festival_id):
        ...
        if request.method == "POST":
            ...
            form = OwnersForm(initial={'users': festival.get_owners()})
        ...
        return render(
            request, 'admin/owners.html', {
                ...
                'original':
                    festival,
                'app_label':
                    Festival._meta.app_label,
                'opts':
                    Festival._meta,
                'form':
                    form,
                'title':
                    ugettext('The owners of %(festival)s') % {
                        'festival': festival
                    },
            }
        )
