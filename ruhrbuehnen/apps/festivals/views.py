...
from ruhrbuehnen.apps.festivals.forms.gallery import PDFForm, PDFDeletionForm
...
def festival_events(request, slug):
    ...
    qs = qs.filter(start_date__gte=datetime.now(), )

    qs = qs.order_by(
        'start_date', 'start_time',
        'production__title_%s' % request.LANGUAGE_CODE
    )
...
def change_festival(request, slug):
    ...
    return show_form_step(
        request,
        FESTIVAL_FORM_STEPS,
        extra_context={'festival': instance},
        instance=instance
    )
...
def duplicate_festival(request, slug):
    ...
    ) or not request.user.has_perm("festivals.add_festival"):
        return access_denied(request)
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponse(
                reverse("change_festival", kwargs={'slug': new_festival.slug})
            )
    else:
        form = FestivalDuplicateForm(instance=festival)
    return render(
        request, "festivals/forms/duplication_form.html", {'form': form}
    )
...
def image_overview(request, slug):
    ...
    return render(
        request, "festivals/gallery/overview.html", {'festival': instance}
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = FestivalImage(festival=instance)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "festival_gallery_overview", slug=instance.slug
                    )
    else:
        if media_file_obj:
            # existing media file
...
def pdf_overview(request, slug):
    ...
    return render(
        request, "festivals/gallery/pdf_overview.html", {
            'festival': festival,
        }
    )
...
def create_update_pdf(request, slug, mediafile_token="", **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = FestivalPDF(festival=festival)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "festival_gallery_overview", slug=festival.slug
                    )
    else:
        if media_file_obj:
            # existing media file
