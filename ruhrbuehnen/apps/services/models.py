...
class Banner(CreationModificationDateMixin):
    ...
    short_description = MultilingualTextField(
    ...
    header_bg_color = models.CharField(
        ...
        max_length=20,
        help_text=_(
            """Use RGB or HTML format, like "rgb(0, 0, 255)" or "#0000ff"."""
        )
    )
    header_icon = FileBrowseField(
        _("Header Icon"),
        max_length=255,
        directory="services/",
    ...
    def __unicode__(self):
        if self.subtitle:
            return u"{title} - {subtitle}".format(
...
class IndexItem(CMSPlugin):
    ...
    width = models.CharField(
        _("Width"), max_length=20, default="single", choices=WIDTH_CHOICES
    )
    ...
    external_link = models.URLField(
        _("External Link"), max_length=255, blank=True
    )
...
class ServiceListItem(CMSPlugin):
    ...
    location = models.ForeignKey(
        ...
        help_text=_("Theater linked to this service"),
        blank=True,
        null=True
    )
    short_description = ExtendedTextField(_("Short Description"), blank=True)
...
class TitleAndText(CMSPlugin):
    ...
    width = models.CharField(
        _("Width"), max_length=20, default="full", choices=WIDTH_CHOICES
    )
...
class ImageAndText(CMSPlugin):
    ...
    image = FileBrowseField(
        _("Image"), max_length=255, extensions=IMAGE_EXTENSIONS
    )
