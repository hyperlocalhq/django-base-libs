...
class EventDocument(DocType):
    ...
    in_program_of = fields.NestedField(
        properties={
            'title_de': fields.StringField(),
            'title_en': fields.StringField(),
            'pk': fields.IntegerField(),
    ...
    play_locations = fields.NestedField(
        properties={
            'title_de': fields.StringField(),
            'title_en': fields.StringField(),
            'pk': fields.IntegerField(),
    ...
    play_stages = fields.NestedField(
        properties={
            'title_de': fields.StringField(),
            'title_en': fields.StringField(),
            'pk': fields.IntegerField(),
    ...
    language_and_subtitles = fields.NestedField(
        properties={
            'title_de': fields.StringField(),
            'title_en': fields.StringField(),
            'pk': fields.IntegerField(),
    ...
    def get_queryset(self):
        return super(EventDocument,
                     self).get_queryset().select_related('production').filter(
    ...
    def get_instances_from_related(self, related_instance):
        if isinstance(related_instance, Production):
            ...
            if not getattr(production, "_skip_search_document_update", False):
                if production.status == "published" and production.part_set.count(
                ) == 0:
            ...
            if production.status == "published" and production.part_set.count(
                ...
                return production.event_set.exclude(event_status="trashed", )
    ...
    def prepare_teaser_de(self, instance):
        return instance.get_rendered_teaser_de(
        ) or instance.production.get_rendered_teaser_de()
    ...
    def get_language_and_subtitles_title(self, language=settings.LANGUAGE_CODE):
        if not self.language_and_subtitles:
            return ''
        return getattr(
            self.language_and_subtitles[0], 'title_{}'.format(language)
        )
    ...
    def prepare_image_author(self, instance):
        ...
        if image_path:
            try:
                ...
                FileDescription.DoesNotExist,
                FileDescription.MultipleObjectsReturned
            ):
                return ''
            else:
