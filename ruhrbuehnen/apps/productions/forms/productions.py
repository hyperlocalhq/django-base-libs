...
from ruhrbuehnen.apps.productions.models import Production, ProductionCategory, ProductionLeadership, ProductionAuthorship, ProductionInvolvement, LanguageAndSubtitles, ProductionSocialMediaChannel, ProductionSponsor
from ruhrbuehnen.apps.people.models import Person, InvolvementType, AuthorshipType
...
from ruhrbuehnen.utils.forms import ModelMultipleChoiceTreeField
...
class BasicInfoForm(autocomplete_light.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['play_locations'].help_text = _(
            'Choose only when differs from the "In the programm of".'
        )

        self.fields['in_program_of'].label += ' (' + ugettext(
            'or'
        ) + ' <a href="" class="enter_location">' + ugettext(
            'enter a new location below'
        ) + '</a>)'
        self.fields['play_locations'].label = ugettext(
            'Theaters'
        ) + ' (' + ugettext('or'
                           ) + ' <a href="" class="enter_location">' + ugettext(
                               'enter a new location below'
        ...
        fieldset_content = [
            ...
            layout.Row(
                ...
                *[
                    layout.Div(
                        layout.
                        Field('website_%s' % lang_code, placeholder="http://"),
                        css_class="col-xs-6 col-sm-6 col-md-6 col-lg-6",
                    ) for lang_code, lang_name in FRONTEND_LANGUAGES
                ]
            )
        ...
        layout_blocks.append(
            layout.Fieldset(
                ...
                layout.HTML(
                    """{% load i18n %}<p class="help-block">{% trans "Select one or more categories." %}</p>"""
                ),
                layout.Row(
                    layout.Div(
                        layout.Div(
                            layout.Field(
                                "categories",
                                template="utils/checkboxselectmultipletree.html"
                            )
                        ),
    ...
    def clean(self):
        ...
        if not cleaned.get('in_program_of'
                          ) and not cleaned.get('location_title'):
            msg = _(
                "Choose a location from the database or enter another location's title and address below."
            )
        ...
        layout_blocks.append(
            layout.Fieldset(
                ...
                layout.Row(
                    layout.Div(
                        layout.Field(
                            "edu_offer_website", placeholder="http://"
                        ),
                        css_class="col-xs-12 col-sm-12 col-md-12 col-lg-12"
                    ),
                ),
...
ProductionLeadershipFormset = inlineformset_factory(
    Production,
    ProductionLeadership,
    form=ProductionLeadershipForm,
...
class ProductionAuthorshipForm(autocomplete_light.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['work_title'].help_text = _(
            "When differs from the title of the production."
        )
...
ProductionAuthorshipFormset = inlineformset_factory(
    Production,
    ProductionAuthorship,
    form=ProductionAuthorshipForm,
    formset=InlineFormSet,
    extra=0
)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for lang_code, lang_name in FRONTEND_LANGUAGES:
            for f in [
                ...
                self.fields[f].label = ugettext("Function") + (
                    """ <span class="lang">%s</span>""" % lang_code.upper()
                ) + ' (' + ugettext(
                    'or'
                ) + ' <a href="" class="choose_type">' + ugettext(
                    'choose a function from the database'
                ) + '</a>)'

        self.fields['involvement_type'].label = ugettext(
            "Function"
        ) + ' (' + ugettext('or'
                           ) + ' <a href="" class="enter_type">' + ugettext(
...
ProductionInvolvementFormset = inlineformset_factory(
    Production,
    ProductionInvolvement,
    form=ProductionInvolvementForm,
    formset=InlineFormSet,
    extra=0
)
...
ProductionSponsorFormset = inlineformset_factory(
    Production,
    ProductionSponsor,
    form=ProductionSponsorForm,
    formset=InlineFormSet,
    extra=0
)
...
def load_data(instance=None, request=None):
    ...
    if instance:
        ...
        form_step_data['description']['characteristics'
                                     ] = instance.characteristics.all()

        for leadership in instance.productionleadership_set.order_by(
            'sort_order'
        ):
            leadership_dict = {
                'id': leadership.pk,
                'person': leadership.person,
                'sort_order': leadership.sort_order,
                'imported_sort_order': leadership.imported_sort_order
            }
            for lang_code, lang_name in FRONTEND_LANGUAGES:
                leadership_dict['function_%s' % lang_code] = getattr(
                    leadership, 'function_%s' % lang_code
                )
            form_step_data['description']['sets']['leaderships'].append(
                leadership_dict
            )

        for authorship in instance.productionauthorship_set.order_by(
            'sort_order'
        ):
            authorship_dict = {
                'id': authorship.pk,
                'person': authorship.person,
                'sort_order': authorship.sort_order,
                'imported_sort_order': authorship.imported_sort_order,
                'authorship_type': authorship.authorship_type
            }
            form_step_data['description']['sets']['authorships'].append(
                authorship_dict
            )

        for involvement in instance.productioninvolvement_set.order_by(
            'sort_order'
        ):
            involvement_dict = {
                'id': involvement.pk,
                'person': involvement.person,
                'sort_order': involvement.sort_order,
                'imported_sort_order': involvement.imported_sort_order,
                'involvement_type': involvement.involvement_type
            }
            for lang_code, lang_name in FRONTEND_LANGUAGES:
                involvement_dict['another_type_%s' % lang_code] = getattr(
                    involvement, 'another_type_%s' % lang_code
                )
                involvement_dict['involvement_role_%s' % lang_code] = getattr(
                    involvement, 'involvement_role_%s' % lang_code
                )
                involvement_dict['involvement_instrument_%s' % lang_code
                                ] = getattr(
                                    involvement,
                                    'involvement_instrument_%s' % lang_code
                                )
            form_step_data['description']['sets']['involvements'].append(
                involvement_dict
            )
...
def submit_step(current_step, form_steps, form_step_data, instance=None):
    if current_step == "basic":
        ...
        fields = [
            'ensembles', 'organizers', 'in_cooperation_with', 'location_title',
            'street_address', 'street_address2', 'postal_code', 'city',
            'latitude', 'longitude', 'show_among_others', 'no_overwriting'
        ]
        for lang_code, lang_name in FRONTEND_LANGUAGES:
            fields += [
                'prefix_%s' % lang_code,
        ...
        if not instance.slug:
            instance.slug = get_unique_value(
                Production,
                better_slugify(instance.title_de) or u"production",
        ...
        models.signals.post_save.send(
            type(instance), instance=instance, created=False
        )
    ...
    if current_step == "description":
        ...
        language_and_subtitles = form_step_data[current_step][
            'language_and_subtitles']
        ...
        if language_and_subtitles:
            if isinstance(language_and_subtitles, LanguageAndSubtitles):
                ...
                instance.language_and_subtitles = LanguageAndSubtitles.objects.get(
                    pk=language_and_subtitles
                )
        ...
        for lang_code, lang_name in FRONTEND_LANGUAGES:
            ...
            setattr(
                instance, 'concert_program_%s_markup_type' % lang_code, 'pt'
            )
            setattr(
                instance, 'supporting_program_%s_markup_type' % lang_code, 'pt'
            )
            setattr(instance, 'remarks_%s_markup_type' % lang_code, 'pt')
            setattr(
                instance, 'price_information_%s_markup_type' % lang_code, 'pt'
            )
            setattr(
                instance, 'other_characteristics_%s_markup_type' % lang_code,
                'pt'
            )
        ...
        for leadership_dict in form_step_data['description']['sets'][
            'leaderships']:
        ...
        instance.productionleadership_set.exclude(
            pk__in=leadership_ids_to_keep
        ).delete()
        ...
        instance.productionauthorship_set.exclude(
            pk__in=authorship_ids_to_keep
        ).delete()
        ...
        instance.productioninvolvement_set.exclude(
            pk__in=involvement_ids_to_keep
        ).delete()
        ...
        instance.productionsponsor_set.exclude(pk__in=sponsor_ids_to_keep
                                              ).delete()
