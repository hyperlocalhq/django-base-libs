...
from ruhrbuehnen.apps.productions.models import Event, EventLeadership, EventAuthorship, EventInvolvement, EventSocialMediaChannel, EventSponsor
...
class BasicInfoForm(autocomplete_light.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['in_program_of'] = forms.CharField(
            ...
            initial=",".join(
                [
                    "%d" % location_id for location_id in in_program_of.
...
class DescriptionForm(autocomplete_light.ModelForm):
    ...
    def clean(self):
        ...
        for fname in self.fields.iterkeys():
            if cleaned[fname] and cleaned[fname] == getattr(
                production, fname, None
            ):
                del cleaned[fname]
        return cleaned
...
EventLeadershipFormset = inlineformset_factory(
    Event,
    EventLeadership,
    form=EventLeadershipForm,
...
EventAuthorshipFormset = inlineformset_factory(
    Event,
    EventAuthorship,
    form=EventAuthorshipForm,
    formset=InlineFormSet,
    extra=0
)
...
EventInvolvementFormset = inlineformset_factory(
    Event,
    EventInvolvement,
    form=EventInvolvementForm,
    formset=InlineFormSet,
    extra=0
)
...
EventSponsorFormset = inlineformset_factory(
    Event, EventSponsor, form=EventSponsorForm, formset=InlineFormSet, extra=0
)
