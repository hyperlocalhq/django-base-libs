...
class ImportToRuhrbuehnenBaseXML(NoArgsCommand, ImportCommandMixin):
    """
    Base command to extend to import productions and events to RuhrBuehnen in XML format
    """
    help = "Import based on http://www.ruhr-buehnen.de/media/docs/import-specification/production_import_specs.html"
    ...
    def prepare(self):
        ...
        raise NotImplementedError(
            "The define_service() method should be implemented."
        )
    ...
    def main(self):
        ...
        if r.status_code != requests.codes.ok:
            ...
            self.stderr.write(
                u"Error status {} when trying to access {}".format(
        ...
        self._total_production_count = int(
            self.get_child_text(root_node.find("./meta"), "total_count")
        )
        ...
        for child_node in node.findall(tag):
            ...
            if all_attributes_match:
                ...
                return force_unicode(
                    u''.join([t for t in child_node.itertext()])
                ).replace(r'\n', '\n')
        return u''
    ...
    def save_file_description(self, path, xml_node):
        ...
        file_description.author = (
            self.get_child_text(xml_node, 'copyright') or text
        ).replace("&copy;", "©")
    ...
    def parse_and_use_texts(self, xml_node, instance):
        instance.description_de = self.get_child_text(
            xml_node, 'description_de'
        )
        instance.description_en = self.get_child_text(
            xml_node, 'description_en'
        )
        ...
        instance.concert_program_de = self.get_child_text(
            xml_node, 'concert_program_de'
        )
        instance.concert_program_en = self.get_child_text(
            xml_node, 'concert_program_en'
        )
        instance.supporting_program_de = self.get_child_text(
            xml_node, 'supporting_program_de'
        )
        instance.supporting_program_en = self.get_child_text(
            xml_node, 'supporting_program_en'
        )
        instance.remarks_de = self.get_child_text(xml_node, 'remarks_de')
        instance.remarks_en = self.get_child_text(xml_node, 'remarks_en')
        instance.duration_text_de = self.get_child_text(
            xml_node, 'duration_text_de'
        )
        instance.duration_text_en = self.get_child_text(
            xml_node, 'duration_text_en'
        )
        instance.subtitles_text_de = self.get_child_text(
            xml_node, 'subtitles_text_de'
        )
        instance.subtitles_text_en = self.get_child_text(
            xml_node, 'subtitles_text_en'
        )
        ...
        instance.price_information_de = self.get_child_text(
            xml_node, 'price_information_de'
        )
        instance.price_information_en = self.get_child_text(
            xml_node, 'price_information_en'
        )
    ...
    def save_page(self, productions_node):
        ...
        from ruhrbuehnen.apps.productions.models import ProductionVideo
        from ruhrbuehnen.apps.productions.models import ProductionLiveStream
        from ruhrbuehnen.apps.productions.models import ProductionImage
        from ruhrbuehnen.apps.productions.models import ProductionSponsor
        ...
        from ruhrbuehnen.apps.productions.models import EventVideo
        from ruhrbuehnen.apps.productions.models import EventLiveStream
        from ruhrbuehnen.apps.productions.models import EventImage
        from ruhrbuehnen.apps.productions.models import EventSponsor
        ...
        for prod_index, prod_node in enumerate(prod_nodes, 1):
            ...
            title_de = self.get_child_text(prod_node,
                                           'title_de').replace('\n',
                                                               ' ').strip()
            title_en = self.get_child_text(prod_node,
                                           'title_en').replace('\n',
                                                               ' ').strip()
            ...
            if self.verbosity >= self.NORMAL:
                self.stdout.write(
                    u"%d/%d %s | %s" % (
                        self._production_counter, self._total_production_count,
                        title_de, title_en
                    )
                )
                self.stdout.flush()
            ...
            prod.status = self.get_child_text(
                prod_node, 'status'
            ) or self.DEFAULT_PUBLISHING_STATUS
            prod.title_de = strip_tags(title_de.replace('<br />', ' '))
            prod.title_en = strip_tags(
                (title_en or title_de).replace('<br />', ' ')
            )
            prod.prefix_de = strip_tags(
                self.get_child_text(prod_node,
                                    'prefix_de').replace('<br />', ' ')
            )
            prod.prefix_en = strip_tags(
                self.get_child_text(prod_node,
                                    'prefix_en').replace('<br />', ' ')
            )
            prod.subtitle_de = strip_tags(
                self.get_child_text(prod_node,
                                    'subtitle_de').replace('<br />', ' ')
            )
            prod.subtitle_en = strip_tags(
                self.get_child_text(prod_node,
                                    'subtitle_en').replace('<br />', ' ')
            )
            prod.original_de = strip_tags(
                self.get_child_text(prod_node,
                                    'original_de').replace('<br />', ' ')
            )
            prod.original_en = strip_tags(
                self.get_child_text(prod_node,
                                    'original_en').replace('<br />', ' ')
            )
            ...
            prod.in_cooperation_with = self.get_child_text(
                prod_node, 'in_cooperation_with'
            )

            prod.free_entrance = (
                self.get_child_text(prod_node, 'free_entrance') == "true"
            )
            try:
                prod.price_from = Decimal(
                    self.get_child_text(prod_node, 'price_from')
                )
            except:
                prod.price_from = None
            try:
                prod.price_till = Decimal(
                    self.get_child_text(prod_node, 'price_till')
                )
            except:
                prod.price_till = None
            prod.tickets_website = self.get_child_text(
                prod_node, 'tickets_website'
            )
            prod.edu_offer_website = self.get_child_text(
                prod_node, 'edu_offer_website'
            )
            ...
            prod.location_title = self.get_child_text(
                prod_node, 'location_title'
            )
            prod.street_address = self.get_child_text(
                prod_node, 'street_address'
            )
            prod.street_address2 = self.get_child_text(
                prod_node, 'street_address2'
            )
            prod.postal_code = self.get_child_text(prod_node, 'postal_code')
            prod.city = self.get_child_text(prod_node, 'city')
            try:
                prod.latitude = float(
                    self.get_child_text(prod_node, 'latitude')
                )
            except:
                prod.latitude = None
            try:
                prod.longitude = float(
                    self.get_child_text(prod_node, 'longitude')
                )
            except:
                prod.longitude = None
            ...
            try:
                prod.language_and_subtitles = LanguageAndSubtitles.objects.get(
                    slug=self.
                    get_child_text(prod_node, 'language_and_subtitles_id')
                )
            ...
            if self.DEFAULT_IN_PROGRAM_OF_LOCATION_ID:
                try:
                    location = Location.objects.get(
                        pk=self.DEFAULT_IN_PROGRAM_OF_LOCATION_ID
                    )
                except Location.DoesNotExist:
                    pass
                else:
                    prod.in_program_of.add(location)
            for location_id_node in prod_node.findall(
                "./in_program_of/location_id"
            ):
                try:
            ...
            for location_id_node in prod_node.findall(
                "./play_locations/location_id"
            ):
                if not self.DEFAULT_IN_PROGRAM_OF_LOCATION_ID or self.DEFAULT_IN_PROGRAM_OF_LOCATION_ID != int(
                    location_id_node.text
                ):
                    try:
                        location = Location.objects.get(
                            pk=location_id_node.text
                        )
                    except Location.DoesNotExist:
                        pass
                    else:
            ...
            for category_id_node in prod_node.findall(
                ...
                try:
                    cat = ProductionCategory.objects.get(
                        pk=category_id_node.text
                    )
            ...
            for ch_id_node in prod_node.findall(
                "./characteristics/characteristic_id"
            ):
                try:
                    ch = ProductionCharacteristics.objects.get(
                        slug=ch_id_node.text
                    )
            ...
            for video_node in prod_node.findall("./videos/video"):
                ...
                creation_date_string = self.get_child_text(
                    video_node, 'creation_date'
                )
                if creation_date_string:
                    video.creation_date = parse_datetime(creation_date_string)

                modified_date_string = self.get_child_text(
                    video_node, 'modified_date'
                )
                ...
                try:
                    video.sort_order = int(
                        self.get_child_text(video_node, 'sort_order')
                    )
                except:
                    video.sort_order = 1
                video.save()
            ...
            for live_stream_node in prod_node.findall(
                "./live_streams/live_stream"
            ):
                ls = ProductionLiveStream(production=prod)
                ls.creation_date = parse_datetime(
                    self.get_child_text(live_stream_node, 'creation_date')
                )
                modified_date_string = self.get_child_text(
                    live_stream_node, 'modified_date'
                )
                if modified_date_string:
                ...
                ls.link_or_embed = self.get_child_text(
                    live_stream_node, 'embed'
                )
                try:
                    ls.sort_order = int(
                        self.get_child_text(live_stream_node, 'sort_order')
                    )
                except:
                    ls.sort_order = 1
                ls.save()
            ...
            if not self.skip_images:
                ...
                for image_node in prod_node.findall('./images/image'):
                    ...
                    if image_response.status_code in (
                        ...
                        mf.copyright_restrictions = self.get_child_text(
                            image_node, 'copyright_restrictions'
                        ) or "general_use"
                        mf.save()
                        image_ids_to_keep.append(mf.pk)
            ...
            for pdf_node in prod_node.findall('./pdfs/pdf'):
                ...
                if pdf_response.status_code in (
                    requests.codes.ok, requests.codes.not_modified
                ):
                    image_mods.FileManager.save_file_for_object(
                        mf,
            ...
            for person_node in prod_node.findall('./leaders/leader'):
                try:
                    prefix = Prefix.objects.get(
                        slug=self.get_child_text(person_node, 'prefix_id')
                    )
                except:
                    prefix = None
                ...
                try:
                    imported_sort_order = int(
                        self.get_child_text(person_node, 'sort_order')
                    )
                except:
            ...
            for person_node in prod_node.findall('./authors/author'):
                ...
                try:
                    authorship_type = AuthorshipType.objects.get(
                        slug=self.
                        get_child_text(person_node, 'authorship_type_id')
                    )
                except:
            ...
            for person_node in prod_node.findall('./participants/participant'):
                ...
                try:
                    involvement_type = InvolvementType.objects.get(
                        slug=self.
                        get_child_text(person_node, 'involvement_type_id')
                    )
                except:
                ...
                prod.productioninvolvement_set.create(
                    ...
                    involvement_role_de=self.get_child_text(
                        person_node, 'role_de'
                    ),
                    involvement_role_en=self.get_child_text(
                        person_node, 'role_en'
                    ),
                    involvement_instrument_de=self.get_child_text(
                        person_node, 'instrument_de'
                    ),
                    involvement_instrument_en=self.get_child_text(
                        person_node, 'instrument_en'
                    ),
                    imported_sort_order=imported_sort_order,
                )
            for sort_order, item in enumerate(
            ...
            for sponsor_node in prod_node.findall('./sponsors/sponsor'):
                sponsor = ProductionSponsor(
                    production=prod,
                    title_de=self.get_child_text(sponsor_node,
                                                 'title_de')[:255],
                    title_en=self.get_child_text(sponsor_node,
                                                 'title_en')[:255],
                ...
                if image_url:
                    ...
                    if image_response.status_code in (
                        ...
                        image_mods.FileManager.save_file_for_object(
                            ...
                            image_response.content,
                            field_name="image",
                            subpath="productions/{}/sponsors/".format(
                                prod.slug
                            ),
                        )
            ...
            for event_node in prod_node.findall('./events/event'):
                ...
                event.start_date = parse_datetime(
                    self.get_child_text(event_node, 'start_date')
                ).date()

                start_time = self.get_child_text(event_node, 'start_time')
                if start_time:
                    event.start_time = parse_datetime(start_time).time()
                ...
                try:
                    event.pauses = int(
                        self.get_child_text(event_node, 'pauses')
                    )
                except:
                    event.pauses = None
                ...
                event.free_entrance = (
                    self.get_child_text(event_node, 'free_entrance') == "true"
                )
                try:
                    event.price_from = Decimal(
                        self.get_child_text(event_node, 'price_from')
                    )
                except:
                    event.price_from = None
                try:
                    event.price_till = Decimal(
                        self.get_child_text(event_node, 'price_till')
                    )
                except:
                    event.price_till = None
                event.tickets_website = self.get_child_text(
                    event_node, 'tickets_website'
                )
                event.location_title = self.get_child_text(
                    event_node, 'location_title'
                )
                event.street_address = self.get_child_text(
                    event_node, 'street_address'
                )
                event.street_address2 = self.get_child_text(
                    event_node, 'street_address2'
                )
                event.postal_code = self.get_child_text(
                    event_node, 'postal_code'
                )
                event.city = self.get_child_text(event_node, 'city')
                try:
                    event.latitude = float(
                        self.get_child_text(event_node, 'latitude')
                    )
                except:
                    event.latitude = None
                try:
                    event.longitude = float(
                        self.get_child_text(event_node, 'longitude')
                    )
                except:
                ...
                try:
                    event.language_and_subtitles = LanguageAndSubtitles.objects.get(
                        slug=self.
                        get_child_text(event_node, 'language_and_subtitles_id')
                    )
                except LanguageAndSubtitles.DoesNotExist:
                ...
                event.event_status = self.get_child_text(
                    event_node, 'event_status'
                )
                event.ticket_status = self.get_child_text(
                    event_node, 'ticket_status'
                )
                ...
                for location_id_node in event_node.findall(
                    "./play_locations/location_id"
                ):
                    if not self.DEFAULT_IN_PROGRAM_OF_LOCATION_ID or self.DEFAULT_IN_PROGRAM_OF_LOCATION_ID != int(
                        location_id_node.text
                ...
                for stage_id_node in event_node.findall(
                    "./play_stages/stage_id"
                ):
                    try:
                        stage = Stage.objects.get(pk=stage_id_node.text)
                    except Stage.DoesNotExist:
                ...
                for ch_id_node in event_node.findall(
                    "./characteristics/characteristic_id"
                ):
                    try:
                        ch = EventCharacteristics.objects.get(
                            slug=ch_id_node.text
                        )
                ...
                for video_node in event_node.findall("./videos/video"):
                    ...
                    video.creation_date = parse_datetime(
                        self.get_child_text(video_node, 'creation_date')
                    )
                    video.modified_date = parse_datetime(
                        self.get_child_text(video_node, 'modified_date')
                    )
                    video.title_de = self.get_child_text(video_node, 'title_de')
                    video.title_en = self.get_child_text(video_node, 'title_en')
                    video.link_or_embed = self.get_child_text(
                        video_node, 'embed'
                    )
                    try:
                        video.sort_order = int(
                ...
                for video_node in event_node.findall("./videos/video"):
                    ...
                    ls.modified_date = parse_datetime(
                        self.get_child_text(video_node, 'modified_date')
                    )
                ...
                for person_node in event_node.findall('./leaders/leader'):
                    ...
                    event.eventleadership_set.create(
                        person=p,
                        function_de=self.get_child_text(
                            person_node, 'function_de'
                        ),
                        function_en=self.get_child_text(
                            person_node, 'function_en'
                        ),
                        imported_sort_order=imported_sort_order,
                    )
                for sort_order, item in enumerate(
