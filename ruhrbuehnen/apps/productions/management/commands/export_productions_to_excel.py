...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, *args, **options):
        ...
        for im_index, im in enumerate(LocationImage.objects.filter(location__status="published").distinct(), 1):
            ...
            ws.append([im.id, im.location_id, 'http://www.ruhr-buehnen.de' + settings.MEDIA_URL + im.path.path, file_desc.title_de, file_desc.description_de, file_desc.author, im.copyright_restrictions])
        ...
        for ev_index, ev in enumerate(Event.objects.filter(production__status="published").distinct(), 1):
            self.stdout.write("%d/%d %s %s\n" % (ev_index, ev_count, smart_str(ev.start_date), smart_str(ev.start_time)))
        ...
        for im_index, im in enumerate(ProductionImage.objects.filter(production__status="published").distinct(), 1):
            ...
            if file_descs:
                file_desc = file_descs[0]
            ws.append([im.id, im.production_id, 'http://www.ruhr-buehnen.de' + settings.MEDIA_URL + im.path.path, file_desc.title_de, file_desc.description_de, file_desc.author, im.copyright_restrictions])
        ...
        for im_index, im in enumerate(EventImage.objects.filter(event__production__status="published").distinct(), 1):
            ...
            if file_descs:
                file_desc = file_descs[0]
            ws.append([im.id, im.event_id, 'http://www.ruhr-buehnen.de' + settings.MEDIA_URL + im.path.path, file_desc.title_de, file_desc.description_de, file_desc.author, im.copyright_restrictions])
