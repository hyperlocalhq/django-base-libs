...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, *args, **options):
        ...
        self.culturebase_service = Service.objects.get(
            sysname="culturebase_prods"
        )

        self.trash_deprecated_for_service(
            Service.objects.get(sysname="culturebase_dob_prods")
        )
        self.trash_deprecated_for_service(
            Service.objects.get(sysname="culturebase_radialsystem_prods")
        )
        self.trash_deprecated_for_service(
            Service.objects.get(sysname="culturebase_sob_prods")
        )

    def trash_deprecated_for_service(self, service):

        if self.verbosity >= self.NORMAL:
            self.stdout.write(u"=== Eliminating Old %s ===\n" % service.title)

        mapper_count = service.objectmapper_set.filter(
            content_type__model="production"
        ).count()
        for mapper_index, new_mapper in enumerate(
            service.objectmapper_set.filter(content_type__model="production"), 1
        ):
            self.stdout.write(
                "%d/%d %s\n" % (
                    mapper_index, mapper_count,
                    smart_str(new_mapper.content_object)
                )
            )
