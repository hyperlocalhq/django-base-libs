...
from ._import_to_ruhrbuehnen_base_xml import *


class ImportToRuhrbuehnenBaseJSON(ImportToRuhrbuehnenBaseXML):
    """
    Base command to extend to import productions and events to RuhrBuehnen in JSON format
    """
    ...
    def main(self):
        ...
        self._total_production_count = root_dict.get('meta',
                                                     {}).get('total_count', "")
    ...
    def save_file_description(self, path, d):
        ...
        file_description.author = (d.get('copyright', "") or
                                   text).replace("&copy;", "©")
    ...
    def parse_and_use_texts(self, d, instance):
        # below using (d.get(fieldname, "") or "") to ensure that it works with null/None values
        instance.description_de = (d.get('description_de', "") or
                                   "").replace("&nbsp;", " ")
        instance.description_en = (d.get('description_en', "") or
                                   "").replace("&nbsp;", " ")
        instance.teaser_de = (d.get('teaser_de', "") or
                              "").replace("&nbsp;", " ")
        instance.teaser_en = (d.get('teaser_en', "") or
                              "").replace("&nbsp;", " ")
        instance.work_info_de = (d.get('work_info_de', "") or
                                 "").replace("&nbsp;", " ")
        instance.work_info_en = (d.get('work_info_en', "") or
                                 "").replace("&nbsp;", " ")
        instance.contents_de = (d.get('contents_de', "") or
                                "").replace("&nbsp;", " ")
        instance.contents_en = (d.get('contents_en', "") or
                                "").replace("&nbsp;", " ")
        instance.press_text_de = (d.get('press_text_de', "") or
                                  "").replace("&nbsp;", " ")
        instance.press_text_en = (d.get('press_text_en', "") or
                                  "").replace("&nbsp;", " ")
        instance.credits_de = (d.get('credits_de', "") or
                               "").replace("&nbsp;", " ")
        instance.credits_en = (d.get('credits_en', "") or
                               "").replace("&nbsp;", " ")
        instance.concert_program_de = (d.get('concert_program_de', "") or
                                       "").replace("&nbsp;", " ")
        instance.concert_program_en = (d.get('concert_program_en', "") or
                                       "").replace("&nbsp;", " ")
        instance.supporting_program_de = (
            d.get('supporting_program_de', "") or ""
        ).replace("&nbsp;", " ")
        instance.supporting_program_en = (
            d.get('supporting_program_en', "") or ""
        ).replace("&nbsp;", " ")
        instance.remarks_de = (d.get('remarks_de', "") or
                               "").replace("&nbsp;", " ")
        instance.remarks_en = (d.get('remarks_en', "") or
                               "").replace("&nbsp;", " ")
        instance.duration_text_de = (d.get('duration_text_de', "") or
                                     "").replace("&nbsp;", " ")
        instance.duration_text_en = (d.get('duration_text_en', "") or
                                     "").replace("&nbsp;", " ")
        instance.subtitles_text_de = (d.get('subtitles_text_de', "") or
                                      "").replace("&nbsp;", " ")
        instance.subtitles_text_en = (d.get('subtitles_text_en', "") or
                                      "").replace("&nbsp;", " ")
        instance.age_text_de = (d.get('age_text_de', "") or
                                "").replace("&nbsp;", " ")
        instance.age_text_en = (d.get('age_text_en', "") or
                                "").replace("&nbsp;", " ")
        instance.price_information_de = (
            d.get('price_information_de', "") or ""
        ).replace("&nbsp;", " ")
        instance.price_information_en = (
            d.get('price_information_en', "") or ""
        ).replace("&nbsp;", " ")
    ...
    def save_page(self, productions):
        ...
        from ruhrbuehnen.apps.productions.models import EVENT_STATUS_CHOICES, TICKET_STATUS_CHOICES
        ...
        for prod_index, prod_dict in enumerate(productions, 1):
            ...
            title_de = (prod_dict.get('title_de') or "").replace('\n',
                                                                 ' ').strip()
            title_en = (prod_dict.get('title_en') or
                        "").replace('\n', ' ').strip() or title_de
            ...
            # Below using (prod_dict.get(fieldname, "") or "") to ensure that the value is not null/None
            prod.prefix_de = strip_tags(
                (prod_dict.get('prefix_de', "") or "").replace('<br />', ' ')
            )
            prod.prefix_en = strip_tags(
                (prod_dict.get('prefix_en', "") or "").replace('<br />', ' ')
            )
            prod.subtitle_de = strip_tags(
                (prod_dict.get('subtitle_de', "") or "").replace('<br />', ' ')
            )
            prod.subtitle_en = strip_tags(
                (prod_dict.get('subtitle_en', "") or "").replace('<br />', ' ')
            )
            prod.original_de = strip_tags(
                (prod_dict.get('original_de', "") or "").replace('<br />', ' ')
            )
            prod.original_en = strip_tags(
                (prod_dict.get('original_en', "") or "").replace('<br />', ' ')
            )
            ...
            for category_id in categories:
                if isinstance(
                    category_id, dict
                ):  # category_id might be either an id or a dictionary with 'category_id' key
            ...
            for ch_id in characteristics:
                if isinstance(
                    ch_id, dict
                ):  # ch_id might be either an id or a dictionary with 'characteristic_id' key
            ...
            for live_stream_dict in live_streams:
                ...
                ls.creation_date = parse_datetime(
                    live_stream_dict.get('creation_date', "")
                )
            ...
            if not self.skip_images:
                ...
                for image_dict in images:
                    ...
                    if image_response.status_code in (
                        ...
                        mf.copyright_restrictions = image_dict.get(
            ...
            for person_dict in leaders:
                try:
                    prefix = Prefix.objects.get(
                        slug=person_dict.get('prefix_id', "")
                    )
                except:
                    prefix = None
            ...
            for person_dict in participants:
                ...
                prod.productioninvolvement_set.create(
                    ...
                    involvement_instrument_de=person_dict.get(
                        'instrument_de', ""
                    ),
                    involvement_instrument_en=person_dict.get(
                        'instrument_en', ""
                    ),
                    imported_sort_order=imported_sort_order,
                )
            ...
            for event_dict in events:
                ...
                event.free_entrance = (
                    event_dict.get('free_entrance', "") == "true"
                )
                try:
                ...
                event.event_status = event_status if event_status in dict(
                    EVENT_STATUS_CHOICES
                ) else ""

                ticket_status = event_dict.get('ticket_status')
                event.ticket_status = ticket_status if ticket_status in dict(
                ...
                for live_stream_dict in live_streams:
                    ...
                    ls.modified_date = parse_datetime(
                        live_stream_dict.get('modified_date', "")
                    )
                    ls.title_de = live_stream_dict.get('title_de', "")
                    ls.title_en = live_stream_dict.get('title_en', "")
                    ls.link_or_embed = live_stream_dict.get('embed', "")
                    try:
                        ls.sort_order = int(
                            live_stream_dict.get('sort_order', "")
                        )
                    except:
                        ls.sort_order = 1
                    ls.save()
                ...
                for pdf_dict in pdfs:
                    ...
                    if pdf_response.status_code in (
                        ...
                        image_mods.FileManager.save_file_for_object(
                            ...
                            pdf_response.content,
                            field_name="path",
                            subpath="productions/{}/events/pdf/".format(
                                event.production.slug
                            ),
                        )
                        mf.save()
