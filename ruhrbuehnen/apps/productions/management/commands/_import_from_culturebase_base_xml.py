...
from ruhrbuehnen.apps.people.models import Person, AuthorshipType
...
class ImportFromCulturebaseBase(NoArgsCommand, ImportCommandMixin):
    ...
    def get_child_text(self, node, tag, **attrs):
        ...
        for child_node in node.findall(
            '%(prefix)s%(tag)s' % dict(tag=tag, **self.helper_dict)
        ):
    ...
    def get_updated_location_and_stage(self, venue_node):
        ...
        venue_title = convert_location_title(
            self.get_child_text(venue_node, 'Name')
        )
        ...
        if stage_settings:
            ...
            if venue_to_save_at_production:
                return LocationAndStage(
                    None, {
                        'title':
                            venue_to_save_at_production,
                        'street_address':
                            self.get_child_text(venue_node, 'Street'),
                        'postal_code':
                            self.get_child_text(venue_node, 'ZipCode'),
                        'city':
                            "Berlin",
                    }
                )
            ...
            if not location.street_address:
                ...
                location.street_address = self.get_child_text(
                    venue_node, 'Street'
                )
                location.postal_code = self.get_child_text(
                    venue_node, 'ZipCode'
                )
                location.city = city_suffix.sub(
                    '',
                    self.get_child_text(venue_node, 'City') or ""
                )
        ...
        if stage_settings:
            if stage_settings.should_create_stage_object:
                ...
                if not stage.street_address:
                    ...
                    stage.street_address = self.get_child_text(
                        venue_node, 'Street'
                    )
                    stage.postal_code = self.get_child_text(
                        venue_node, 'ZipCode'
                    )
                    stage.city = city_suffix.sub(
                        '',
                        self.get_child_text(venue_node, 'City') or ""
                    )
                ...
                return LocationAndStage(
                    location, {
                        'title':
                            stage_settings.internal_stage_title,
                        'street_address':
                            self.get_child_text(venue_node, 'Street'),
                        'postal_code':
                            self.get_child_text(venue_node, 'ZipCode'),
    ...
    def save_file_description(self, path, xml_node):
        ...
        file_description.title_de = self.get_child_text(
            xml_node, 'Title', Language="de"
        )
        file_description.title_en = self.get_child_text(
            xml_node, 'Title', Language="en"
        )
        ...
        file_description.author = self.get_child_text(
            xml_node, 'Copyright'
        ) or text or ""
    ...
    def parse_and_use_texts(self, xml_node, instance):
        ...
        for text_node in xml_node.findall(
            './%(prefix)sText' % self.helper_dict
        ):
            text_cat_id = int(
                text_node.find('%(prefix)sCategory' % self.helper_dict
                              ).get('Id')
            )
            text_de = self.cleanup_text(
                self.get_child_text(text_node, 'Value', Language="de")
            )
            text_en = self.cleanup_text(
                self.get_child_text(text_node, 'Value', Language="en")
            )
