...
class ImportFromHeimatBase(NoArgsCommand, ImportCommandMixin):
    ...
    def load_and_parse_locations(self):
        response = requests.get(
            "http://web2.heimat.de/cb-out/exports/address/address_id.php?city=berlin"
        )
    ...
    def get_updated_location_and_stage(self, venue_node):
        ...
        if venue_node.get('isId') == "1":
            ...
            return self.get_updated_location_and_stage_from_free_text(
                venue_node.text
            )
        ...
        stage_settings = STAGE_TO_LOCATION_MAPPER.get(
            culturebase_location.title.lower(), None
        )
        if stage_settings:
            try:
                location = Location.objects.get(
                    title_de=stage_settings.location_title
                )
            except Location.DoesNotExist:
                location = Location()
                location.title_de = location.title_en = stage_settings.location_title
        else:
            venue_to_save_at_production = PRODUCTION_VENUES.get(
                culturebase_location.title.lower(), ''
            )
            if venue_to_save_at_production:
                stage_dict = {
                    'title': venue_to_save_at_production,
                }
                culturebase_location = self.LOCATIONS_BY_TITLE.get(
                    venue_to_save_at_production, None
                )
                if culturebase_location:
                    stage_dict['street_address'
                              ] = culturebase_location.street_address
                    stage_dict['postal_code'] = culturebase_location.postal_code
                    stage_dict['city'] = u"Berlin"
                return LocationAndStage(None, stage_dict)
            try:
                location = Location.objects.get(
                    title_de=convert_location_title(culturebase_location.title)
                )
            except Location.DoesNotExist:
                location = Location()
                location.title_de = location.title_en = convert_location_title(
                    culturebase_location.title
                )
            ...
            if stage_settings.should_create_stage_object:
                try:
                    stage = Stage.objects.get(
                        location=location or self.in_program_of,
                        title_de=stage_settings.internal_stage_title
                    )
                    ...
                    self.stderr.write(
                        "Error: Multiple stages under {location} (ID={location_id}) with title {stage_title}"
                        .format(
                            location=loc.title,
                            location_id=loc.pk,
    ...
    def get_updated_location_and_stage_from_free_text(self, free_text_venue):
        ...
        if stage_settings:
            if stage_settings.should_create_stage_object:
                ...
                if not stage.street_address:
                    culturebase_location = self.LOCATIONS_BY_TITLE.get(
                        stage_settings.internal_stage_title, None
                    )
                    if culturebase_location:
                        stage.street_address = culturebase_location.street_address
                        stage.postal_code = culturebase_location.postal_code
    ...
    def save_file_description(self, path, xml_node):
        ...
        file_description.title_de = self.get_child_text(
            xml_node, 'title', languageId="1"
        ) or self.get_child_text(
            xml_node, 'text', languageId="1"
        )
        file_description.title_en = self.get_child_text(
            xml_node, 'title', languageId="2"
        ) or self.get_child_text(
    ...
    def parse_and_use_texts(self, xml_node, instance):
        ...
        for text_node in xml_node.findall('./mediaText'):
            ...
            text_de = self.cleanup_text(
                self.get_child_text(text_node, 'text', languageId="1")
            )
            text_en = self.cleanup_text(
                self.get_child_text(text_node, 'text', languageId="2")
            )
        ...
        if pressetext_de or kritik_de:
            instance.press_text_de = u"\n".join(
                [text for text in (pressetext_de, kritik_de) if text]
            )
            instance.press_text_de_markup_type = 'pt'
        instance.press_text_en = u""
        if pressetext_en or kritik_en:
            instance.press_text_en = u"\n".join(
                [text for text in (pressetext_en, kritik_en) if text]
            )
        ...
        if inhaltsangabe_de or programbuch_de:
            instance.contents_de = u"\n".join(
                [text for text in (inhaltsangabe_de, programbuch_de) if text]
            )
            instance.contents_de_markup_type = 'pt'
        instance.contents_en = u""
        if inhaltsangabe_en or programbuch_en:
            instance.contents_en = u"\n".join(
                [text for text in (inhaltsangabe_en, programbuch_en) if text]
            )
        ...
        if teaser_de or description_de:
            instance.description_de = u"\n".join(
                [text for text in (teaser_de, description_de) if text]
            )
        elif werkinfo_kurz_de:
        ...
        if teaser_en or description_en:
            instance.description_en = u"\n".join(
                [text for text in (teaser_en, description_en) if text]
            )
        elif werkinfo_kurz_en:
        ...
        instance.work_info_de = u"\n".join(
            [
                text for text in (
                    werkinfo_kurz_de, werkinfo_gesamt_de,
        ...
        instance.work_info_en = u"\n".join(
            [
                text for text in (
                    werkinfo_kurz_en, werkinfo_gesamt_en,
        ...
        if language_and_subtitles_node is not None:
            ...
            try:
                instance.language_and_subtitles = LanguageAndSubtitles.objects.filter(
                    pk=language_and_subtitles_id
                )[0]
                instance.subtitles_text_de = ""
                instance.subtitles_text_en = ""
            except IndexError:
                if not instance.subtitles_text_de:
                    instance.subtitles_text_de = self.get_child_text(
                        xml_node, 'language_and_subtitles'
                    )
    ...
    def save_page(self, root_node):
        ...
        for prod_index, prod_node in enumerate(prod_nodes, 1):
            ...
            title_de = self.get_child_text(
                prod_node, 'title', languageId="1"
            ).replace('\n', ' ').strip()
            title_en = self.get_child_text(
                prod_node, 'title', languageId="2"
            ).replace('\n', ' ').strip()
            ...
            for person_node in prod_node.findall('person'):
                role_de = self.get_child_text(
                    person_node, 'mediaText/text', languageId="1"
                )
                role_en = self.get_child_text(
                    person_node, 'mediaText/text', languageId="2"
                )
                if not role_de and int(person_node.get('roleId')
                                      ) in self.ROLE_ID_MAPPER:
                    role_de, role_en = self.ROLE_ID_MAPPER[int(
                        person_node.get('roleId')
                    )]
                for person_name in re.split(
                    r'\s*[/,]\s*', person_node.get('personFreetext')
                ):
            ...
            for sponsor_node in prod_node.findall('./sponsor'):
                sponsor = ProductionSponsor(
                    production=prod,
                    title_de=self.get_child_text(
                        sponsor_node, 'title', languageId="1"
                    ),
                    title_en=self.get_child_text(
                        sponsor_node, 'title', languageId="2"
                    ),
            ...
            for event_node in prod_node.findall('event'):
                ...
                if not external_event_id:
                    external_event_id = u"%s_%s" % (
                        external_prod_id, event_node.get('datetime')
                    )
                ...
                if re.search(r"^\d\d\d\d", start_datetime_string):
                    ...
                    start_datetime = parse_datetime(
                        start_datetime_string, dayfirst=True
                    )
                ...
                if price_node is not None:
                    price_from = (price_node.get('minPrice') or
                                  u"").replace(',', '.') or None
                    ...
                    price_till = (price_node.get('maxPrice') or
                                  u"").replace(',', '.') or None
    ...
    def should_reimport(self):
        ...
        mappers = self.service.objectmapper_set.filter(
