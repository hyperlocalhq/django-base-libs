...
from ruhrbuehnen.apps.sponsors.models import SponsorBase
...
class ProductionManager(models.Manager):
    def accessible_to(self, user):
        ...
        owned_production_ids = map(
            int,
            self.owned_by(user=user).values_list("pk", flat=True)
        )
    ...
    def for_newsletter(self):
        return self.filter(
            status="published", newsletter=True
        ).order_by('start_date', 'start_time')


class Production(CreationModificationMixin, UrlMixin, SlugMixin()):
    prefix = MultilingualCharField(
        _("Title prefix"), max_length=255, blank=True
    )
    title = MultilingualCharField(_("Title"), max_length=255, db_index=True)
    subtitle = MultilingualCharField(_("Subtitle"), max_length=255, blank=True)
    original = MultilingualCharField(
        _("Original title"), max_length=255, blank=True
    )
    website = MultilingualURLField(
        _("Production URL"), blank=True, max_length=255
    )

    in_program_of = models.ManyToManyField(
        "locations.Location",
        verbose_name=_("In the programme of"),
        blank=True,
        related_name="program_productions"
    )
    play_locations = models.ManyToManyField(
        "locations.Location",
        verbose_name=_("Theaters"),
        blank=True,
        related_name="located_productions"
    )
    play_stages = models.ManyToManyField(
        "locations.Stage", verbose_name=_("Stages"), blank=True
    )
    ...
    in_cooperation_with = models.CharField(
        _("In cooperation with"), blank=True, max_length=255
    )
    ...
    city = models.CharField(
        _("City"), default="Berlin", max_length=255, blank=True
    )
    ...
    categories = TreeManyToManyField(
        ProductionCategory, verbose_name=_("Categories"), blank=True
    )
    ...
    supporting_program = MultilingualTextField(
        _("Supporting program"), blank=True
    )
    remarks = MultilingualTextField(_("Remarks"), blank=True)
    duration_text = MultilingualCharField(
        _("Duration text"), max_length=255, blank=True
    )
    subtitles_text = MultilingualCharField(
        _("Subtitles text"), max_length=255, blank=True
    )
    age_text = MultilingualCharField(_("Age text"), max_length=255, blank=True)

    festivals = models.ManyToManyField(
        "festivals.Festival", verbose_name=_("Festivals"), blank=True
    )
    language_and_subtitles = models.ForeignKey(
        LanguageAndSubtitles,
        verbose_name=_("Language / Subtitles"),
        blank=True,
        null=True
    )
    related_productions = models.ManyToManyField(
        "self", verbose_name=_("Related productions"), blank=True
    )

    free_entrance = models.BooleanField(_("Free entrance"), default=False)
    price_from = models.DecimalField(
        _(u"Price from (€). Separate cents by a point."),
        max_digits=5,
        decimal_places=2,
        blank=True,
        null=True
    )
    price_till = models.DecimalField(
        _(u"Price till (€). Separate cents by a point."),
        max_digits=5,
        decimal_places=2,
        blank=True,
        null=True
    ...
    price_information = MultilingualTextField(
        _("Additional price information"), blank=True
    )
    ...
    other_characteristics = MultilingualTextField(
        _("Other characteristics"), blank=True
    )
    age_from = models.PositiveSmallIntegerField(
        _(u"Age from"), blank=True, null=True
    )
    age_till = models.PositiveSmallIntegerField(
        _(u"Age till"), blank=True, null=True
    )
    edu_offer_website = URLField(
        _("Educational offer website"), blank=True, max_length=255
    )

    show_among_others = models.BooleanField(
        _("Show among others"),
        default=True,
        help_text=_(
            "Should this production be shown in event details among other productions at the same venue?"
        )
    )
    no_overwriting = models.BooleanField(
        _("Do not overwrite by the next import"), default=False
    )
    ...
    start_date = models.DateField(
        _("Actual start date"), blank=True, null=True, editable=False
    )
    start_time = models.TimeField(
        _("Actual start time"), blank=True, null=True, editable=False
    )

    import_source = models.ForeignKey(
        'external_services.Service', editable=False, blank=True, null=True
    )
    ...
    def update_actual_date_and_time(self):
        ...
        if event:
            ...
            now = tz_now()
            if self.status != "expired" and (
                (event.end_date is None and event.start_date < now.date()) or
        ...
        if make_expired:
            self.status = "expired"
            Production.objects.filter(pk=self.pk).update(status="expired", )
    ...
    def get_nearest_occurrence(self, timestamp=tz_now):
        ...
        if not event_times:
            event_times = self.event_set.exclude(event_status="trashed"
                                                ).order_by("-end_date")
    ...
    def get_past_occurrences(self, timestamp=tz_now):
        if callable(timestamp):
            timestamp = timestamp()
        return self.event_set.exclude(event_status="trashed"
                                     ).filter(end_date__lt=timestamp.date())
    ...
    def get_future_occurrences(self, timestamp=tz_now):
        ...
        events = list(
            self.event_set.exclude(event_status="trashed"
                                  ).filter(start_date__gte=now_date
                                          ).order_by('start_date')
        )

        while len(events) and events[
            0].start_date == now_date and events[0].start_time < now_time:
            del events[0]
    ...
    def get_categories(self):
        ...
        categories = ProductionCategory.objects.filter(
            production__pk__in=[self.pk] +
            [part.production_id for part in self.get_parts()]
        )
        return categories
    ...
    def get_import_source(self):
        if not hasattr(self, "_import_source"):
            ...
            self._import_source = ""
            if mappers:
                self._import_source = mappers[0].service.title.replace(
                    ' Productions', ''
                )
    ...
    def duplicate(self, new_values=None):
        ...
        target_prod.slug = get_unique_value(
            Production, better_slugify(target_prod.title_de)
        )
        ...
        abs_source_media_path = os.path.join(
            settings.MEDIA_ROOT, source_media_dir
        )
        abs_target_media_path = os.path.join(
            settings.MEDIA_ROOT, target_media_dir
        )
        if os.path.exists(abs_source_media_path):
            copy_tree(abs_source_media_path, abs_target_media_path)
        for file_desc in FileDescription.objects.filter(
            file_path__startswith=source_media_dir
        ):
            file_desc.pk = None
            file_desc.file_path = file_desc.file_path.path.replace(
                source_media_dir, target_media_dir
            )
            file_desc.save()
        ...
        for image in source_prod.productionimage_set.all():
            image.pk = None
            image.path = image.path.path.replace(
                source_media_dir, target_media_dir
            )
    ...
    def is_editable(self, user=None):
        ...
        if not hasattr(self, "_is_editable_cache"):
            ...
            self._is_editable_cache = user.has_perm(
                "productions.change_production", self
            ) or any(
                (
                    location.is_editable() for locations in
                    (self.in_program_of.all(), self.play_locations.all())
...
class ProductionSocialMediaChannel(models.Model):
    ...
    channel_type = models.CharField(
        _("Social media type"),
        max_length=255,
        help_text=_("e.g. twitter, facebook, etc.")
    )
...
class ProductionImage(CreationModificationDateMixin):
    ...
    copyright_restrictions = models.CharField(
        _('Copyright restrictions'),
        max_length=20,
        blank=True,
...
class ProductionPDF(CreationModificationDateMixin):
    ...
    path = FileBrowseField(
        ...
        directory="productions/",
        extensions=['.pdf'],
        help_text=_("A path to a locally stored PDF file.")
    )
    sort_order = PositionField(_("Sort order"), collection="production")
...
class ProductionLeadership(CreationModificationDateMixin):
    ...
    person = models.ForeignKey(
        'people.Person', verbose_name=_("Person"), blank=True, null=True
    )
    ...
    sort_order = models.IntegerField(_("Sort order"), default=0)
    imported_sort_order = models.IntegerField(
        _("Imported sort order"), default=0
    )
...
class ProductionAuthorship(CreationModificationDateMixin):
    ...
    authorship_type = models.ForeignKey(
        'people.AuthorshipType', verbose_name=_('Type')
    )
...
class ProductionInvolvement(CreationModificationDateMixin):
    ...
    involvement_type = models.ForeignKey(
        'people.InvolvementType', verbose_name=_('Type'), blank=True, null=True
    )
    another_type = MultilingualCharField(
        _("Another type"), max_length=255, blank=True
    )
    involvement_role = MultilingualCharField(
        _('Role'), max_length=255, blank=True
    )
    involvement_instrument = MultilingualCharField(
        _('Instrument'), max_length=255, blank=True
    )
    #sort_order = PositionField(_("Sort order"), collection="production", default=0)
    sort_order = models.IntegerField(_("Sort order"), default=0)
    imported_sort_order = models.IntegerField(
    ...
    class Meta:
        index_together = [['production', 'sort_order']]
        ordering = ["sort_order"]
    ...
    def get_function(self):
        ...
        return getattr(self, 'another_type_%s' % lang_code, '') or getattr(
            self, 'involvement_role_%s' % lang_code, ''
        ) or getattr(self, 'involvement_instrument_%s' % lang_code, '')


class EventCharacteristics(CreationModificationDateMixin, SlugMixin()):
    title = MultilingualCharField(_('Title'), max_length=200)
    sort_order = models.IntegerField(_("Sort Order"), default=0)
    show_as_main_category = models.BooleanField(
        _("Show as main category"), default=False
    )
...
class Event(CreationModificationMixin, UrlMixin):
    ...
    play_locations = models.ManyToManyField(
        "locations.Location", verbose_name=_("Theaters"), blank=True
    )
    play_stages = models.ManyToManyField(
        "locations.Stage", verbose_name=_("Stages"), blank=True
    )
    ...
    characteristics = models.ManyToManyField(
        EventCharacteristics, verbose_name=_("Characteristics"), blank=True
    )
    other_characteristics = MultilingualTextField(
        _("Other characteristics"), blank=True
    )
    ...
    def __unicode__(self):
        return unicode(self.production
    ...
    def ev_or_prod_description(self):
        ...
        ) or self.production.get_rendered_description()

    def ev_or_prod_teaser(self):
        return self.get_rendered_teaser(
        ) or self.production.get_rendered_teaser()

    def ev_or_prod_work_info(self):
        return self.get_rendered_work_info(
        ) or self.production.get_rendered_work_info()

    def ev_or_prod_contents(self):
        return self.get_rendered_contents(
        ) or self.production.get_rendered_contents()

    def ev_or_prod_press_text(self):
        return self.get_rendered_press_text(
        ) or self.production.get_rendered_press_text()

    def ev_or_prod_credits(self):
        return self.get_rendered_credits(
        ) or self.production.get_rendered_credits()

    ### Culturebase-specific fields ###

    def ev_or_prod_concert_program(self):
        return self.get_rendered_concert_program(
        ) or self.production.get_rendered_concert_program()

    def ev_or_prod_supporting_program(self):
        return self.get_rendered_supporting_program(
        ) or self.production.get_rendered_supporting_program()

    def ev_or_prod_remarks(self):
        return self.get_rendered_remarks(
        ) or self.production.get_rendered_remarks()
    ...
    def ev_or_prod_price_information(self):
        return self.get_rendered_price_information(
        ) or self.production.get_rendered_price_information()
    ...
    def get_festivals(self):
        return self.production.festivals.filter(
            status__in=("published", "not_listed", "expired"),
...
class EventInvolvement(CreationModificationDateMixin):
    ...
    class Meta:
        index_together = [['event', 'sort_order']]
        ordering = ["sort_order"]
        verbose_name = _("Involvement")
        verbose_name_plural = _("Involvements")
...
class ProductionSponsor(SponsorBase):
    production = models.ForeignKey(
        Production, verbose_name=_("Production"), on_delete=models.CASCADE
    )
