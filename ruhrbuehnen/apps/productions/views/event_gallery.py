...
from ruhrbuehnen.apps.productions.models import Event, EventVideo, EventLiveStream, EventImage, EventPDF
...
def update_video_ordering(tokens, event):
    ...
    for mediafile_token in tokens.split(u","):
        mediafile = get_object_or_404(
            EventVideo, event=event, pk=EventVideo.token_to_pk(mediafile_token)
        )
...
def video_overview(request, slug, event_id):
    ...
    return render(
        request, "productions/events/gallery/video_overview.html", {
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = EventVideo(event=event)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "production_image_overview", slug=production.slug
                    )
    else:
        if media_file_obj:
            # existing media file
...
def streaming_overview(request, slug, event_id):
    ...
    return render(
        request, "productions/events/gallery/streaming_overview.html", {
            'production': production,
            'event': event,
        }
    )
...
@never_cache
@login_required
def create_update_streaming(
    request, slug, event_id, mediafile_token="", **kwargs
):
    production = get_object_or_404(Production, slug=slug)
    event = get_object_or_404(Event, pk=event_id, production=production)
    if not production.is_editable():
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = EventLiveStream(event=event)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "production_gallery_overview", slug=production.slug
                    )
    else:
        if media_file_obj:
            # existing media file
...
def update_image_ordering(tokens, event):
    ...
    for mediafile_token in tokens.split(u","):
        mediafile = get_object_or_404(
            EventImage, event=event, pk=EventImage.token_to_pk(mediafile_token)
        )
        mediafiles.append(mediafile)
    sort_order = 0
    for mediafile in mediafiles:
...
def image_overview(request, slug, event_id):
    ...
    return render(
        request, "productions/events/gallery/image_overview.html", {
            'production': production,
            'event': event
        }
    )
...
def create_update_image(request, slug, event_id, mediafile_token="", **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = EventImage(event=event)
            media_file_obj.copyright_restrictions = cleaned[
            ...
            try:
                ...
                file_description = FileDescription(
                    file_path=media_file_path or path
                )

            for lang_code, lang_name in FRONTEND_LANGUAGES:
                setattr(
                    file_description, 'title_%s' % lang_code,
                    cleaned['title_%s' % lang_code]
                )
                setattr(
                    file_description, 'description_%s' % lang_code,
...
def update_pdf_ordering(tokens, event):
    ...
    for mediafile_token in tokens.split(u","):
        mediafile = get_object_or_404(
            EventPDF, event=event, pk=EventPDF.token_to_pk(mediafile_token)
        )
        mediafiles.append(mediafile)
    sort_order = 0
    for mediafile in mediafiles:
...
def pdf_overview(request, slug, event_id):
    ...
    return render(
        request, "productions/events/gallery/pdf_overview.html", {
            'production': production,
            'event': event,
        }
    )
...
def create_update_pdf(request, slug, event_id, mediafile_token="", **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = EventPDF(event=event)
