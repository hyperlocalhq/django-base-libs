...
class ProductionVideoInline(ExtendedStackedInline):
    ...
    fieldsets += [
        (None, {
            'fields': ('link_or_embed', 'sort_order')
        }),
    ]
    inline_classes = ('grp-collapse grp-open', )
...
class ProductionLeadershipInline(ExtendedStackedInline):
    ...
    fieldsets = [
        (None, {
            'fields': ('person', 'sort_order', 'imported_sort_order')
        }),
    ]
    fieldsets += get_admin_lang_section(_("Function"), [
        'function',
    ])
...
class ProductionInvolvementInline(ExtendedStackedInline):
    ...
    fieldsets += get_admin_lang_section(
        _("Role / Instrument"), ['involvement_role', 'involvement_instrument']
    )
    raw_id_fields = ['person']
    inline_classes = ('grp-collapse grp-open', )
...
class ProductionAdmin(ExtendedModelAdmin):
    list_display = (
        'title_de', 'get_locations', 'get_import_source', 'get_owners_list',
        'creation_date', 'modified_date', 'show_among_others', 'no_overwriting',
        'newsletter', 'status'
    )
    list_editable = (
        'show_among_others', 'no_overwriting', 'newsletter', 'status'
    )
    search_fields = ('title_de', 'title_en')
    list_filter = [
        'show_among_others', 'no_overwriting', "creation_date", "modified_date",
        'import_source', 'newsletter', 'status'
    ]
    date_hierarchy = "modified_date"

    fieldsets = get_admin_lang_section(
        _("Title"), ['title', 'prefix', 'subtitle', 'original', 'website']
    )
    fieldsets += [
        (None, {
            'fields': ('slug', )
    ...
    fieldsets += [
        (
            _("Location"), {
                'fields':
                    [
                        'in_program_of', 'ensembles', 'play_locations',
                        'play_stages', 'organizers', 'in_cooperation_with'
                    ]
            }
        ),
    ]
    fieldsets += [
        (
            _("Free Location"), {
                'fields':
                    [
                        'location_title', 'street_address', 'street_address2',
    ...
    fieldsets += [
        (
            _("Relations"), {
                'fields': ['categories', 'festivals', 'related_productions']
            }
        ),
    ]
    fieldsets += get_admin_lang_section(
        _("Description"), [
            'description', 'teaser', 'work_info', 'contents', 'press_text',
            'credits'
        ]
    )
    fieldsets += get_admin_lang_section(
        _("Imported"), [
            'concert_program', 'supporting_program', 'remarks', 'duration_text',
            'subtitles_text', 'age_text'
        ]
    )
    fieldsets += [
        (
            _("Prices"), {
                'fields':
                    [
                        'price_from', 'price_till', 'free_entrance',
                        'tickets_website',
                        get_admin_lang_section(
                            _("Price information"), ['price_information']
                        )
                    ]
            }
        ),
    ...
    fieldsets += [
        (
            _("Additional details"), {
                'fields':
                    [
                        'language_and_subtitles', 'characteristics',
                        get_admin_lang_section(
    ...
    fieldsets += [
        (
            _("Additional Information"), {
                'fields':
                    ['get_owners_list', 'get_editors_list', 'get_external_id'],
                'description':
                    _(
                        "Owners are users who have permissions to modify this production directly. Editors are owners of this production or of the related theaters."
                    )
            }
        ),
    ]
    ...
    filter_horizontal = [
        'in_program_of', 'play_locations', 'play_stages', 'categories',
        'characteristics'
    ]
    ...
    inlines = [
        ...
        ProductionLiveStreamInline,
        ProductionImageInline,
        ProductionPDFInline,
        ProductionLeadershipInline,
        ProductionAuthorshipInline,
        ProductionInvolvementInline,
    ...
    def queryset(self, request):
        ...
        qs = qs.prefetch_related(
            "in_program_of", "play_locations", "play_stages"
        )
        return qs
    ...
    def get_locations(self, obj):
        ...
        if obj.in_program_of.exists():
            html.append(
                ugettext('In program of') + ': ' + ', '.join(
                    (
                        u'<a href="/admin/locations/location/{}/" target="_blank">{}</a>'
                        .format(loc.pk, loc) for loc in obj.in_program_of.all()
                    )
                )
            )
        if obj.play_locations.exists():
            html.append(
                ugettext('Theaters') + ': ' + ', '.join(
                    (
                        u'<a href="/admin/locations/location/{}/" target="_blank">{}</a>'
                        .format(loc.pk, loc) for loc in obj.play_locations.all()
                    )
                )
            )
        if obj.play_stages.exists():
            html.append(
                ugettext('Stages') + ': ' + ', '.join(
                    (
                        u'<a href="/admin/locations/location/{}/" target="_blank">{}</a>'
                        .format(stage.location.pk, stage.location)
    ...
    def get_owners_list(self, obj):
        owners_list = set()
        manage_owners_link = '<a href="/admin/productions/production/%s/owners/"><span>%s</span></a>' % (
    ...
    def owners_view(self, request, production_id):
        ...
        if request.method == "POST":
            ...
            form = OwnersForm(initial={'users': production.get_owners()})
        ...
        return render(
            request, 'admin/owners.html', {
                ...
                'original':
                    production,
                'app_label':
                    Production._meta.app_label,
                'opts':
                    Production._meta,
                'form':
                    form,
                'title':
                    ugettext('The owners of %(production)s') % {
...
class EventCharacteristicsAdmin(ExtendedModelAdmin):
    ...
    fieldsets += [
        (None, {
            'fields': ('slug', 'sort_order', 'show_as_main_category')
        }),
    ]
...
class EventAdmin(ExtendedModelAdmin):
    ...
    fieldsets += [
        (
            _("Location"), {
                'fields': ['play_locations', 'play_stages', 'organizers']
            }
        ),
    ]
    fieldsets += [
    ...
    inlines = [
        ...
        EventVideoInline,
        EventLiveStreamInline,
        EventImageInline,
        EventPDFInline,
        EventLeadershipInline,
        EventAuthorshipInline,
        EventInvolvementInline,
        EventSponsorInline,
    ]
