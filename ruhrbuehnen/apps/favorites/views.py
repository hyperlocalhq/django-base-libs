...
def user_favorites(request, user_token, **kwargs):
        ...
        (
            (
                ...
                Location.objects.filter(
                    id__in=location_ids, status="published"
                )
            ),
            (
                'events',
                Event.objects.filter(
                    id__in=event_ids, production__status="published"
                )
            ),
            (
                'festivals',
                Festival.objects.filter(
                    id__in=festival_ids, status="published"
                )
            ),
            (
                'departments',
                Department.objects.filter(
                    id__in=department_ids, status="published"
                )
            ),
            (
                'projects',
                Project.objects.filter(id__in=project_ids, status="published")
            ),
        )
    )
    ...
    if latitude and longitude:
        ...
        favorites['locations'] = favorites['locations'].annotate(
            title_uni=models.Case(
                models.When(
                    then=models.Value('title_de'),
                    **{request_specific_field_name: ''}
                ),
...
class FavoritesByEmailForm(forms.Form):
    ...
    recipient_name = forms.CharField(label=_("Your friend's name"))
    recipient_email = forms.EmailField(label=_("Your friend's email"))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = layout.Layout(
            layout.Fieldset(
                ...
                "recipient_email",
                "message",
            ), bootstrap.FormActions(PrimarySubmit('submit', _('Send')), )
        )
...
def send_user_favorites_by_email(request, user_token, **kwargs):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            sender_name, sender_email = cleaned['sender_name'], cleaned[
                'sender_email']  # settings.MANAGERS[0]
            user_favorites_view = get_website_url(
                reverse('user_favorites', kwargs={'user_token': user_token})
            )
            send_email_using_template(
                recipients_list=[
                    Recipient(
                        name=cleaned['recipient_name'],
                        email=cleaned['recipient_email']
                    )
                ],
                ...
                obj_placeholders={
                    ...
                    'object_creator_title':
                        '%s (%s)' %
                        (cleaned['sender_name'], cleaned['sender_email']),
                    'object_description':
                        cleaned['message'],
                },
            ...
            return render(
                request, 'favorites/send_user_favorites_by_email_done.html'
            )
    else:
        form = FavoritesByEmailForm(
...
class FavoriteListOptionsForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = layout.Layout(
            ...
            ), bootstrap.FormActions(PrimarySubmit('submit', _('Save')), )
        )
...
def change_favorite_list(request, user_token, **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            opts.save()
            return render(
                request, "favorites/change_favorite_list_done.html", {}
            )
    ...
    return render(
        request, "favorites/change_favorite_list.html", {
            'form': form,
        }
    )
