...
def latest_tweets(request, twitter_username, number_of_tweets):
    ...
    for tweet in tweets:
        ...
        for word in re.split(re.compile(r'\s+'), tweet['text_urlized']):
            ...
            if match:
                word = '<a href="http://twitter.com/' + match.group(
                    1
                ) + '">@' + match.group(1) + '</a>' + match.group(2)
            tmp.append(word)
        tweet['text_urlized_atlinked'] = ' '.join(tmp)
        tweet['created_at_formatted'] = dateformat.format(
            datetime_parse(tweet['created_at']), 'd.m.Y H:i'
        )
        tweet['created_timesince'] = ugettext("%s ago") % timesince(
            datetime_parse(tweet['created_at'])
        )
