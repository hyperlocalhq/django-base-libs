...
from ruhrbuehnen.apps.advertising.managers import AdManager
...
class AdBase(CreationModificationMixin, UrlMixin):
    """
    ...
    show_ad_label = models.BooleanField(
        _('Show label "Advertisement"'), default=True
    )

    impressions_stats = models.TextField(
        verbose_name=_("Impressions"), blank=True, editable=False
    )
    clicks_stats = models.TextField(
        verbose_name=_("CLicks"), blank=True, editable=False
    )
    ...
    def collect_impressions(self):
        ...
        impressions_by_month = [
            date_filter(current_month_minus_2, "F") + (
                ": <strong>%d</strong>" % self.adimpression_set.filter(
    ...
    def collect_clicks(self):
        ...
        clicks_by_month = [
            date_filter(current_month_minus_2, "F") + (
                ": <strong>%d</strong>" % self.adclick_set.filter(
