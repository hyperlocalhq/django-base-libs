...
from ruhrbuehnen.apps.advertising.models import *
...
class AdBaseAdmin(ExtendedModelAdmin):
    list_display = [
        'title', 'url', 'advertiser', 'zone', 'category', 'show_ad_label',
        'start_showing', 'stop_showing', 'get_impressions', 'get_clicks'
    ]
    list_filter = [
...
class TextAdAdmin(AdBaseAdmin):
    ...
    fieldsets = [
        (
            _(u'Main data'), {
                'fields': ('title', 'url', 'content', 'show_ad_label')
            }
        ),
        (
            _(u'Categories'), {
