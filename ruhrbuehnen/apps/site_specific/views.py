...
def dashboard(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_locations = Location.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(locations_location.modified_date, locations_location.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_productions = Production.objects.accessible_to(
            request.user
        ).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(productions_production.modified_date, productions_production.creation_date)'
            }
        ).filter(
            status__in=(
                'published', 'draft', 'expired', 'not_listed', 'import'
            )
        ).order_by("-modified_or_creation_date")[:3]
        owned_multiparts = Parent.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(multiparts_parent.modified_date, multiparts_parent.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_festivals = Festival.objects.accessible_to(request.user).filter(
            status__in=(
                'published', 'draft', 'expired', 'not_listed', 'import'
            )
        ).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(festivals_festival.modified_date, festivals_festival.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_job_offers = JobOffer.objects.accessible_to(request.user).filter(
            status__in=(
                'published', 'draft', 'expired', 'not_listed', 'import'
            )
        ).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(marketplace_joboffer.modified_date, marketplace_joboffer.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_educational_departments = EducationalDepartment.objects.accessible_to(
            request.user
        ).filter(
            status__in=(
                'published', 'draft', 'expired', 'not_listed', 'import'
            ...
            select={
                'modified_or_creation_date':
                    'COALESCE(education_department.modified_date, education_department.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_educational_projects = EducationalProjects.objects.accessible_to(
            request.user
        ).filter(
            status__in=(
                'published', 'draft', 'expired', 'not_listed', 'import'
            ...
            select={
                'modified_or_creation_date':
                    'COALESCE(education_project.modified_date, education_project.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
    else:
        owned_locations = Location.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'IFNULL(locations_location.modified_date, locations_location.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_productions = Production.objects.accessible_to(
            request.user
            ...
            select={
                'modified_or_creation_date':
                    'IFNULL(productions_production.modified_date, productions_production.creation_date)'
            }
        ).filter(
            status__in=(
                'published', 'draft', 'expired', 'not_listed', 'import'
        ...
        owned_multiparts = Parent.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'IFNULL(multiparts_parent.modified_date, multiparts_parent.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_festivals = Festival.objects.accessible_to(request.user).filter(
            status__in=(
            ...
            select={
                'modified_or_creation_date':
                    'IFNULL(festivals_festival.modified_date, festivals_festival.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_job_offers = JobOffer.objects.accessible_to(request.user).filter(
            status__in=(
            ...
            select={
                'modified_or_creation_date':
                    'IFNULL(marketplace_joboffer.modified_date, marketplace_joboffer.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_educational_departments = EducationalDepartment.objects.accessible_to(
            request.user
            ...
            select={
                'modified_or_creation_date':
                    'IFNULL(education_department.modified_date, education_department.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
        owned_educational_projects = EducationalProjects.objects.accessible_to(
            request.user
            ...
            select={
                'modified_or_creation_date':
                    'IFNULL(education_project.modified_date, education_project.creation_date)'
            }
        ).order_by("-modified_or_creation_date")[:3]
...
def dashboard_locations(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_location_qs = Location.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(locations_location.modified_date, locations_location.creation_date)'
            }
    ...
    if q:
        owned_location_qs = owned_location_qs.filter(
            ...
            models.Q(subtitle_de__icontains=q) |
            models.Q(subtitle_en__icontains=q)
        )
    ...
    if sort_by == 'title':
        ...
        owned_location_qs = owned_location_qs.order_by(
            "modified_or_creation_date"
        )
    else:
        owned_location_qs = owned_location_qs.order_by(
...
def dashboard_productions(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_production_qs = Production.objects.accessible_to(
            request.user
        ).extra(
            select={
                'modified_or_creation_date':
    ...
    if q:
        owned_production_qs = owned_production_qs.filter(
            models.Q(title_de__icontains=q) | models.Q(title_en__icontains=q) |
            models.Q(prefix_de__icontains=q) |
            models.Q(prefix_en__icontains=q) |
            models.Q(subtitle_de__icontains=q) |
            models.Q(subtitle_en__icontains=q) |
            models.Q(original_de__icontains=q) |
            models.Q(original_en__icontains=q)
        )
    ...
    if sort_by == 'title':
        ...
        owned_production_qs = owned_production_qs.order_by(
            "modified_or_creation_date"
        )
    else:
        owned_production_qs = owned_production_qs.order_by(
...
def dashboard_festivals(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_festival_qs = Festival.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(festivals_festival.modified_date, festivals_festival.creation_date)'
            }
    ...
    if sort_by == 'title':
        ...
        owned_festival_qs = owned_festival_qs.order_by(
            "modified_or_creation_date"
        )
    else:
        owned_festival_qs = owned_festival_qs.order_by(
...
def dashboard_multiparts(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_multipart_qs = Parent.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(multiparts_parent.modified_date, multiparts_parent.creation_date)'
            }
    ...
    if status in ('published', 'draft', 'expired', 'not_listed'):
        owned_multipart_qs = owned_multipart_qs.filter(
            production__status=status
        )

    q = request.REQUEST.get('q', '')
    if q:
        owned_multipart_qs = owned_multipart_qs.filter(
            models.Q(production__title_de__icontains=q) |
            models.Q(production__title_en__icontains=q) |
            models.Q(production__prefix_de__icontains=q) |
            models.Q(production__prefix_en__icontains=q) |
            models.Q(production__subtitle_de__icontains=q) |
            models.Q(production__subtitle_en__icontains=q) |
            models.Q(production__original_de__icontains=q) |
            models.Q(production__original_en__icontains=q)
        )
    ...
    if sort_by == 'title':
        ...
        owned_multipart_qs = owned_multipart_qs.order_by(
            "-production__title_de"
        )
    elif sort_by == '-date':
        owned_multipart_qs = owned_multipart_qs.order_by(
            "modified_or_creation_date"
...
def dashboard_job_offers(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_job_offer_qs = JobOffer.objects.accessible_to(request.user).extra(
            select={
                'modified_or_creation_date':
                    'COALESCE(marketplace_joboffer.modified_date, marketplace_joboffer.creation_date)'
            }
    ...
    if sort_by == 'title':
        ...
        owned_job_offer_qs = owned_job_offer_qs.order_by(
            "modified_or_creation_date"
        )
    else:
        owned_job_offer_qs = owned_job_offer_qs.order_by(
...
def dashboard_educational_department(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_department_qs = EducationalDepartment.objects.accessible_to(
            request.user
        ).extra(
            select={
                'modified_or_creation_date':
    ...
    if sort_by == 'title':
        ...
        owned_department_qs = owned_department_qs.order_by(
            "modified_or_creation_date"
        )
    else:
        owned_department_qs = owned_department_qs.order_by(
    ...
    return render(
        request, "site_specific/dashboard_educational_department.html", context
    )
...
def dashboard_educational_project(request):
    if "postgresql" in settings.DATABASES['default']['ENGINE']:
        owned_project_qs = EducationalProjects.objects.accessible_to(
            request.user
        ).extra(
            select={
                'modified_or_creation_date':
    ...
    if sort_by == 'title':
        ...
        owned_project_qs = owned_project_qs.order_by(
            "modified_or_creation_date"
        )
    else:
        owned_project_qs = owned_project_qs.order_by(
    ...
    return render(
        request, "site_specific/dashboard_educational_project.html", context
    )
...
def dashboard_info_files(request):
    ...
    return render(
        request, 'site_specific/dashboard_info_files.html', {'files': files}
    )
...
def invite_to_claim_location(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            invitation_code = cryptString(
                cleaned['email'] + "|" + str(cleaned['location'].pk)
            )
...
def register_and_claim_location(request, invitation_code):
    ...
    register_form = ClaimingRegisterForm(
        u, initial={'email': email}, prefix="register"
    )
    login_form = ClaimingLoginForm(
        u, initial={'email_or_username': email}, prefix="login"
    )
    ...
    if request.method == "POST":
        ...
        if "register" in request.POST:
            register_form = ClaimingRegisterForm(
                u, request.POST, prefix="register"
            )
...
def culturebase_export_productions(request, location_slug):
    ...
    REVERSE_CATEGORY_MAPPER = dict(
        zip(CATEGORY_MAPPER.values(), CATEGORY_MAPPER.keys())
    )

    production_nodes = []
    for prod in Production.objects.filter(
        models.Q(in_program_of=location) | models.Q(play_locations=location),
        status="published",
    ).distinct():
        ...
        for event in prod.event_set.exclude(event_status="trashed"):
            persons = '\n'.join(
                [
                    u'%s - %s' %
                    (involvement.person, involvement.get_function())
                    for involvement in event.eventinvolvement_set.all()
                ]
            )
            if event.price_from:
            ...
            event_nodes.append(
                E.event(
                    ...
                    E.platzkategorie(E.vkpreis(price_range)),
                    E.eventMerkmal(
                        CDATA(
                            ', '.join(
                                event.characteristics.values_list(
                                    "title_de", flat=True
                                )
                            )
                        )
        ...
        try:
            first_date = prod.event_set.exclude(
                event_status="trashed"
            ).order_by("start_date")[0].start_date.strftime('%Y-%m-%d')
        except:
            first_date = ""
        try:
            last_date = prod.event_set.exclude(
                event_status="trashed"
            ).order_by("-start_date")[0].start_date.strftime('%Y-%m-%d')
        except:
            last_date = ""
        persons = '\n'.join(
            [
                u'%s - %s' % (involvement.person, involvement.get_function())
                for involvement in prod.productioninvolvement_set.all()
            ]
        )
        categories = ','.join(
            [
                str(REVERSE_CATEGORY_MAPPER[cat.pk])
                for cat in prod.categories.all()
                if REVERSE_CATEGORY_MAPPER.get(cat.pk, False)
            ]
        )
        ...
        production_nodes.append(
            E.eventReihe(
                E.erId(str(prod.pk)), E.erName(CDATA(prod.title_de)),
                ...
                E.erStadt(CDATA(location.city)), E.erBegin(first_date),
                E.erEnde(last_date),
                ...
                E.erMerkmal(
                    CDATA(
                        ', '.join(
                            prod.characteristics.values_list(
                                "title_de", flat=True
                            )
                        )
                    )
...
API_CHANGELOG = [
    {
        'date':
            datetime(2018, 11, 22, 12, 0),
        'changes':
            [
                u"The <strong>classiccard</strong> fields removed from productions and events.",
            ],
    },
    {
        'date':
            datetime(2017, 9, 10, 12, 0),
        'changes':
            [
    ...
    },
    {
        ...
        'changes': [u"""Stages for Berliner Ensemble changed.""", ],
    },
    {
        'date':
            datetime(2017, 5, 16, 12, 0),
        'changes':
            [
                u"""New <strong>event_status</strong> "trashed" added to events.""",
                u"""Future events which were imported before, but don't exist in the feeds anymore, get the <strong>event_status</strong> "trashed" instead of "canceled". <br /><em>Note that "canceled" events are shown in the Ruhr Bühnen website with a special label, whereas "trashed" events are hidden.</em>""",
                u"For trashed events only ID, event status, and creation and modification dates are provided in the export API.",
            ],
    },
    {
        'date':
            datetime(2017, 5, 11, 12, 0),
        'changes':
            [
                u"""
    ...
    },
    {
        ...
        'changes': [u"The website now uses HTTPS protocol.", ],
    },
    {
        'date':
            datetime(2016, 12, 20, 12, 0),
        'changes':
            [
    ...
    },
    {
        ...
        'changes': [u"Festival export", ],
    },
    {
        'date':
            datetime(2015, 10, 12, 12, 0),
        'changes':
            [
    ...
    {
        'date':
            datetime(2015, 3, 23, 12, 0),
        'changes':
            [
                u"Field <strong>organizer_title</strong> changed to <strong>organizers</strong> at events."
            ],
    },
    {
...
def api_changelog(request):
    return render(
        request, "site_specific/api_changelog.html",
