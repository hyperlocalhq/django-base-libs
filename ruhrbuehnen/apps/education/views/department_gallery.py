...
from ruhrbuehnen.apps.education.models import Image, PDF
...
def update_image_ordering(tokens, department):
    ...
    for mediafile_token in tokens.split(u","):
        mediafile = get_object_or_404(
            Image, department=department, pk=Image.token_to_pk(mediafile_token)
        )
...
def image_overview(request, slug):
    ...
    return render(
        request, "education/department_gallery/image_overview.html",
        {'department': instance}
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = Image(department=instance)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "department_image_overview", slug=instance.slug
                    )
    else:
        if media_file_obj:
            # existing media file
...
def update_pdf_ordering(tokens, department):
    ...
    for mediafile_token in tokens.split(u","):
        mediafile = get_object_or_404(
            PDF, department=department, pk=PDF.token_to_pk(mediafile_token)
        )
        mediafiles.append(mediafile)
    sort_order = 0
    for mediafile in mediafiles:
...
def pdf_overview(request, slug):
    ...
    return render(
        request, "education/department_gallery/pdf_overview.html",
        {'department': instance}
    )
...
def create_update_pdf(request, slug, mediafile_token="", **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = PDF(department=instance)
