...
from ruhrbuehnen.apps.education.forms.gallery import VideoForm, VideoDeletionForm
from ruhrbuehnen.apps.education.forms.gallery import ImageForm, ImageDeletionForm
from ruhrbuehnen.apps.education.forms.gallery import PDFForm, PDFDeletionForm

from jetson.apps.image_mods.models import FileManager
from filebrowser.models import FileDescription

from ruhrbuehnen.apps.education.models import Project
from ruhrbuehnen.apps.education.models import ProjectVideo, ProjectImage, ProjectPDF
...
def video_overview(request, slug):
    ...
    return render(
        request, "education/project_gallery/video_overview.html",
        {'project': instance}
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = ProjectVideo(project=instance)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "project_gallery_overview", slug=instance.slug
                    )
    else:
        if media_file_obj:
            # existing media file
...
def image_overview(request, slug):
    ...
    return render(
        request, "education/project_gallery/image_overview.html",
        {'project': instance}
    )
...
def create_update_image(request, slug, mediafile_token="", **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = ProjectImage(project=instance)
            ...
            if "hidden_iframe" in request.REQUEST:
                ...
                if cleaned['goto_next']:
                    ...
                    return redirect(
                        "project_image_overview", slug=instance.slug
                    )
    else:
        if media_file_obj:
            # existing media file
...
def pdf_overview(request, slug):
    ...
    return render(
        request, "education/project_gallery/pdf_overview.html",
        {'project': instance}
    )
...
def create_update_pdf(request, slug, mediafile_token="", **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not media_file_obj:
                media_file_obj = ProjectPDF(project=instance)
