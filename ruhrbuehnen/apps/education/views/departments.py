...
from ruhrbuehnen.apps.education.forms.departments import DEPARTMENT_FORM_STEPS
...
def department_list(request):
    ...
    qs = Department.objects.filter(status="published").order_by(
        Lower("title_%s" % request.LANGUAGE_CODE)
    )
...
def change_department(request, slug):
    ...
    return show_form_step(
        ...
        extra_context={'department': instance},
        instance=instance
    )
