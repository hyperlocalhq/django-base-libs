...
from ruhrbuehnen.apps.education.models import Project, ProjectTime
from ruhrbuehnen.apps.education.forms.projects import PROJECT_FORM_STEPS, ProjectDuplicateForm
...
def project_detail(request, slug, event_id=None):
    ...
    extra_context = {'department': None, 'event': None}

    if obj.departments.filter(status="published").count():
        try:
            extra_context['department'] = obj.departments.filter(
                status="published"
            )[0]
        except IndexError:
            pass
...
def change_project(request, slug):
    ...
    return show_form_step(
        request,
        PROJECT_FORM_STEPS,
        extra_context={'project': instance},
        instance=instance
    )
...
def duplicate_project(request, slug):
    ...
    ) or not request.user.has_perm("education.add_project"):
        return access_denied(request)
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponse(
                reverse("change_project", kwargs={'slug': new_project.slug})
            )
    else:
        form = ProjectDuplicateForm(instance=project)
    return render(
        request, "education/projects/forms/duplication_form.html",
        {'form': form}
    )
