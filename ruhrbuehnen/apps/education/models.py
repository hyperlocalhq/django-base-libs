...
class Department(CreationModificationMixin, UrlMixin, SlugMixin()):
    ...
    contact_name = models.CharField(
        _("Contact Name"), max_length=255, blank=True
    )
    ...
    def get_future_projects(self):
        ...
        projects_with_events = all_projects.filter(projecttime__isnull=False
                                                  ).distinct()
        projects_sorted_events = sorted(
            projects_with_events, key=lambda t: t.get_next_timestamp()
        )
...
class Project(CreationModificationMixin, UrlMixin, SlugMixin()):
    ...
    departments = models.ManyToManyField(
        Department,
        verbose_name=_("Educational departments"),
        blank=True,
    ...
    contact_department = models.CharField(
        _("Contact Department"), max_length=255, blank=True
    )
    contact_name = models.CharField(
        _("Contact Name"), max_length=255, blank=True
    )
    ...
    participant_count = MultilingualTextField(
        _("Participant count"), blank=True
    )
    needs_teachers = models.BooleanField(
        _("Project needs teachers"), default=False
    )
    ...
    special_conditions = MultilingualTextField(
        _("Special conditions"), blank=True
    )
    ...
    target_groups = models.ManyToManyField(
        "ProjectTargetGroup", verbose_name=_("Target groups"), blank=True
    )
    formats = models.ManyToManyField(
        "ProjectFormat", verbose_name=_("Project formats"), blank=True
    )
    ...
    def duplicate(self, new_values=None):
        ...
        target_proj.slug = get_unique_value(
            Project, better_slugify(target_proj.title_de)
        )
