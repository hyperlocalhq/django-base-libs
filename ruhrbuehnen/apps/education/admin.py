...
class DepartmentAdmin(ExtendedModelAdmin):
    list_display = (
        'title', 'creation_date', 'modified_date', 'newsletter', 'status'
    )
    list_editable = (
        'newsletter',
        'status',
    ...
    inlines = [
        DepartmentMemberInline,
        DepartmentSocialMediaChannelInline,
    ...
    def owners_view(self, request, department_id):
        ...
        if request.method == "POST":
            ...
            form = OwnersForm(initial={'users': department.get_owners()})
        ...
        return render(
            request, 'admin/owners.html', {
                ...
                'original':
                    department,
                'app_label':
                    Department._meta.app_label,
                'opts':
                    Department._meta,
                'form':
                    form,
                'title':
                    ugettext('The owners of %(department)s') % {
...
class ProjectAdmin(ExtendedModelAdmin):
    list_display = (
        'title_de', 'get_locations', 'creation_date', 'modified_date',
        'newsletter', 'status'
    )
    list_editable = (
        'newsletter',
    ...
    fieldsets += get_admin_lang_section(
        _("Description"), [
            'description', 'special_conditions', 'remarks', 'cooperation',
            'supporters', 'participant_count'
        ]
    )
    fieldsets += [
        (
            _("Prices"), {
                'fields':
                    [
                        'free_entrance', 'tickets_website',
                        get_admin_lang_section(
                            _("Price information"), ['prices']
                        )
                    ]
            }
        ),
    ...
    fieldsets += [
        (
            _("Additional details"), {
                'fields':
                    [
                        'age_from', 'age_till', 'needs_teachers',
                        'target_groups', 'formats'
                    ]
            }
        )
    ]
    ...
    def get_locations(self, obj):
        ...
        if obj.departments.count():
            html.append(
                ugettext('Departments') + ': ' + ', '.join(
                    (
                        '<a href="' + loc.get_url_path() +
                        '" tagret="_blank">' + unicode(loc) + '</a>'
                        for loc in obj.departments.all()
                    )
                )
            )
        return '<br />'.join(html)
    ...
    def owners_view(self, request, project_id):
        ...
        if request.method == "POST":
            ...
            form = OwnersForm(initial={'users': project.get_owners()})
        ...
        return render(
            request, 'admin/owners.html', {
                ...
                'form':
                    form,
                'title':
                    ugettext('The owners of %(project)s') % {
                        'project': project
                    },
            }
        )
