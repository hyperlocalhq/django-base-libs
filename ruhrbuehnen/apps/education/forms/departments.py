...
from ruhrbuehnen.apps.education.models import SocialMediaChannel
from ruhrbuehnen.apps.education.models import DepartmentMember
    ...
    def __init__(self, *args, **kwargs):
        ...
        layout_blocks.append(
            layout.Fieldset(
                ...
                layout.HTML(
                    """{% load i18n %}<div class="subtitle">{% trans "leave blank if you want to use the data from the location" %}</div>"""
                ),
                layout.Row(
                    layout.Div(
                        "street_address",
...
DepartmentMemberFormset = inlineformset_factory(
    Department,
    DepartmentMember,
...
def submit_step(current_step, form_steps, form_step_data, instance=None):
        ...
        if form_step_data[current_step]['location']:
            if isinstance(form_step_data[current_step]['location'], Location):
                ...
                instance.location = Location.objects.get(
                    pk=form_step_data[current_step]['location']
                )
    ...
    if current_step == "description":
        ...
        instance.departmentmember_set.exclude(pk__in=member_ids_to_keep
                                             ).delete()
