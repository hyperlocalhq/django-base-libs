...
class JobOfferAdmin(ExtendedModelAdmin):
    list_display = (
        'position', 'creation_date', 'modified_date', 'deadline',
        'start_contract_on', 'get_owners_list', 'status'
    )
    ...
    fieldsets += [
        (_("Dates"), {
            'fields': ('deadline', 'start_contract_on')
        }),
    ]
    fieldsets += [
        (
            _("Address"), {
                'fields':
                    (
                        'company', 'street_address', 'street_address2',
    ...
    def get_map(self, instance):
        return mark_safe(
            render_to_string(
                "admin/map.html", {'STATIC_URL': settings.STATIC_URL}
            )
        )
    ...
    def owners_view(self, request, job_offer_id):
        ...
        if request.method == "POST":
            ...
            form = OwnersForm(initial={'users': job_offer.get_owners()})
        ...
        return render(
            request, 'admin/owners.html', {
                ...
                'original':
                    job_offer,
                'app_label':
                    JobOffer._meta.app_label,
                'opts':
                    JobOffer._meta,
                'form':
                    form,
                'title':
                    ugettext('The owners of %(job_offer)s') % {
                        'job_offer': job_offer
                    },
            }
        )
