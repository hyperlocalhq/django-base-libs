...
from ruhrbuehnen.apps.accounts.forms import password_change_form_helper
from ruhrbuehnen.apps.accounts.forms import password_reset_form_helper
from ruhrbuehnen.apps.accounts.forms import password_reset_change_form_helper

urlpatterns = patterns("",
    url(r'^login/$', 'ruhrbuehnen.apps.accounts.views.login', {'template_name': 'accounts/login.html', 'redirect_to': '/'}, name="login"),
    url(r'^logout/$', 'django.contrib.auth.views.logout', {'next_page': "/"}, name="logout"),
    url(r'^signup/$', 'ruhrbuehnen.apps.accounts.views.register', name="signup"),
    ...
    url(r'^signup/(?P<encrypted_email>[a-zA-Z0-9\+\/_\-=]+)/$', 'ruhrbuehnen.apps.accounts.views.confirm_registration'),
    ...
    url(r'^password-reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', 'django.contrib.auth.views.password_reset_confirm', {'template_name': 'accounts/password_reset_confirm.html', 'extra_context': {'form_helper': password_reset_change_form_helper}}),
