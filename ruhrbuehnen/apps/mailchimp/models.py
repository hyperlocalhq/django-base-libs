...
class Settings(models.Model):
    ...
    double_optin = models.BooleanField(
        _("Double Optin"),
        default=True,
        help_text=_(
            "Flag to control whether a double opt-in confirmation message is sent. ABUSING THIS MAY CAUSE YOUR ACCOUNT TO BE SUSPENDED."
        )
    )
    update_existing = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Flag to congtrol whether the existing subscribers should be updated instead of throwing an error."
        )
    )
    send_welcome = models.BooleanField(
        _("Send welcome"),
        default=False,
        help_text=_(
            "If double optin is false and this is true, MailChimp will send a welcome message to new subscribers."
        )
    )
    delete_member = models.BooleanField(
        _("Delete member"),
        default=False,
        help_text=_(
            "Flag to completely delete the member from the list instead of just unsubscribing"
        )
    )
    send_goodbye = models.BooleanField(
        _("Send goodbye"),
        default=True,
        help_text=_("Flag to send the goodbye message to the email address.")
    )
...
class Subscription(CreationModificationDateMixin):
    subscriber = models.ForeignKey(
        User, verbose_name=_("Subscriber"), null=True, blank=True
    )
    ...
    status = models.CharField(
        ...
        max_length=200,
        blank=True,
        choices=INFOSUBSCRIPTION_STATUS_CHOICES
    )
...
class MList(SingleSiteMixin):
    ...
    is_public = models.BooleanField(
        _("Public"),
        help_text=_(
            'Will this mailing list be displayed in the public settings of subscriptions?'
        ),
        default=True
    )
...
class Campaign(CreationModificationMixin):
    sender_name = models.CharField(
        _("Sender name"),
        max_length=255,
        default=getattr(settings, 'MAILING_DEFAULT_FROM_NAME', '')
    )
    sender_email = models.EmailField(
        _("Sender email"),
        default=getattr(settings, 'MAILING_DEFAULT_FROM_EMAIL', '')
    )
    ...
    def get_mailinglist_with_link(self):
        r = '<div><a href="%(link)s" class="cross_link">%(name)s</a> (%(count)s recipients)</div>' % {
            ...
            'name':
                self.mailinglist.title,
            'count':
                self.mailinglist.get_count(),
        }
        return r
...
CONTENT_TYPE_CHOICES = getattr(
    settings, "MAILING_CONTENT_TYPE_CHOICES", (
...
class MailingContentBlock(models.Model):
    ...
    content_type = models.CharField(
        'Content Type', max_length=40, choices=CONTENT_TYPE_CHOICES, blank=True
    )
