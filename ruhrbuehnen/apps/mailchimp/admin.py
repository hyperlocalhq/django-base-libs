...
class SettingsAdmin(admin.ModelAdmin):
    list_display = (
        'api_key', 'double_optin', 'update_existing', 'send_welcome',
    ...
    def clean(self):
        ...
        if not cleaned.get("subscriber", None) and not cleaned.get("email", ""):
            self._errors['email'] = ErrorList(
                [
                    _(
                        "Either subscriber or subscriber email should be filled in."
                    )
                ]
            )
        return cleaned
...
class SubscriptionAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'email', 'first_name', 'last_name', 'get_mailinglist_with_link',
    ...
    fieldsets = (
        (
            None, {
                'fields':
                    (
                        "mailinglist", "subscriber", "first_name", "last_name",
                        "email", "ip", "status"
                    )
            }
        ),
...
class MListAdmin(admin.ModelAdmin):
    form = MListAdminForm
    list_display = (
        'title', 'get_mailchimp_list', 'get_count_with_link', 'last_sync',
        'is_public'
    )
