...
def do_infobanner(parser, token):
    try:
        ...
        try:
            ...
            raise template.TemplateSyntaxError, "{0} tag requires a following syntax: {% {0} <sysname> [using <template_path>] %}".format(
                token.contents[0]
            )
...
class InfoBannerRenderer(template.Node):
    ...
    def render(self, context):
        ...
        banner, created = InfoBanner.objects.get_or_create(sysname=sysname, )
        ...
        output = loader.render_to_string(
            template_path or "infobanners/infobanner.html", context_vars
        )
        context_vars.pop()
        return output
