...
class SlideInline(ExtendedStackedInline):
    ...
    fieldsets = get_admin_lang_section(
        None, ['title', 'subtitle', 'credits', 'alt']
    )
    fieldsets += [
        (None, {
            'fields': ["path", "link", "highlight"]
        }),
    ]
    fieldsets += [
        (None, {
            'fields': ["published_from", "published_till"]
        }),
    ]
    fieldsets += [
        (None, {
...
class SlideshowOptions(ExtendedModelAdmin):
    ...
    def slides_shown_now(self, obj):
        ...
        return _("%(num)s out of %(total)s") % {
            'num':
                obj.slide_set.filter(
                    models.Q(published_from__lte=now) |
                    models.Q(published_from__isnull=True),
                    models.Q(published_till__gt=now) |
                    models.Q(published_till__isnull=True)
                ).count(),
            'total':
                obj.slide_set.count(),
        }
