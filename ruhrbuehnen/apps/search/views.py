...
class SearchView(haystack_views.SearchView):
    def __init__(
        ...
        super(SearchView, self).__init__(
            template, load_all, form_class, searchqueryset, results_per_page
        )
        self.limit = limit
        ...
        if self.form.is_valid():
            for short_name, verbose_name in model_choices():
                if self.form.cleaned_data[
                    self.form.MODELS_PARAM_NAME
                ] and short_name not in self.form.cleaned_data[
                    self.form.MODELS_PARAM_NAME]:
                    continue
                app_model = indexes[short_name]  # e.g. "museums.museum"
                results = self.results.models(
                    apps.get_model(*app_model.split("."))
                )
                ...
                if length and results:
                    d = {
                        ...
                        'count':
                            length,
                        'results':
                            results[:self.limit] if self.limit else results,
                    }
                    if self.limit is None and short_name in self.form.cleaned_data[
                        self.form.MODELS_PARAM_NAME]:
