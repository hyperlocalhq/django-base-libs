...
mptt.jQuery(function($){
    ...
    function isExpandedNode(id) {
        return mptt.collapsed_nodes.indexOf(id) == -1;
    }

    function markNodeAsExpanded(id) {
        // remove itemId from array of collapsed nodes
        var idx = mptt.collapsed_nodes.indexOf(id);
        if(idx >= 0)
            mptt.collapsed_nodes.splice(idx, 1);
    }

    function markNodeAsCollapsed(id) {
        if(isExpandedNode(id))
            mptt.collapsed_nodes.push(id);
    }
	...
	$.extend($.fn.mpttTree = function() {
		$('tr', this).each(function(i, el) {
			...
			$('a.drag_handle').bind('mousedown', function(event) {
			BEFORE = 0;
			AFTER = 1;
			CHILD = 2;
			CHILD_PAD = 20;
			...
			$("body").disableSelection().bind('mousemove', function(event) {
				...
				if($("#drag_line").length < 1) {
					$("body").append('<div id="drag_line" style="position:absolute"><div></div></div>');
				}
				...
				$("tr", originalRow.parent()).each(function(index, element) {
					...
					if (event.pageY >= top && event.pageY <= top + rowHeight) {
						...
						if(targetRow) {
							...
							var padding = 90 + element.attr('rel') * CHILD_PAD;
    ...
    function storeCollapsedNodes(nodes) {
        $.cookie('mptt_collapsed_nodes', "[" + nodes.join(",") + "]", { expires: 7 });
    }

    function retrieveCollapsedNodes() {
        var n = $.cookie('mptt_collapsed_nodes');
        if(n != null) {
            try {
                n = $.parseJSON(n);
            } catch(e) {
    ...
    function expandOrCollapseNode(item) {
        ...
        storeCollapsedNodes(mptt.collapsed_nodes);
