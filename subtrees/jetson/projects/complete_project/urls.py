...
urlpatterns += patterns('',
    ...
    url(r'^dashboard/$', 'django.views.generic.simple.direct_to_template', {'template': 'index.html'}, name="dashboard"),
    ...
    url(r'^%s/(?P<slug>[^/]+)/invited/(?P<encrypted_email>.+)/$' % URL_ID_PERSON, 'jetson.apps.individual_relations.views.confirm_invitation'),
     ...
     # info blog
    url(r'^(?P<url_identifier>blog)/', include('jetson.apps.blog.urls'), 
        {
    ...
    url(r'^feedback/$', 'jetson.apps.tracker.views.create_ticket'),
    url(r'^feedback/(?P<concern>[^/]+)/$', 'jetson.apps.tracker.views.create_ticket'),
    url(r'^feedback/(?P<content_type_id>[0-9]+)/(?P<object_id>[0-9]+)/$', 'jetson.apps.tracker.views.create_ticket'),
    url(r'^feedback/(?P<concern>[^/]+)/(?P<content_type_id>[0-9]+)/(?P<object_id>[0-9]+)/$', 'jetson.apps.tracker.views.create_ticket'),
    ...
    url(r'^login/$', 'jetson.apps.people.views.login', {'template_name': 'accounts/login.html'}, name="login"),
    url(r'^logout/$', 'django.contrib.auth.views.logout', {'next_page': "/"}, name="logout"),
    url(r'^changeuser/$', 'django.contrib.auth.views.logout_then_login' ),
    url(r'^register/$', 'jetson.apps.people.views.register', name="register"),
    url(r'^register/done/$', 'django.views.generic.simple.direct_to_template', {'template': 'accounts/register_verify_required.html'}, name="register_done"),
