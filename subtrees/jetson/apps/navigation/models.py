...
LINKABLE_MODELS = getattr(settings, "LINKABLE_MODELS", ("flatpages.Flatpage", ))
...
class NavigationLink(
    MPTTModel, SingleSiteMixin, LinkedObjectMixin,
    SysnameMixin(
        help_text=_(
            "Do not change this value! Sysnames are used to display tree branches of navigation links in templates. Also they are used for binding specific styling or scripts to specific navigation items."
        )
    )
):
    ...
    link_url = models.CharField(
        ...
        blank=True,
        help_text=_(
            "It can contain template tags and variables. The link is shown in the navigation menu only if it returns a non-empty string."
        )
    )
    related_urls = PlainTextModelField(
        _("Related URLs"),
        blank=True,
        help_text=_(
            "Other URLs for which this link should be highlighted, one per line. It can contain template tags and variables."
        )
    )
    is_group = models.BooleanField(_("Group of links"), default=False)
    is_group_name_shown = models.BooleanField(
        _("Show group name"), default=True
    )
    is_shown_for_visitors = models.BooleanField(
        _("Shown for visitors"), default=True
    )
    ...
    def get_children_for_authenticated_user(self):
        if not hasattr(self, "_children_for_authenticated_user"):
            self._children_for_authenticated_user = self.get_children().filter(
                ...
                models.Q(site__pk=settings.SITE_ID),
