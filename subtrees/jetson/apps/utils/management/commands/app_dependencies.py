        ...
        for app in settings.INSTALLED_APPS:
            ...
            if app_info['using']:
                ...
                for dep in sorted(app_info['using'].keys()):
                    print "        ", colored(dep, "white"), (app_info['using'][dep] and " " or colored("NOT INSTALLED", "red"))
                
            if app_info['can_use']:
                print "    optionally uses:"
                for dep in sorted(app_info['can_use'].keys()):
                    print "        ", colored(dep, "white"), (app_info['can_use'][dep] and " " or colored("not installed", "red"))
                    
            if not app_info['used_by'] and not app_info['using'] and not app_info['can_use']:
