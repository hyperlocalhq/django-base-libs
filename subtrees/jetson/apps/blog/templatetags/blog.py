...
class TagCloudForBlogNode(template.Node):
    ...
    def render(self, context):
        ...
        context[self.context_var] = sorted(
            Tag.objects.cloud_for_model(Post, **self.kwargs),
...
def do_tag_cloud_for_blog(parser, token):
    ...
    if len_bits != 4 and len_bits not in range(6, 9):
        raise TemplateSyntaxError(
            _(
                '%s tag requires either three or between five and seven arguments'
            ) % bits[0]
        )
    if bits[2] != 'as':
        raise TemplateSyntaxError(
            ...
            try:
                ...
                if name == 'steps' or name == 'min_count':
                    ...
                    if value in ['linear', 'log']:
                        ...
                        raise TemplateSyntaxError(
                            _(
                                "%(tag)s tag's '%(option)s' option was not a valid choice: '%(value)s'"
                            ) % {
                                'tag': bits[0],
                                'option': name,
                                'value': value,
                    ...
                    raise TemplateSyntaxError(
                        _(
                            "%(tag)s tag was given an invalid option: '%(option)s'"
                        ) % {
                            'tag': bits[0],
                            'option': name,
                        }
                ...
                raise TemplateSyntaxError(
                    _(
                        "%(tag)s tag was given a badly formatted option: '%(option)s'"
                    ) % {
                        'tag': bits[0],
                        'option': bits[i],
                    }
    ...
    'c':
        {
            ...
            'preprocessor':
                shell_comment_rule,
            'keyword':
                r'\b(bool|break|byte|case|char|const|continue|default|do|double|else|enum|extern|false|float|for|goto|if|int|long|null|return|short|sizeof|static|struct|switch|true|uint|ulong|ushort|volatile|void|while)\b',
        },
    'cmd':
        {
            'comment':
    ...
    'javascript':
        {
            ...
            'string':
                string_rule,
            'global':
                r'\b(toString|valueOf|window|element|prototype|constructor|document|escape|unescape|parseInt|parseFloat|setTimeout|clearTimeout|setInterval|clearInterval|NaN|isNaN|Infinity)\b',
            'keyword':
                r'\b(arguments|break|case|continue|default|delete|do|else|false|for|function|if|in|instanceof|new|null|return|switch|this|true|typeof|var|void|while|with)\b',
        },
    'python':
        {
            'comment':
            ...
            'string':
                ...
                + r'''"""(?:[^"]|\\"|"{1,2}(?!"))*"""|"(?:[^"]|\\")*"(?!"))''',
            'keyword':
                r'\b(and|assert|break|class|continue|def|del|elif|else|except|exec|finally|for|from|global|if|import|in|is|lambda|not|or|pass|print|raise|return|try|while|yield|None|True|False)\b',
        },
    'sql':
        {
            'comment':
    ...
    def highlight_block(self, doc, text, language):
        ...
        while text:
            ...
            if first:
                ...
                code.appendChild(
                    doc.createTextNode(text[:first['match'].start(1)])
                )
