...
class EmailTemplatePlaceholderOptions(admin.ModelAdmin):
    ...
    fieldsets += [
        (None, {
            'fields': ("sysname", "relates_to", "help_text")
        }),
    ]

    prepopulated_fields = {
        "sysname": ("name_%s" % settings.LANGUAGE_CODE, ),
    }
    ...
    for m in queryset:
        if not m.is_sent:
            if m.send():
                messages.success(
                    request,
                    ugettext("""Message "%s" has been sent""") % m
                )
            else:
                messages.error(
                    request,
                    ugettext("""Message "%s" has not been sent""") % m
                )
...
class EmailMessageOptions(ExtendedModelAdmin):
    list_display = (
        'creation_date', 'subject', 'recipients_display', 'is_sent',
    ...
    search_fields = (
        'subject', 'body', 'body_html', 'sender_email', 'recipient_emails'
    )
    save_on_top = True
    fieldsets = (
        (
        ...
        (
            _("Recipients"), {
                'classes': ('grp-collapse grp-open', ),
                'fields': ('recipient', 'recipient_emails')
            }
        ),
        (
            _("Message"), {
        ...
        (
            _("Flags"), {
                ...
                'fields': ('is_sent', 'delete_after_sending')
            }
        ),
    )
