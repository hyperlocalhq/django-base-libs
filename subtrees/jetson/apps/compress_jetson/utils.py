...
def media_root(filename):
    """
    Return the full path to ``filename``. ``filename`` is a relative path name in JETSON_MEDIA_ROOT
    """
    return os.path.join(django_settings.JETSON_MEDIA_ROOT, filename)

def media_url(url, prefix=None):
    if prefix:
        return prefix + urlquote(url)
    return django_settings.JETSON_MEDIA_URL + urlquote(url)
