...
class SiteSettingsBase(MetaTagsMixin):
    ...
    extra_head = PlainTextModelField(
        _("Extra head"),
        help_text=_(
            "Third-party code snippets to be added to the end of the HEAD section."
        ),
        blank=True
    )
    extra_body = PlainTextModelField(
        _("Extra body"),
        help_text=_(
            "Third-party code snippets to be added to the end of the BODY section."
        ),
        blank=True
    )
    ...
    def prepare_nonces_for_extra_head_and_extra_body(self):
        ...
        if self.extra_head:
            self._extra_head_with_nonces = style_pattern.sub(
                self._add_style_nonce, self.extra_head
            )
            self._extra_head_with_nonces = script_pattern.sub(
                self._add_script_nonce, self._extra_head_with_nonces
            )
        if self.extra_body:
            self._extra_body_with_nonces = style_pattern.sub(
                self._add_style_nonce, self.extra_body
            )
            self._extra_body_with_nonces = script_pattern.sub(
                self._add_script_nonce, self._extra_body_with_nonces
            )

    def get_extra_head_with_nonces(self):
        if (self.extra_head and not self._extra_head_with_nonces
           ) or (self.extra_body and not self._extra_body_with_nonces):
    ...
    def get_script_nonces(self):
        ...
        return mark_safe(
            ' '.join(
                [
                    "'nonce-{}'".format(nonce)
                    for nonce in (self._script_nonces or [])
                ]
            )
        )
    ...
    def get_style_nonces(self):
        ...
        return mark_safe(
            ' '.join(
                [
                    "'nonce-{}'".format(nonce)
                    for nonce in (self._style_nonces or [])
                ]
            )
        )
...
class PageSettingsManager(models.Manager):
    def get_settings_for_page(self, path):
        ...
        ps_list = self.filter(
            path=path_without_root, site__id=settings.SITE_ID
        ).order_by(["-user"])
        if ps_list:
            return ps_list[0]
        else:
