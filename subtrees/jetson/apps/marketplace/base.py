...
class JobOfferBase(CreationModificationMixin, PublishingMixin, UrlMixin):
    ...
    contact_person_name = models.CharField(_("Organizer"), blank=True, max_length=255)


    postal_address = models.ForeignKey(Address, verbose_name=_("Postal Address"), related_name="address_job_offers", null=True, blank=True)
    ...
    url0_type = models.ForeignKey(URLType, verbose_name=_("URL Type"), blank=True, null=True, related_name='job_offers0')
    ...
    url1_type = models.ForeignKey(URLType, verbose_name=_("URL Type"), blank=True, null=True, related_name='job_offers1')
    ...
    url2_type = models.ForeignKey(URLType, verbose_name=_("URL Type"), blank=True, null=True, related_name='job_offers2')
    ...
    im0_type = models.ForeignKey(IMType, verbose_name=_("IM Type"), blank=True, null=True, related_name='job_offers0')
    ...
    im1_type = models.ForeignKey(IMType, verbose_name=_("IM Type"), blank=True, null=True, related_name='job_offers1')
    ...
    im2_type = models.ForeignKey(IMType, verbose_name=_("IM Type"), blank=True, null=True, related_name='job_offers2')
    ...
    email0_type = models.ForeignKey(EmailType, verbose_name=_("Email Type"), blank=True, null=True, related_name='job_offers0')
    ...
    email1_type = models.ForeignKey(EmailType, verbose_name=_("Email Type"), blank=True, null=True, related_name='job_offers1')
    ...
    email2_type = models.ForeignKey(EmailType, verbose_name=_("Email Type"), blank=True, null=True, related_name='job_offers2')
    ...
    publish_emails = models.BooleanField(_("Show email addresses to unregistered visitors?"), default=False)
    is_commercial = models.BooleanField(_("One has to pay to get information about the job"), default=False)
    ...
    def get_phones(self):
        if not hasattr(self, '_phones_cache'):
            ...
            } for pos in range(3) if getattr(self, "phone%d_number" % pos) and not getattr(self, "is_phone%d_on_hold" % pos)]
    ...
    def get_ims(self):
        if not hasattr(self, '_ims_cache'):
            ...
            } for pos in range(3) if getattr(self, "im%d_address" % pos) and not getattr(self, "is_im%d_on_hold" % pos)]
    ...
    def get_emails(self):
        if not hasattr(self, '_emails_cache'):
            self._emails_cache = [{
                ...
                "address_protected": getattr(self, "email%d_address" % pos).replace("@", ugettext(" (at) ")).replace(".", ugettext(" (dot) ")),
                "is_default": getattr(self, "is_email%d_default" % pos),
                "is_on_hold": getattr(self, "is_email%d_on_hold" % pos),
            } for pos in range(3) if getattr(self, "email%d_address" % pos) and not getattr(self, "is_email%d_on_hold" % pos)]
    ...
    def are_contacts_displayed(self, user=None):
        if not hasattr(self, "_are_contacts_displayed_cache"):
            ...
            self._are_contacts_displayed_cache = bool(
                self.postal_address
                or (user and user.has_perm("marketplace.change_job_offer", self))
                )
