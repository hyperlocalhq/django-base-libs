...
class ForumContainerOptions(MultiSiteContainerMixinAdminOptions):
    ...
    fieldsets += [
        (
            _('Administration'), {
                'fields': ('allow_bumping', 'max_level'),
...
class ForumOptions(TreeEditor):
    ...
    search_fields = ('title', )
    list_display = (
        'actions_column', 'indented_short_title', 'container', 'creator',
        'get_nof_threads', 'get_nof_replies', 'status'
    )
    list_filter = (
...
class ForumThreadOptions(ExtendedModelAdmin):
    ...
    list_display = (
        '__unicode__', 'forum', 'creator', 'get_nof_replies', 'is_sticky',
        'views'
    )
    list_filter = (
        'forum', 'creation_date', 'creator', 'modified_date', 'modifier'
    )

    fieldsets = [
        (None, {
            'fields': ('forum', 'is_sticky')
        }),
    ]
...
class ForumReplyOptions(TreeEditor):
    ...
    list_display = (
        'actions_column', 'indented_short_title', 'thread', 'creator'
    )
    list_filter = (
        'thread', 'creation_date', 'creator', 'modified_date', 'modifier'
    )
