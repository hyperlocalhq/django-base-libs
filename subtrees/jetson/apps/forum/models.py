...
#TODO
FORUM_PERMISSIONS = (
    ('can_apply_forum_options', 'Can apply global options to all forums under the forum container'),
    ...
    ('can_change_self_post', 'Can edit a self written post (thread starting post or reply)'),
...
FORUM_ROLE_PERMISSIONS = {'members': (), 'moderators': (), 'owners': (),}
...
class ForumContainer(MultiSiteContainerMixin, CreationModificationDateMixin):
    ...
    allow_bumping = models.BooleanField(
        _("allow bumping"), 
        default=False,
        help_text=_("check this field, if you want to allow your threads be displayed at the top again, if nobody has answered yet. So, every thread should get a chance to be heard, even if the forum is highly frequented. 'Bumping' is done by posting a second time to the (not yet answered) thread."),
    )
    
    max_level = models.IntegerField(
        _("max nesting level"), 
        default=1,
        help_text=_("the maximum nesting level. As forums can contain forums, restrict the nesting level here. If the maximum nesting level is set to 0, only one default forum is allowed."),
    )
...
class Forum(MPTTModel, CreationModificationMixin, UrlMixin, SlugMixin()):
    ...
    """
    container = models.ForeignKey(ForumContainer, verbose_name=_('container'),)
    ...
    short_title = models.CharField(
        _('short title'), 
        help_text=_("a short title to be displayed in page breadcrumbs and headline. If left blank, short title is derived from title."),
        max_length=32,
        blank=True,
        )
    ...
    status = models.IntegerField(
        _("status"), 
        default=STATUS_CODE_PUBLIC, 
        choices=STATUS_CODES_CHOICES
    )
    ...
    def get_children_latest_threads(self, exclude_is_sticky=True):
        ...
        return qs.order_by('-is_sticky', '-forumreply__creation_date', '-creation_date')
...
class ForumThread(CreationModificationMixin, ViewsMixin, UrlMixin, SlugMixin(proposal="thread")):
    """
    ...
    is_sticky = models.BooleanField(
        _("is sticky"), 
        default=False,
        help_text=_("check this field, if you want your thread to appear continually at the top of the forum."),
    )
    status = models.IntegerField(
        _("status"), 
        default=STATUS_CODE_PUBLIC, 
...
class ForumReply(MPTTModel, CreationModificationMixin, UrlMixin, SlugMixin(proposal="reply")):
    """
