...
ORGANIZING_INSTITUTION_CHOICES = XChoiceList(
    get_related_queryset(Event, "organizing_institution")
)
...
class EventForm:  # namespace
    class MainDataForm(dynamicforms.Form):
        ...
        organizer_ind = forms.ChoiceField(
            initial=0, choices=ORGANIZER_CHOICES, widget=forms.RadioSelect()
        )
            ...
            if (
                ...
                if isinstance(initial_related_events[0], models.Model):
                    kwargs['initial']['related_events'] = [
                        force_unicode(obj.pk) for obj in initial_related_events
                    ]
        ...
        def clean(self):
            ...
            if self.cleaned_data.get('venue_title', None):
                ...
                if self.cleaned_data.get('venue', None):
                    for field_name in [
                        'venue_title', 'street_address', 'postal_code', 'city',
                        'country'
                    ]:
            ...
            if organizer_ind == 1:
                if self.cleaned_data.get('organizer_title', "") != "":
                    ...
                    if self.cleaned_data.get(
                        ...
                        for field_name in [
                            'organizer_title', 'organizer_url_link'
                        ]:
                            if self._errors.get(field_name, False):
                                del self._errors[field_name]
            else:
                for field_name in [
                    'organizing_institution', 'organizer_title',
                    'organizer_url_link'
                ]:
                    if self._errors.get(field_name, False):
                        del self._errors[field_name]
    ...
    class EventTimeForm(dynamicforms.Form):
        ...
        def clean(self):
            ...
            if start_dd:
                if not start_mm:
                    self._errors['start_dd'] = [
                        _("Please enter a valid month.")
                    ]
            try:
                start_date = datetime.date(
                    int(start_yyyy), int(start_mm or 1), int(start_dd or 1)
                )
            except:
            ...
            if 'start_hh' in self._errors or 'start_ii' in self._errors:
                self._errors['start_dd'] = [
                    _(
                        "Please enter a valid time using format 'HH:MM' or choose 'All Day'"
                    )
                ]
            ...
            if self.cleaned_data.get(
                'start_hh', None
            ) and not self.cleaned_data.get('start_ii', None):
                self.cleaned_data['start_ii'] = '0'

            if self.cleaned_data.get(
                'end_hh', None
            ) and not self.cleaned_data.get('end_ii', None):
            ...
            if self.cleaned_data.get('start_hh', None):
                if not (start_yyyy and start_mm and start_dd):
                    self._errors['start_hh'] = [
                        _(
                            "If you choose a time, please enter a valid day, month and year."
                        )
                    ]
            ...
            if end_yyyy or end_mm or end_dd:
                ...
                try:
                    end_date = datetime.date(
                        int(end_yyyy), int(end_mm or 1), int(end_dd or 1)
                    )
                except:
                    self._errors['end_dd'] = [
                        _(
                            "If you want to specify an end date, please enter a valid one."
                        )
                    ]
    ...
    class FeesForm(dynamicforms.Form):
        ...
        def clean(self):
            ...
            for week_day in WEEK_DAYS:
                ...
                is_closed = self.cleaned_data.get(
                    week_day + '_is_closed', False
                )
    ...
    def save_data(form_steps, form_step_data):
        ...
        # orgnaizer is selected separately
            ...
            if step_main_data.get('organizing_institution', None):
                ...
                organizer_url_link = step_main_data.get(
                    'organizer_url_link', None
                )
        ...
        event.is_appointment_based = step_fees_opening_hours.get(
        ...
        if media_file:
            tmp_path = os.path.join(
                settings.PATH_TMP, media_file['tmp_filename']
            )
            f = open(tmp_path, 'r')
            filename = tmp_path.rsplit("/", 1)[1]
            image_mods.FileManager.save_file_for_object(
                event, filename, f.read(), subpath="avatar/"
            )
            f.close()
