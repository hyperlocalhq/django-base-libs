  ...
  CMS.Toolbar = CMS.Class.$extend({
    ...
    initialize: function(container, options) {
      ...
      if ($(container).length > 2) {
        throw new Error(
          "Toolbar Error: one element expected, multiple elements given."
        );
      }
    ...
    _setup: function() {
      ...
      $.cookie("CMS_toolbar-collapsed") == "false"
      ...
      if ($.browser.msie && $.browser.version <= "6.0") {
        $(window).bind("resize", function() {
          that.wrapper.css("width", $(window).width());
        });
    ...
    _registerSwitcher: function(obj) {
      ...
      // add events
      template
        .find(".cms_toolbar-item_switcher-link")
        .bind("click", function(e) {
          ...
          if (btn.data("state")) {
            btn
              .stop()
              .animate({ backgroundPosition: "-40px -198px" }, function() {
                // disable link
            ...
            btn
              .stop()
              .animate({ backgroundPosition: "0px -198px" }, function() {
                // enable link
                document.location = that.setUrl(document.location, {
    ...
    _registerList: function(obj) {
      ...
      $(obj.items).each(function(index, value) {
        ...
        var icon_styles = value.icon
          ? ' class="cms_toolbar_icon cms_toolbar_icon-enabled" style="background-image:url(' +
            value.icon +
            ');"'
          : "";
        ...
        // replace attributes
        tmp += list
          .replace("[list_title]", value.title)
            ...
            $(item)
              ...
              .bind("click", function(e) {
                ...
                $.ajax({
                  ...
                  error: function() {
                    throw new Error(
                      "CMS.Toolbar was unable to perform this ajax request. Try again or contact the developers."
                    );
                  }
                });
    ...
    _processTemplate: function(id, obj) {
      ...
      if (!obj.icon && obj.type === "button")
        template = template.replace("&nbsp;", "").replace("&nbsp;", "");
      // template = template.replace('[token]', this.options.csrf_token);
      template = obj.action
        ? template.replace("[action]", obj.action)
        : template.replace("[action]", "");
      template = obj.hidden
        ? template.replace("[hidden]", obj.hidden)
      ...
      if (obj.icon) {
        template
          .find(".cms_toolbar-btn_right .toolbar_icon-prefix")
