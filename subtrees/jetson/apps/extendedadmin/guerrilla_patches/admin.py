...
def patch_admin():
    def new_admin_field_init(self, form, field, is_first):
        ...
        self.has_markup_type = "hasMarkupType" in self.field.field.widget.attrs.get(
            'class', ""
        )
            ...
            if has_module_perms:
                ...
                if True in perms.values():
                    ...
                    if app_label in app_dict:
                        app_dict[app_label]['model_dict'][model.__name__
                                                         ] = model_dict
                    else:
    ...
    def adminsite_app_index(self, request, app_label, extra_context=None):
                ...
                for model_name in app_options["models"]:
                    if model_name in app_dict['model_dict']:
                        app_dict['models'].append(
                            app_dict['model_dict'][model_name]
                        )
            if "classes" in app_options:
        ...
        context.update(extra_context or {})
        return render_to_response(
            self.app_index_template or 'admin/app_index.html',
            context,
            context_instance=template.RequestContext(request)
        )
    ...
    class AdminAuthenticationForm(AuthenticationForm):
        ...
        this_is_the_login_form = forms.BooleanField(
            ...
            error_messages={
                'required':
                    _("Please log in again, because your session has expired.")
            }
        )
    ...
    class ExtendedChangeList(main.ChangeList):
        ...
        def __init__(
            self, request, model, list_display, list_display_links, list_filter,
            ...
            list_display_links = self.get_translated_list(
            ...
            if model_admin.ordering:
                model_admin.ordering = self.get_translated_list(
                    model, model_admin.ordering
                )
            elif model._meta.ordering:
                model_admin.ordering = self.get_translated_list(
                    model, model._meta.ordering
                )

            super(ExtendedChangeList, self).__init__(
