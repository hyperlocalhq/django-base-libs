...
class FaqCategory(MPTTModel, CreationModificationMixin, UrlMixin, SlugMixin()):
    ...
    faqs_on_separate_page = models.BooleanField(
        _('separate page'), 
        help_text=_("check, if you want to display relating faqs on a separate page. If this category is not a 'leaf category', it has no effect."),
        default=False,
        )
    ...
    def has_link(self):
        ...
        return not self.is_leaf_node() or (self.has_faqs() and self.faqs_on_separate_page)
