...
from jetson.apps.faqs.models import FaqContainer, FaqCategory, QuestionAnswer
...
class FaqContainerOptions(MultiSiteContainerMixinAdminOptions):
    ...
    list_display = (
        'title', 'get_sites', 'get_content_object_display', 'sysname'
    )
    list_filter = (
        "creation_date",
        "modified_date",
...
class QuestionAnswer_Inline(ExtendedStackedInline):
    ...
    fieldsets += get_admin_lang_section(
        _("Contents"), ['question', 'answer'], True
    )
...
class FaqCategoryAdminForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for lang_code, lang_name in settings.LANGUAGES:
            self.fields['description_%s_markup_type' %
                        lang_code].widget.attrs["class"] = "markupType"
...
class FaqCategoryOptions(TreeEditor):
    ...
    list_display = [
        'actions_column', 'indented_short_title', 'container', 'short_title',
        'questionanswer_count'
    ]
    list_filter = (
        'container', "creation_date", "creator", "modified_date", "modifier"
    )

    fieldsets = [
        (None, {
            'fields': ('container', 'parent', 'slug')
        }),
    ]
    fieldsets += get_admin_lang_section(
        _("Contents"), ['title', 'short_title', 'description']
    )

    _additional_fields = [
        'children_sort_order_format',
        'faqs_on_separate_page',
    ]
    # hackish way to display markup types for descriptions
    for lang_code, lang_name in settings.LANGUAGES:
...
class QuestionAnswerOptions(ExtendedModelAdmin):
    ...
    list_filter = (
        'category', "creation_date", "creator", "modified_date", "modifier"
    )
    ...
    prepopulated_fields = {
        "slug": ("question_%s" % settings.LANGUAGE_CODE, ),
    }
