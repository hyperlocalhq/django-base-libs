...
class img_replacement:
    ...
    def __init__(self, data):
        self.data = data
        self.data['id'] = base64.b64encode(
            smart_str(u''.join(self.data.values()))
        )
        self.data['size'] = int(self.data['size'])

        # Check if file exists
        if not path.isfile(
            path.join(self.image_path, self.data['id'] + ".png")
        ):
            ...
            f = ImageFont.truetype(
                path.join(self.font_path, data['font']), data['size']
            )
...
def image_replacement(value, info, autoescape=None):
    ...
    try:
        ...
        return mark_safe("%s <!-- %s -->" % (value, e))
    args['image_path'] = "%simage_replacement/images/%s.png" % (
        settings.MEDIA_URL, image['id']
    )
...
def image_replacement_path(value, info, autoescape=None):
    ...
    image = img_replacement(args)
    return "%simage_replacement/images/%s.png" % (
        settings.MEDIA_URL, image['id']
    )
