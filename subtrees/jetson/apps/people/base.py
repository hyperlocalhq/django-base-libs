...
STATUS_CHOICES = getattr(
    settings, "PERSON_STATUS_CHOICES", (
...
YEAR_OF_BIRTH_CHOICES = [
    (i, i) for i in range(tz_now().year - 16,
                          tz_now().year - 100, -1)
]
...
class PersonManager(models.Manager):
    ...
    def latest_published(self):
        return self.filter(status="published", ).order_by("-user__date_joined")
...
class PersonBase(CreationModificationDateMixin, UrlMixin):
    ...
    individual_type = TreeForeignKey(
        IndividualType, verbose_name=_("Type"), blank=True, null=True
    )
    prefix = models.ForeignKey(
        Prefix, verbose_name=_("Prefix"), null=True, blank=True
    )
    salutation = models.ForeignKey(
        Salutation, verbose_name=_("Salutation"), null=True, blank=True
    )
    person_repr = models.CharField(
        _("Person Representation"), max_length=200, blank=True, editable=False
    )
    nickname = models.CharField(_("Nickname"), max_length=200, blank=True)
    birthname = models.CharField(
        _("Birth / Maiden name"), max_length=200, blank=True
    )
    gender = models.CharField(
        _("Gender"), max_length=1, choices=GENDER_CHOICES, blank=True
    )
    birthday_yyyy = models.IntegerField(
        _("Year of Birth"),
        blank=True,
        null=True,
        choices=YEAR_OF_BIRTH_CHOICES
    )
    birthday_mm = models.SmallIntegerField(
        _("Month of Birth"), blank=True, null=True, choices=MONTH_CHOICES
    )
    birthday_dd = models.SmallIntegerField(
        _("Day of Birth"), blank=True, null=True, choices=DAY_CHOICES
    )
    # can have choices
    ...
    occupation = models.CharField(
        _("Current Occupation"), max_length=200, blank=True
    )
    ...
    context_categories = TreeManyToManyField(
        ...
        limit_choices_to={'is_applied4person': True},
        blank=True
    )
    ...
    timezone = models.ForeignKey(
        "i18n.TimeZone",
        verbose_name=_("Current Timezone"),
        max_length=200,
        blank=True,
        null=True
    )
    ...
    display_birthday = models.BooleanField(
        _("Display birthday to public"), default=True
    )
    display_email = models.BooleanField(
        _("Display email address to public"), default=False
    )
    display_address = models.BooleanField(
        _("Display address data to public"), default=True
    )
    display_phone = models.BooleanField(
        _("Display phone numbers to public"), default=True
    )
    display_fax = models.BooleanField(
        _("Display fax numbers to public"), default=True
    )
    display_mobile = models.BooleanField(
        _("Display mobile phones to public"), default=True
    )
    display_im = models.BooleanField(
        _("Display instant messengers to public"), default=True
    )

    display_username = models.BooleanField(
        _("Display user name instead of full name"), default=False
    )
    allow_search_engine_indexing = models.BooleanField(
        _("Allow indexing by search engines"), default=True
    )
    ...
    def get_spoken_languages(self):
        from jetson.apps.i18n.models import Language
        sl_db_table = self._meta.get_field("spoken_languages",
    ...
    def get_contacts(self, cache=True):
        if not hasattr(self, "_contacts_cache") or not cache:
            self._contacts_cache = self.individualcontact_set.order_by(
                '-is_primary', 'id'
            )
    ...
    def is_birthday_displayed(self, user=None):
        if not hasattr(self, "_is_birthday_displayed_cache"):
            ...
            self._is_birthday_displayed_cache = bool(
                self.display_birthday or
                user.has_perm("people.can_see_birthday", self)
            )
    ...
    def is_email_displayed(self, user=None):
        if not hasattr(self, "_is_email_displayed_cache"):
            ...
            self._is_email_displayed_cache = bool(
                self.display_email or
                user.has_perm("people.can_see_email", self)
            )
    ...
    def is_address_displayed(self, user=None):
        if not hasattr(self, "_is_address_displayed_cache"):
            ...
            self._is_address_displayed_cache = bool(
                self.display_address or
                user.has_perm("people.can_see_addresses", self)
            )
    ...
    def is_phone_displayed(self, user=None):
        if not hasattr(self, "_is_phone_displayed_cache"):
            ...
            self._is_phone_displayed_cache = bool(
                self.display_phone or
                user.has_perm("people.can_see_phones", self)
            )
    ...
    def is_fax_displayed(self, user=None):
        if not hasattr(self, "_is_fax_displayed_cache"):
            ...
            self._is_fax_displayed_cache = bool(
                self.display_fax or user.has_perm("people.can_see_faxes", self)
            )
    ...
    def is_mobile_displayed(self, user=None):
        if not hasattr(self, "_is_mobile_displayed_cache"):
            ...
            self._is_mobile_displayed_cache = bool(
                self.display_mobile or
                user.has_perm("people.can_see_mobiles", self)
            )
    ...
    def is_im_displayed(self, user=None):
        if not hasattr(self, "_is_im_displayed_cache"):
            ...
            self._is_im_displayed_cache = bool(
                self.display_im or user.has_perm("people.can_see_ims", self)
            )
...
class IndividualContactBase(models.Model):
    ...
    location_type = models.ForeignKey(
        ...
        default=get_default_ind_loc_type
    )
    ...
    institutional_title = models.CharField(
        _("Title in the institution"),
        max_length=255,
        blank=True,
        help_text=_('i.e. "director", "manager", "student", "doctor", etc.')
    )
                ...
                if phone_number and not is_on_hold:
                    if (
                        not phone_type or getattr(
                            self.person, "is_%s_displayed" % phone_type.slug,
                            lambda: True
                        )()
                    ):
        ...
        if not hasattr(self, '_ims_cache'):
            if self.person.is_im_displayed():
                self._ims_cache = [
                    ...
                    if getattr(self, "im%d_address" % pos) and
    ...
    def get_vcard(self):
        ...
        v.n.value = vobject.vcard.Name(
            family=self.person.get_last_name(),
            given=self.person.get_first_name()
        )
        v.add('fn')
        v.fn.value = "%s %s" % (
            self.person.get_first_name(), self.person.get_last_name()
        )
        v.add('email')
