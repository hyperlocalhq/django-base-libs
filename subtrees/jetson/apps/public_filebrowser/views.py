...
def get_user_path(user, relative_directory=""):
    ...
    return get_path(accessible_directories[0].accessible_root.path + "/" + relative_directory)
...
def browse(request):
        ...
        if append:
            try:
                ...
                if fileobject.path.startswith(user_path_prefix):
                    fileobject.public_path_relative_directory = fileobject.path[len(user_path_prefix):]
    ...
    return render_to_response('public_filebrowser/index.html', {
        'dir': path,
        'p': p,
        'page': page,
...
def mkdir(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # remove pagination
                redirect_url = reverse("pfb_browse") + query_helper(query, "ot=desc,o=date", "ot,o,filter_type,filter_date,q,p")
                return HttpResponseRedirect(redirect_url)
    ...
    return render_to_response('public_filebrowser/makedir.html', {
        'form': form,
        'query': query,
...
def upload(request):
    ...
    return render_to_response('public_filebrowser/upload.html', {
        'query': query,
...
def delete(request):
    ...
    query = request.GET
    path = get_user_path(request.user, query.get('dir', ''))
    dir_name = query.get('dir', '')  # TODO: find out what exactly dir_name has to be
    ...
    if request.GET:
        if request.GET.get('filetype') != "Folder":
            ...
            try:
                ...
                messages.success(request, msg)
                redirect_url = reverse("pfb_browse") + query_helper(query, "", "filename,filetype")
                return HttpResponseRedirect(redirect_url)
            except OSError:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.success(request, msg)
                redirect_url = reverse("pfb_browse") + query_helper(query, "", "filename")
                return HttpResponseRedirect(redirect_url)
            except OSError, (errno, strerror):
                form.errors['name'] = forms.util.ErrorList([_('Error.')])
    else:
        form = RenameForm(abs_path, file_extension, initial={
            'name': os.path.splitext(filename)[0].lower()
            })
    
    return render_to_response('public_filebrowser/rename.html', {
        'form': form,
        'query': query,
...
def versions(request):
    ...
    return render_to_response('public_filebrowser/versions.html', {
...
def download(request):
    ...
    response['Content-Length'] = os.path.getsize(os.path.join(abs_path, filename))
