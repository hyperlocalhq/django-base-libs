...
ImageModificationGroup = models.get_model(
...
class ImageModificationOptions(admin.ModelAdmin):
    ...
    fieldsets = [
        (None, {
            'fields': ('sysname', 'title', 'group', 'notes')
        }),
        (
            _('Dimensions'), {
        ...
        (
            _('Filters'), {
                'fields':
                    ('color', 'brightness', 'contrast', 'sharpness', 'filters'),
...
class ImageCroppingOptions(admin.ModelAdmin):
    ...
    list_filter = ("mods", )
    list_display = [
        'original', 'coords', 'cropping_size', 'original_size', 'bgcolor',
        'mods_list'
    ]
