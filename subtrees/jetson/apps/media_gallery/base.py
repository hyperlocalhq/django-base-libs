...
MEDIA_FILE_TYPE_RECOGNITION = (
    (
        re.compile(r"^https?://.*?youtube\.com/watch\?v=|https?://youtu.be/"),
        "y"
    ),
...
class MediaGalleryManager(models.Manager):
    ...
    def get_queryset(self):
        ...
        if is_installed("comments.models"):
            ...
            select[
                'comments'
            ] = """SELECT COUNT(*) from {} WHERE content_type_id = '{}' AND object_id = CAST({}.id AS CHAR)""".format(
                comments_db_table, ct.id, gallery_db_table
            )
        if is_installed("favorites.models"):
            ...
            select[
                ...
                favorites_db_table, ct.id, gallery_db_table
            )
        if select:
            q = q.extra(select=select, )
        return q
...
class MediaGalleryBase(
    ObjectRelationMixin(is_required=True), CreationModificationDateMixin,
    ViewsMixin, UrlMixin
):
    title = MultilingualCharField(_("Title"), max_length=100, blank=True)
    description = MultilingualTextField(_("Description"), blank=True)
    content_object_repr = MultilingualCharField(
        _("Content object representation"),
        max_length=100,
        blank=True,
        default="",
    ...
    content_object_id = models.CharField(
        _("Content object ID combo"),
        max_length=100,
        blank=True,
        default="",
        help_text=_("Used for grouping the sortable galleries."),
        editable=False
    )
    ...
    sort_order = PositionField(
        _("Sort order"), collection="content_object_id", default=0
    )
    ...
    def save(self, **kwargs):
        if self.content_object:
            ...
            for lang_code, lang_name in settings.LANGUAGES:
                ...
                setattr(
                    self, "content_object_repr_%s" % lang_code,
...
class MediaFileBase(CreationModificationMixin):
    gallery = models.ForeignKey(
        "media_gallery.MediaGallery", verbose_name=_("Media Gallery")
    )
    ...
    path = FileBrowseField(
        ...
        max_length=255,
        blank=True,
        help_text=_("A path to a locally stored image, video, or audio file.")
    )

    external_url = URLField(
        _('External URL'),
        help_text=_("A URL of an external image, video, or audio file."),
        blank=True
    )

    splash_image_path = FileBrowseField(
        _('Splash-image path'),
        max_length=255,
        extensions=['.jpg', '.jpeg', '.gif', '.png'],
        blank=True,
        help_text=_(
            "Used for still images in Flash players and for thumbnails."
        )
    )

    file_type = models.CharField(
        max_length=1,
        choices=MEDIA_FILE_TYPE_CHOICES,
        default="-",
        editable=False
    )
            ...
            if os.path.exists(abs_path):
                try:
                    path, query_params = image_mods.FileManager.modified_path(
                        self.splash_image_path.path, "gt"
                    )
                except:
                    pass
                else:
            ...
            if os.path.exists(abs_path):
                try:
                    url = "".join(
                        (
                            UPLOADS_URL,
                            image_mods.FileManager.modified_path(
                                self.splash_image_path.path, "glist"
                            ),
                        )
                    )
                except:
    ...
    def get_splash_image_url(self):
        if self.splash_image_path:
            ...
            if os.path.exists(abs_path):
                try:
                    modified_path, query = image_mods.FileManager.modified_path(
                        self.splash_image_path.path, "gd"
                    )
        ...
        if self.file_type == "i":
            if self.path:
                return render_to_string(
                    "media_gallery/media_file_types/uploaded_image%s.html" %
                    suffix,
                    context_dict,
                )
            if self.external_url:
                return render_to_string(
                    "media_gallery/media_file_types/external_image%s.html" %
                    suffix,
                    context_dict,
                )
        if self.file_type == "a":
            if self.path:
                return render_to_string(
                    "media_gallery/media_file_types/uploaded_audio%s.html" %
                    suffix,
                    context_dict,
                )
            if self.external_url:
                return render_to_string(
                    "media_gallery/media_file_types/external_audio%s.html" %
                    suffix,
                    context_dict,
                )
        if self.file_type == "v":
            if self.path:
                return render_to_string(
                    "media_gallery/media_file_types/uploaded_video%s.html" %
                    suffix,
                    context_dict,
                )
            if self.external_url:
                return render_to_string(
                    "media_gallery/media_file_types/external_video%s.html" %
                    suffix,
                    context_dict,
                )
        if self.file_type == "y":
            if self.external_url:
                ...
                if match:
                    return render_to_string(
                        "media_gallery/media_file_types/youtube_video%s.html" %
                        suffix,
                        dict(youtube_video_id=match.group(2), **context_dict)
                    )
        if self.file_type == "m":
            if self.external_url:
                match = re.compile(r"^https?://.*?vimeo\.com/(\d+)", ).match(
                    self.external_url
                )
                if match:
                    return render_to_string(
                        "media_gallery/media_file_types/vimeo_video%s.html" %
                        suffix,
