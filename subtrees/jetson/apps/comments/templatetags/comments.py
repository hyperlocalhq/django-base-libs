...
class CommentFormNode(template.Node):
    def __init__(
        self,
        template_dir_lookup_var,
        content_type_lookup_var,
        content_type,
        obj_id_lookup_var,
        obj_id,
        photos_optional=False,
        photos_required=False,
        photo_options='',
        ratings_optional=False,
    ...
    def render(self, context):
        ...
        if self.content_type_lookup_var is not None:
            try:
                ct_string = template.resolve_variable(
                    self.content_type_lookup_var, context
                )
        ...
        if self.obj_id_lookup_var is not None:
            try:
                self.obj_id = template.resolve_variable(
                    self.obj_id_lookup_var, context
                )
        ...
        for var, abbr in (
            ('photos_required',
             PHOTOS_REQUIRED), ('photos_optional', PHOTOS_OPTIONAL),
            ('ratings_required', RATINGS_REQUIRED),
            ('ratings_optional', RATINGS_OPTIONAL), ('is_public', IS_PUBLIC)
        ):
        ...
        context['rating_options'] = normalize_newlines(
            base64.encodestring(self.rating_options).strip()
        )
        if self.rating_options:
            context['rating_range'], context[
                'rating_choices'] = Comment.objects.get_rating_options(
                    self.rating_options
                )
        context['hash'] = Comment.objects.get_security_hash(
            context['options'], context['photo_options'],
            context['rating_options'], context['target']
        )
        ...
        comment_form.fields['prevent_spam'
                           ].initial = security_field.generate_value()
...
class CommentListNode(template.Node):
    ...
    def render(self, context):
        ...
        comment_list = get_list_function(
            **kwargs
        ).order_by(self.sort_order + 'submit_date').select_related()
        ...
        if getattr(settings, 'COMMENTS_BANNED_USERS_GROUP', ''):
            comment_list = [
                ...
                if not c.is_hidden or (user_id == c.user_id)
            ]
...
class DoCommentForm:
    ...
    def __call__(self, parser, token):
        ...
        if len(tokens) < 4:
            raise template.TemplateSyntaxError, "%r tag requires at least 3 arguments" % tokens[
                0]
        token_index = 1
        ...
        if tokens[token_index] != 'for':
            raise template.TemplateSyntaxError, "Argument %d in %r tag must be 'for'" % (
                token_index + 1, tokens[0]
            )

        # ok, maybe the content_type is in the form [pkg].[py_module_name]
        # or it is a context variable name!!!
        content_type_lookup_var, content_type, package, module = None, None, None, None
        try:
            ...
            raise template.TemplateSyntaxError, "%r tag has invalid content-type '%s.%s'" % (
                tokens[0], package, module
            )
        ...
        if tokens[token_index + 2].isdigit():
            ...
            try:  # ensure the object ID is valid
                ...
                raise template.TemplateSyntaxError, "%r tag refers to %s object with ID %s, which doesn't exist" % (
                    tokens[0], content_type.name, obj_id
                )
        else:
            obj_id_lookup_var = tokens[token_index + 2]

        if (tokens[1] == 'using' and
            len(tokens) > 6) or (tokens[1] != 'using' and len(tokens) > 4):
            if tokens[token_index + 3] != 'with':
                raise template.TemplateSyntaxError, "Fourth argument in %r tag must be 'with'" % tokens[
                    0]
            for option, args in zip(
                tokens[token_index + 4::2], tokens[token_index + 5::2]
            ):
                if option in ('photos_optional', 'photos_required'):
                    # VALIDATION ##############################################
                    option_list = args.split(',')
                    if len(option_list) % 3 != 0:
                        raise template.TemplateSyntaxError, "Incorrect number of comma-separated arguments to %r tag" % tokens[
                            0]
                    for opt in option_list[::3]:
                        if not opt.isalnum():
                            raise template.TemplateSyntaxError, "Invalid photo directory name in %r tag: '%s'" % (
                                tokens[0], opt
                            )
                    for opt in option_list[1::3] + option_list[2::3]:
                        if not opt.isdigit() or not (
                            MIN_PHOTO_DIMENSION <= int(opt) <=
                            MAX_PHOTO_DIMENSION
                        ):
                            raise template.TemplateSyntaxError, "Invalid photo dimension in %r tag: '%s'. Only values between %s and %s are allowed." % (
                                tokens[0], opt, MIN_PHOTO_DIMENSION,
                                MAX_PHOTO_DIMENSION
                            )
                    # VALIDATION ENDS #########################################
                    kwargs[option] = True
                    ...
                    if 2 < len(args.split('|')) > 9:
                        raise template.TemplateSyntaxError, "Incorrect number of '%s' options in %r tag. Use between 2 and 8." % (
                            option, tokens[0]
                        )
                    if re.match('^scale:\d+\-\d+:$', args.split('|')[0]):
                        raise template.TemplateSyntaxError, "Invalid 'scale' in %r tag's '%s' options" % (
                            tokens[0], option
                        )
                    # VALIDATION ENDS #########################################
                    kwargs[option] = True
                    ...
                    raise template.TemplateSyntaxError, "%r tag got invalid parameter '%s'" % (
                        tokens[0], option
                    )
        return CommentFormNode(
            template_dir_lookup_var, content_type_lookup_var, content_type,
            obj_id_lookup_var, obj_id, **kwargs
        )
...
class DoCommentCount:
    ...
    def __call__(self, parser, token):
        ...
        if len(tokens) != 6:
            raise template.TemplateSyntaxError, "%r tag requires 5 arguments" % tokens[
                0]
        if tokens[1] != 'for':
        ...
        try:
            ...
            raise template.TemplateSyntaxError, "Third argument in %r tag must be in the format 'package.module'" % tokens[
                0]
        try:
            content_type = ContentType.objects.get(
                app_label__exact=package,
        ...
        if tokens[3].isdigit():
            ...
            var_name = tokens[3]
        if tokens[4] != 'as':
            raise template.TemplateSyntaxError, "Fourth argument in %r must be 'as'" % tokens[
                0]
...
class DoGetCommentList:
    ...
    def __call__(self, parser, token):
        ...
        if not len(tokens) in (6, 7):
            raise template.TemplateSyntaxError, "%r tag requires 5 or 6 arguments" % tokens[
                0]
        if tokens[1] != 'for':
            raise template.TemplateSyntaxError, "Second argument in %r tag must be 'for'" % tokens[
                0]
        ...
        if len(tokens) == 7:
            if tokens[6] != 'reversed':
                raise template.TemplateSyntaxError, "Final argument in %r must be 'reversed' if given" % tokens[
                    0]
            sort_order = "-"
        else:
            sort_order = ""
        return CommentListNode(
            package, module, var_name, obj_id, tokens[5], sort_order
        )
