...
PersonGroupObjectRelation = ObjectRelationMixin(
    limit_content_type_choices_to={"model__in": ("address", "institution")},
)


class PersonGroupBase(
    CreationModificationDateMixin, PersonGroupObjectRelation, UrlMixin,
    SlugMixin(
    ...
    group_type = TreeForeignKey(
        GroupType, verbose_name=_("Group Type"), related_name="type_groups"
    )
    ...
    is_by_invitation = models.BooleanField(
        _("Membership by Invitation"), default=False
    )
    is_by_confirmation = models.BooleanField(
        _("Membership by Confirmation"), default=False
    )

    context_categories = TreeManyToManyField(
        ContextCategory,
        verbose_name=_("Context categories"),
        limit_choices_to={'is_applied4persongroup': True},
        blank=True
    )
    ...
    image = FileBrowseField(
        _('Image'),
        max_length=255,
        directory="/%s/" % URL_ID_PERSONGROUPS,
    ...
    def create_permission_roles(self):
        ...
        for role_name, permissions in PG_ROLE_PERMISSIONS.items():
            role = PerObjectGroup(sysname=role_name, )
    ...
    def get_owners(self):
        ...
        members = self.get_all_members().extra(
            where=("(groups_networks_groupmembership.role = 'owners')", )
        )
        members.sort_order_map = None
        members.sort_order_mapper = {
            'alphabetical_asc':
    ...
    def get_moderators(self):
        """ get all moderators """
        members = self.get_all_members().extra(
            where=('(groups_networks_groupmembership.role = "moderators")', )
        )
        members.sort_order_map = None
        members.sort_order_mapper = {
            'alphabetical_asc':
    ...
    def get_members(self):
        ...
        members = self.get_all_members().extra(
            where=('(groups_networks_groupmembership.role = "members")', )
        )
        members.sort_order_map = None
        members.sort_order_mapper = {
            'alphabetical_asc':
    ...
    def are_members_shown(self):
        ...
        return bool(
            user.has_perm("groups_networks.change_persongroup", self) or
            self.get_all_members().filter(user=user) or
    ...
    def is_membership_requestable(self, user=None):
        ...
        return bool(
            (
                self.is_by_confirmation or
                not (self.is_by_invitation or self.is_by_confirmation)
            ) and not self.get_all().filter(user=user)
        )

    def is_member_request_cancelable(self, user=None):
        user = get_current_user(user)
        return bool(self.get_unconfirmed().filter(user=user))

    def is_member_request_acceptable(self, user=None):
        current_user = get_current_user()
        return bool(
            current_user != user and
            self.get_unconfirmed().filter(user=user) and
    ...
    def is_member_invitable(self, user=None):
        ...
        return bool(
            (
                self.is_by_invitation or
                not (self.is_by_invitation or self.is_by_confirmation)
            ) and self.get_owners().filter(user=current_user) and
            not self.get_all().filter(user=user)
        )
    ...
    def is_member_invitation_cancelable(self, user=None):
        ...
        current_user = get_current_user()
        return bool(
            self.get_owners().filter(user=current_user) and
            self.get_invited().filter(user=user)
        )
    ...
    def is_membership_editable(self, user=None):
        user = get_current_user(user)
        return bool(
            self.get_all_members().filter(user=user) and
            not self.get_owners().filter(user=user)
        )
    ...
    def is_addable_to_memos(self, user=None):
        ...
        return bool(
            user.has_perm("groups_networks.change_persongroup", self) or
            self.get_all_members().filter(user=user) or self.access_type in (
                "public",
                "private",
            )
        )
...
class GroupMembershipBase(models.Model):
    ...
    role = models.CharField(
        _("Role"), choices=PG_ROLE_CHOICES, max_length=30, default='members'
    )
    title = MultilingualCharField(_("Title"), max_length=255, blank=True)

    inviter = models.ForeignKey(
        User,
        related_name="group_inviter",
    ...
    timestamp = models.DateTimeField(
        _("Timestamp"), auto_now_add=True, editable=False, null=True
    )
    ...
    def get_title(self, language=None):
        ...
        return force_unicode(
            getattr(self, "title_%s" % language, "") or self.title
        )
