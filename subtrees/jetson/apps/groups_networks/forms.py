...
GROUP_TYPE_CHOICES = XChoiceList(
    get_related_queryset(PersonGroup,
                         'group_type').exclude(slug="institutional"),
...
class GroupMembershipForm(dynamicforms.Form):
    ...
    def __init__(self, relation_action, user, group, **kwargs):
        ...
        self.group = group
        try:
            person = group.get_all().get(user=user)
        except:
            pass
        else:
...
class GroupAddingForm:  # Namespace
    class MainDataForm(dynamicforms.Form):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fields['institution'].queryset = get_current_user(
            ).profile.get_institutions()
    ...
    class CategoriesForm(dynamicforms.Form):
        ...
        def __init__(self, *args, **kwargs):
            super(GroupAddingForm.CategoriesForm,
    ...
    def save_data(form_steps, form_step_data):
        ...
        group = PersonGroup(
            ...
            slug=get_unique_value(
                PersonGroup,
                better_slugify(form_step_data[0].get('title',
                                                     '')).replace("-", "_"),
                separator="_"
            ),
            ...
            preferred_language=get_related_queryset(
                PersonGroup, "preferred_language"
            ).get(pk=form_step_data[0].get('main_language', ''), ),
            description_en=form_step_data[2].get('description_en', ""),
            description_de=form_step_data[2].get('description_de', ""),
        )
        group.organizing_institution = form_step_data[0].get(
            'institution', None
        )
        ...
        group.is_by_invitation = membership_options in (
            "invite", "invite_or_confirm"
        )
        ...
        if media_file_path:
            ...
            image_mods.FileManager.save_file_for_object(
                group, filename, f.read(), subpath="avatar/"
            )
            f.close()
    ...
    0:
        {
            'title':
                _("main data"),
            'template':
                "groups_networks/persongroups/add_group_main_data.html",
            'form':
                GroupAddingForm.MainDataForm,
        },
    1:
        {
            'title':
                _("categories"),
            'template':
                "groups_networks/persongroups/add_group_categories.html",
            'form':
                GroupAddingForm.CategoriesForm,
        },
    2:
        {
...
class Invitation:  # Namespace
    class InstitutionInvitedPeople(dynamicforms.Form):
        ...
        def __init__(self, *args, **kwargs):
            super(Invitation.InstitutionInvitedPeople,
                  self).__init__(*args, **kwargs)
    ...
    def submit_step(current_step, form_steps, form_step_data):
        if current_step == 0:
            for i in range(1, 6):
                ...
                if email:
                    try:
                        p = form_step_data[current_step]["p%d" % i
                                                        ] = Person.objects.get(
                                                            user__email=email,
                                                        )
                    except:
                        ...
                        if p.prefix_id:
                            form_step_data[current_step]["p%d_prefix" % i
                                                        ] = p.prefix_id
                            form_step_data[current_step][
                                "get_p%d_prefix_display" % i] = force_unicode(
                                    p.prefix.get_title()
                                )
                        form_step_data[current_step]["p%d_first_name" % i
                                                    ] = p.user.first_name
                        form_step_data[current_step]["p%d_last_name" % i
                                                    ] = p.user.last_name
        return form_step_data
        ...
        for i in range(1, 6):
            ...
            if not (p and group.get_all_members().filter(user=p.user)):
                if p:
                    ...
                    form_step_data[0].get('p%d_first_name' % i, False) and
                    form_step_data[0].get('p%d_last_name' % i, False) and
                    form_step_data[0].get('p%d_email' % i, False)
                ):
                    # create user
                ...
                if p:
                    individual_contact = p.individualcontact_set.create(
                        ...
                        phone0_country=form_step_data[0].get(
                            'p%d_phone_country' % i, ""
                        ),
                        phone0_area=form_step_data[0].get(
                            'p%d_phone_area' % i, ""
                        ),
                        phone0_number=form_step_data[0].get(
                            'p%d_phone_number' % i, ""
                        ),
                        phone1_type=PhoneType.objects.get(slug='fax'),
                        phone1_country=form_step_data[0].get(
                            'p%d_fax_country' % i, ""
                        ),
                        phone1_area=form_step_data[0].get(
                            'p%d_fax_area' % i, ""
                        ),
                        phone1_number=form_step_data[0].get(
                            'p%d_phone_number' % i, ""
                        ),
                        phone2_type=PhoneType.objects.get(slug='mobile'),
                        phone2_country=form_step_data[0].get(
                            'p%d_mobile_country' % i, ""
                        ),
                        phone2_area=form_step_data[0].get(
                            'p%d_mobile_area' % i, ""
                        ),
                        phone2_number=form_step_data[0].get(
                            'p%d_mobile_number' % i, ""
                        ),
    ...
    1:
        {
            'title':
                _("Confirm Data"),
            'template':
                "groups_networks/institution_invitation_confirmation.html",
            'form':
                forms.Form,  # dummy form
        },
