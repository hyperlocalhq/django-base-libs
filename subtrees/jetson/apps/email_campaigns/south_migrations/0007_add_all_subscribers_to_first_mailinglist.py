...
class Migration:
    ...
    def forwards(self, orm):
        try:
            mailinglist = orm.MailingList.objects.all()[0]
        except IndexError:
            try:
                campaign = orm.Campaign.objects.all()[0]
            except IndexError:
                campaign = orm.Campaign()
                for lang in settings.LANGUAGES:
                    setattr(campaign, 'title_%s' % lang[0], 'Dummy Campaign')
                campaign.save()
            mailinglist = orm.MailingList()
            mailinglist.campaign = campaign
            for lang in settings.LANGUAGES:
                setattr(mailinglist, 'title_%s' % lang[0], 'Dummy Mailing List')
            mailinglist.save()
        
        for i in orm.InfoSubscription.objects.all():
            i.mailinglist = mailinglist
            i.save()
    
    
    def backwards(self, orm):
        # Do nothing here because "mailinglist" is a required field that may not be blank/null
        pass
    ...
    models = {
        ...
        'email_campaigns.mailing': {
            ...
            'template': ('TemplatePathField', [], {'path': "'email_campaigns/mailing/'", 'match': "r'\\.html$'"})
        },
