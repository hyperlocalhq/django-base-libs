...
HAS_LANG_PREFIX_RE = re.compile(r"^/(%s)/.*" % "|".join(map(lambda l: l[0], settings.LANGUAGES)))
...
class MultilingualURLMiddleware:
    ...
    def process_request(self, request):
        ...
        if not has_lang_prefix(path) and (True not in (path.startswith(p) for p in PATHS_NO_REDIRECTION)):
