...
from ccb.apps.site_specific.feeds import BrowseRssFeed, BrowseAtomFeed
...
urlpatterns = patterns('',
    ...
    url(r'^ticket/(?P<concern>[^/]+)/$', _project_name + '.apps.tracker.views.create_ticket', name="create_ticket"),
    url(r'^ticket/(?P<content_type_id>[0-9]+)/(?P<object_id>[0-9]+)/$', _project_name + '.apps.tracker.views.create_ticket', name="create_ticket"),
    url(r'^ticket/(?P<concern>[^/]+)/(?P<content_type_id>[0-9]+)/(?P<object_id>[0-9]+)/$', _project_name + '.apps.tracker.views.create_ticket', name="create_ticket"),
    ...
    url(r'^creative-sector/[^/]+/(?P<ot_url_part>%s|%s|%s|%s|%s)/simplesearch/$' % (URL_ID_DOCUMENTS, URL_ID_EVENTS, URL_ID_PERSONGROUPS, URL_ID_INSTITUTIONS, URL_ID_PEOPLE), _project_name + '.apps.search.views.simplesearch', simplesearch_dict, name="cs_model_specific_simple_search"),
    
    # info vCard
    url(r'^vcard/(?P<content_type_id>[0-9]+)/(?P<object_id>[0-9]+)/$', 'ccb.apps.site_specific.views.get_vcard', name="vcard"),
)

urlpatterns += patterns('django.views.static',
    # media
    url(r'^uploads/(?P<path>.*)$', 'serve', {'document_root': settings.UPLOADS_ROOT}, name="uploads_url"),
    ...
    url(r'^jetson-media/\d+/(?P<path>.*)$', 'serve', {'document_root': settings.JETSON_MEDIA_ROOT}, name="jetson_media_url"),
    url(
...
urlpatterns += patterns('',
    ...
    url(r'^helper/edit-(?P<object_type>%s|%s)-profile/(?P<slug>[^/]+)/(?P<section_name>contact)/$' % (URL_ID_JOB_OFFER, URL_ID_EVENT), 'ccb.apps.site_specific.views.edit_profile'),
    url(r'^helper/edit-(?P<object_type>%s|%s)-profile/(?P<slug>[^/]+)/(?P<section_name>contact)/(?P<index>[^/]+)/$' % (URL_ID_INSTITUTION, URL_ID_PERSON), 'ccb.apps.site_specific.views.edit_profile'),
...
urlpatterns += patterns('',
    ...
    url(r'^%s/(?P<slug>[^/]+)/$' % URL_ID_INSTITUTION, object_detail, institution_details_info),
    url(r'^%s/(?P<slug>[^/]+)/map/$' % URL_ID_INSTITUTION, object_detail, dict(institution_details_info, template_name="institutions/institution_map.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/$' % URL_ID_INSTITUTION, object_detail, dict(institution_details_info, template_name="institutions/institution_network.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/staff/$' % URL_ID_INSTITUTION, _project_name + '.apps.institutions.views.institution_staff_list', dict(person_list_info, template_name="institutions/institution_staff.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/partners/$' % URL_ID_INSTITUTION, _project_name + '.apps.institutions.views.institution_partners_list', dict(institution_list_info, template_name="institutions/institution_partners.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/%s/$' % (URL_ID_INSTITUTION, URL_ID_PERSONGROUPS), _project_name + '.apps.institutions.views.institution_groups_list', dict(group_list_info, template_name="institutions/institution_groups.html")),
    ...
    url(r'^%s/(?P<slug>[^/]+)/projects/$' % URL_ID_INSTITUTION, object_detail, dict(institution_details_info, template_name="institutions/institution_projects.html")),
    ...
    url(r'^%s/(?P<slug>[^/]+)/reviews/$' % URL_ID_INSTITUTION, object_detail, dict(institution_details_info, template_name="institutions/institution_reviews.html")),
    url(r'^%s/(?P<slug>[^/]+)/message/$' % URL_ID_INSTITUTION, 'jetson.apps.messaging.views.contact', dict(institution_details_info, template_name='institutions/institution_message.html')),
    url(r'^%s/(?P<slug>[^/]+)/message/alldone/$' % URL_ID_INSTITUTION, 'jetson.apps.messaging.views.contact_done', dict(institution_details_info, template_name='institutions/institution_message.html')),
 
    url(r'^%s/(?P<slug>[^/]+)/invite/' % URL_ID_INSTITUTION, 'ccb.apps.groups_networks.views.invite_institution_members',
     ),
    
    url(r'^%s/$' % URL_ID_DOCUMENTS, _project_name + '.apps.resources.views.document_list', dict(list_filter=_document_list_filter, **document_list_info)),
    url(r'^%s/(?P<show>favorites|memos|cultural-funding|scholarship|support-programme|information-founders|other)/$' % URL_ID_DOCUMENTS, _project_name + '.apps.resources.views.document_list', dict(list_filter=_document_list_filter, **document_list_info)),
    
    url(r'^%s/(?P<slug>[^/]+)/$' % URL_ID_DOCUMENT, object_detail, document_details_info),
    url(r'^%s/(?P<slug>[^/]+)/reviews/$' % URL_ID_DOCUMENT, object_detail, dict(document_details_info, template_name="resources/documents/document_reviews.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/$' % URL_ID_DOCUMENT, object_detail, dict(document_details_info, template_name="resources/documents/document_network.html")),
    ...
    url(r'^%s/(?P<slug>[^/]+)/map/$' % URL_ID_PERSON, object_detail, dict(person_details_info, template_name="people/person_map.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/$' % URL_ID_PERSON, object_detail, dict(person_details_info, template_name="people/person_network.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/person_contacts/$' % URL_ID_PERSON, _project_name + '.apps.people.views.person_person_contacts_list', dict(person_list_info, template_name="people/person_person_contacts.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/institution_contacts/$' % URL_ID_PERSON, _project_name + '.apps.people.views.person_institution_contacts_list', dict(institution_list_info, template_name="people/person_institution_contacts.html")),
    url(r'^%s/(?P<slug>[^/]+)/network/%s/$' % (URL_ID_PERSON, URL_ID_PERSONGROUPS), _project_name + '.apps.people.views.person_groups_list', dict(group_list_info, template_name="people/person_groups.html")),
    ...
    url(r'^%s/(?P<slug>[^/]+)/projects/$' % URL_ID_PERSON, object_detail, dict(person_details_info, template_name="people/person_projects.html")),
    url(
        r'^%s/(?P<slug>[^/]+)/%s/'
        r'('
            r'(?P<start_date>\d{8})'
    ...
    url(r'^%s/(?P<slug>[^/]+)/reviews/$' % URL_ID_PERSON, object_detail, dict(person_details_info, template_name="people/person_reviews.html")),
    url(r'^%s/(?P<slug>[^/]+)/message/$' % URL_ID_PERSON, 'jetson.apps.messaging.views.contact', dict(person_details_info, template_name='people/person_message.html')),
    url(r'^%s/(?P<slug>[^/]+)/message/alldone/$' % URL_ID_PERSON, 'jetson.apps.messaging.views.contact_done', dict(person_details_info, template_name='people/person_message.html')),

    url(r'^%s/$' % URL_ID_PERSONGROUPS, 'ccb.apps.groups_networks.views.persongroup_list', group_list_info),
    url(
    ...
    url(r'^%s/invitations/$' % URL_ID_PERSONGROUPS, 'ccb.apps.groups_networks.views.persongroup_invitation_list'),
    url(r'^%s/invitations/(?P<group_ind>my-%s|other-%s|invite-to-%s)/$' % (URL_ID_PERSONGROUPS, URL_ID_PERSONGROUPS, URL_ID_PERSONGROUPS, URL_ID_PERSONGROUP), 'ccb.apps.groups_networks.views.persongroup_invitation_list'),    
    ...
    url(r'^%s/(?P<slug>[^/]+)/member/(?P<user_id>[0-9]+)/edit/$' % URL_ID_PERSONGROUP, 'ccb.apps.groups_networks.views.edit_group_member'),
    
    url(r'^%s/(?P<slug>[^/]+)/projects/$' % URL_ID_PERSONGROUP, object_detail, dict(group_details_info, template_name="groups_networks/persongroups/group_projects.html")),
    url(r'^%s/(?P<slug>[^/]+)/%s/$' % (URL_ID_PERSONGROUP, URL_ID_EVENTS), object_detail, group_details_info),
    
    url(r'^lists/$', 'jetson.apps.utils.views.object_list', list_list_info),
    
    url(r'^(?P<section>%s|%s)/(?P<slug>[^/]+)/invited/(?P<encrypted_email>.+)/$' % (URL_ID_INSTITUTION, URL_ID_PERSON), 'ccb.apps.groups_networks.views.confirm_invitation'),


    # used for wrapping the reviews see 
    url(r'^(%s|%s|%s|%s|%s)/(?P<slug>[^/]+)/post/$' % (URL_ID_EVENT, URL_ID_DOCUMENT, URL_ID_PERSONGROUP, URL_ID_INSTITUTION, URL_ID_PERSON), 'ccb.apps.site_specific.views.wrap_post_comment'),
    
    url(r'^(%s|%s|%s|%s|%s)/(?P<slug>[^/]+)/deletereview/(?P<id>\d+)/$' % (URL_ID_EVENT, URL_ID_DOCUMENT, URL_ID_PERSONGROUP, URL_ID_INSTITUTION, URL_ID_PERSON), 'ccb.apps.site_specific.views.delete_review'),
    ...
    # claiming
    url(r'^(?P<ot_url_part>%s|%s|%s)/(?P<slug>[^/]+)/claim/$' % (URL_ID_INSTITUTION, URL_ID_EVENT, URL_ID_DOCUMENT), 
         'ccb.apps.site_specific.views.claim_object'
    ),
    ...
    url(
        ...
        dict(event_list_info, template_name="site_specific/kreativarbeiten_events.html", slug="NellyHolle"),
